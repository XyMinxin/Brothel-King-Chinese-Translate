# TODO: Translation updated at 2024-04-27 18:30

# game/BKhelp.rpy:464
translate schinese help_798c3657:

    # "Food effects reset."
    "食物效果已重置。"

# game/BKhelp.rpy:470
translate schinese help_7bc228f6:

    # "The number of times a girl pack picture is drawn in-game will now be tracked and visible in the gallery."
    "现在记录并在画廊中显示游戏中绘制女孩图包图片的次数。"

# game/BKhelp.rpy:484
translate schinese help_b244640b:

    # "Girl pack pictures will no longer be counted."
    "女孩图包图片不再计数。"

# game/BKhelp.rpy:492
translate schinese help_db2b02a5:

    # "Cheats are only available from the {b}main screen{/b} to avoid unexpected crashes."
    "作弊功能只能在{b}主界面{/b}使用,以避免出现意外导致游戏崩溃。"

# game/BKhelp.rpy:518
translate schinese help_9d15c261:

    # "There are no active mods for this game, or the mods do not provide options."
    "这个游戏没有已激活的Mod,或者Mod没有提供开关选项。"

# game/BKhelp.rpy:529
translate schinese help_88a2ee38:

    # "You can ask the game to {b}ignore{/b} a specific picture from a girlpack by hitting the 'DELETE' key while it is displayed.\n[nb] picture(s) are currently ignored."
    "你可以通过在显示时按下'DELETE'键来{b}屏蔽{/b}指定的女孩图包图片。\n当前已忽略[nb]张图片。"

# game/BKhelp.rpy:742
translate schinese help_main_6b7b0671:

    # sill happy "This location is the main hub of your brothel, from where you can control everything in your business."
    sill happy "这里是青楼的主页面,你可以在这里管理你的经营业务。"

# game/BKhelp.rpy:744
translate schinese help_main_70c1f886:

    # sill "On top of the screen, you can see from left to right: the {b}current date{/b}, your available {b}gold{/b}, your remaining {b}actions{/b} and {b}mana{/b} for the day, and the {b}Help{/b} button. Hints may also appear if you hover over something."
    sill "在屏幕顶部,你可以从左到右看到:{b}当前日期{/b}、持有的{b}金币{/b}、剩余的{b}行动力{/b}和{b}法力{/b},以及{b}帮助{/b}按钮。如果将鼠标移到按钮上,也可能会出现提示。"

# game/BKhelp.rpy:747
translate schinese help_main_c972cd35:

    # sill "The {b}Character{/b} tab allows you to see more information about yourself."
    sill "{b}角色状态{/b}菜单可以让你查看更多关于自己的信息。"

# game/BKhelp.rpy:749
translate schinese help_main_0e0ee736:

    # sill "The {b}Girls{/b} tab is where you interact with your slavegirls in all sorts of ways. It is very important!"
    sill "{b}女孩{/b}菜单是你与奴隶互动的重要地方,有各种各样的互动方式。这很重要!"

# game/BKhelp.rpy:751
translate schinese help_main_104377ab:

    # sill "The {b}Brothel{/b} tab allows you to upgrade your brothel and set up your advertising, security and maintenance. Don't neglect this one!"
    sill "{b}青楼管理{/b}菜单允许你升级青楼,设置广告、安保和清洁维护。不要忽视它!"

# game/BKhelp.rpy:754
translate schinese help_main_c0ded2fc:

    # sill "From the Brothel tab, you can also access the {b}Wagon{/b} to build furniture and set up {b}Customer options{/b}."
    sill "在青楼管理界面,你还可以进入{b}改造{/b}菜单来建造家具,并调整{b}经营策略{/b}。"

# game/BKhelp.rpy:757
translate schinese help_main_79c98d2c:

    # sill "The {b}Farm{/b} tab allows you to visit Gizel and her spooky farm. This will only lead to trouble, if you ask me."
    sill "{b}奴隶农场{/b}菜单让你可以拜访吉泽和她阴森的农场。要是你问我，我只能说这地方将导致不幸。"

# game/BKhelp.rpy:759
translate schinese help_main_d774e7a6:

    # sill "The {b}City{/b} tab allows you to visit the city and its available district. Give it a go if you get bored here!"
    sill "{b}探索城市{/b}可以探访城市及其各个区域。如果在这里感到无聊,不妨去试试看。"

# game/BKhelp.rpy:761
translate schinese help_main_9b92e5a1:

    # sill "The {b}Slave Market{/b} is bustling with sex slaves and the people who buy and sell them! Use it to get fresh slaves - if you can afford it."
    sill "{b}奴隶市场{/b}熙熙攘攘,到处都是买卖性奴的人!在这里获取新的奴隶 - 只要你有钱。"

# game/BKhelp.rpy:763
translate schinese help_main_f1d1ac57:

    # sill "The {b}Shop{/b} is where you can buy all sorts of mundane items for you or your girls. Maybe you'll find a bargain?"
    sill "{b}商店{/b}是你可以为自己或女孩购买各种日用品的地方。也许你可以找到便宜货?"

# game/BKhelp.rpy:765
translate schinese help_main_478cb481:

    # sill "The {b}Postings{/b} tab allows you to look for ads about quests and classes for your girl. It is useful to build up their skills and reputation."
    sill "{b}公告大厅{/b}可以查找任务和培训课程,对于提升女孩的技能和声望很有用。"

# game/BKhelp.rpy:767
translate schinese help_main_e66bb697:

    # "From the main screen, you can access the option menu by right clicking or pressing 'Esc'. You can save or load a game and change all sorts of useful options from there."
    "在主界面,你可以单击鼠标右键或按'Esc'来访问选项菜单。你可以在那里保存或读取游戏,并更改各种有用的选项。"

# game/BKhelp.rpy:779
translate schinese help_MC_1d5a2b33:

    # sill "{b}The Character tab{/b}. This is you, Master! From here, you can see your personal information, change your portrait, manage your items and your spells." nointeract
    sill "{b}角色菜单{/b}。这就是你的数据,主人!在这里,你可以查看个人信息,更改肖像,管理物品和法术。" nointeract

# game/BKhelp.rpy:783
translate schinese help_MC_cfa38b07:

    # sill "Sure thing! Just click on your name, and choose a new one. Oh, I'm curious!!! What's it gonna be?"
    sill "当然!只需要点击你的名字,就可以改名了。哦,我很好奇!!!你会选什么名字呢?"

# game/BKhelp.rpy:787
translate schinese help_MC_73802f03:

    # sill "First, let me tell you about your character class. Your class affects events in the game, your available spells and starting skills."
    sill "首先,让我告诉你你的角色职业。你的职业会影响游戏中的事件,可用的法术和起始技能。"

# game/BKhelp.rpy:791
translate schinese help_MC_590d6db5:

    # sill "You are a warrior, a skilled fighter with a tough spirit. Your social skills are a bit lacking, though, if I may say so."
    sill "你是一名武士,是一个技艺高超的战士,具有坚韧的精神。不过你的社交技能略有欠缺,这也难免。"

# game/BKhelp.rpy:793
translate schinese help_MC_12ca863b:

    # you "Grumph."
    you "哼。"

# game/BKhelp.rpy:797
translate schinese help_MC_ac7724cb:

    # sill "You are a wizard, skilled in spellcasting and with some wits to match. Don't get into close combat, though, this is not your style."
    sill "你是一名法师,擅长释放法术,智力过人。不过请不要陷入近身战斗,这并不适合你。"

# game/BKhelp.rpy:799
translate schinese help_MC_0437b977:

    # you "I wouldn't want my hands to get dirty anyway. What with this expensive manicure I just got."
    you "我也不想把手弄脏。手部保养可是很贵的。"

# game/BKhelp.rpy:803
translate schinese help_MC_f335efcb:

    # sill "You are good at barter and trade, and have learned the skills to survive in the streets. Magic is not your forte, however."
    sill "你很擅长讨价还价和谈判,你学会了在街头生存的技能。但魔法并非你的长项。"

# game/BKhelp.rpy:805
translate schinese help_MC_ee2db8f0:

    # you "Well, there were all those books to read, so... I figured I'd just sell them."
    you "嗯,有那么多书要读,所以...我想我还不如卖掉它们。"

# game/BKhelp.rpy:807
translate schinese help_MC_a414a0e4:

    # sill "You are currently level [MC.level]. Each new level will grant you a skill point which you may use to boost your skills."
    sill "你目前的等级是[MC.level]。每提升一级都会得到一个技能点用于提升技能。"

# game/BKhelp.rpy:809
translate schinese help_MC_dc0e11f0:

    # sill "You may also learn new {b}Spells{/b} and {b}Talents{/b}, so it's worth getting all the {b}prestige{/b} you can get."
    sill "你也可以学习新的{b}法术{/b}和{b}天赋{/b},所以获得尽可能多的{b}声望{/b}是很重要的。"

# game/BKhelp.rpy:811
translate schinese help_MC_7406d225:

    # sill "There are four skills you need to be concerned with: {b}Strength{/b}, {b}Spirit{/b}, {b}Charisma{/b}, and {b}Speed{/b}."
    sill "你需要关注的四项技能是:{b}力量{/b}、{b}精神{/b}、{b}魅力{/b}和{b}速度{/b}。"

# game/BKhelp.rpy:813
translate schinese help_MC_ba455e04:

    # sill "{b}Strength{/b} affects how well you perform physical tasks and hold your own in close combat. It also boosts your brothel security if you have remaining AP at the end of the day."
    sill "{b}力量{/b}影响你完成体力劳动和格斗的结果。它也会让你的青楼更加安全,前提是你在当天还有剩余的行动力。"

# game/BKhelp.rpy:815
translate schinese help_MC_2e786f0e:

    # sill "{b}Spirit{/b} affects how well you can sense magic and cast spells. Spirit lowers the cost of class spells for all player classes."
    sill "{b}精神{/b}影响你感知魔法和施展法术的结果。精神会降低所有玩家职业的法术消耗。"

# game/BKhelp.rpy:817
translate schinese help_MC_d2444d4a:

    # sill "{b}Charisma{/b} affects your social skills and people's reactions to dialog, including your girls."
    sill "{b}魅力{/b}影响你的社交技能和人们对话的反应,包括你的女孩。"

# game/BKhelp.rpy:819
translate schinese help_MC_e1179f3e:

    # sill "{b}Speed{/b} affects how many actions are available to you every day."
    sill "{b}速度{/b}影响你每天的行动点数。"

# game/BKhelp.rpy:821
translate schinese help_MC_6154eff7:

    # sill "Finally, you can find information about your {b}beliefs{/b} and {b}alignment{/b}. This is impacted by the actions you take, and used during dialog and story events."
    sill "最后,你可以查看你的{b}信仰{/b}和{b}阵营{/b}信息。这会受到你的行动影响,并在对话和剧情事件中产生不同的结果。"

# game/BKhelp.rpy:827
translate schinese help_MC_f6bcc6e6:

    # sill "Just click on the arrows to select your portrait. This is just for flavor and doesn't affect the game in any other way."
    sill "点击箭头就可以切换你的头像。这只是装饰性的,不会影响游戏。"

# game/BKhelp.rpy:829
translate schinese help_MC_fda9a393:

    # sill "By the way, if you don't like those, did you know you can add more portraits in the {b}game\\MC\\{/b} folder?"
    sill "顺便说一下,如果你不喜欢这些,你知道吗?你可以在{b}game\\MC\\{/b}文件夹里添加更多头像。"

# game/BKhelp.rpy:834
translate schinese help_MC_6816548b:

    # sill "{b}Prestige{/b} reflects your character's renown in Zan. Earning prestige allows you to level up and get skill points."
    sill "{b}声望{/b}反映了你在泽恩的声望。获得更多的声望才能让你升级并获得技能点。"

# game/BKhelp.rpy:836
translate schinese help_MC_2e21e849:

    # sill "You earn prestige everytime you or your girls have sex. You also earn prestige when a girl levels up."
    sill "每当你或你的女孩发生性行为,你都会获得声望。当一个女孩升级时,你也会获得声望。"

# game/BKhelp.rpy:838
translate schinese help_MC_98e36b67:

    # sill "You may use {b}Skill points{/b} to increase your {b}Strength{/b}, {b}Spirit{/b}, {b}Charisma{/b}, and {b}Speed{/b}, up to a maximum of 10."
    sill "你可以使用{b}技能点{/b}来提升你的{b}力量{/b}、{b}精神{/b}、{b}魅力{/b}和{b}速度{/b},每项属性最高提升到10。"

# game/BKhelp.rpy:843
translate schinese help_MC_8879ba31:

    # sill "It's easy! Just click on the item you want to use, and select 'use' or 'equip'. You can unequip an item in the same way."
    sill "很简单!只需点击你想使用的物品,选择'使用'或'装备'。同样的方法也可以卸下装备。"

# game/BKhelp.rpy:845
translate schinese help_MC_dc079105:

    # sill "Please note that you can only use items that are intended {color=[c_steel]}{b}for you{/b}{/color} and not {color=[c_pink]}{b}for your girls{/b}{/color}. Watch for the color in the item tab."
    sill "请注意,你只能使用{color=[c_steel]}{b}主角限定{/b}{/color}的物品,而不能使用{color=[c_pink]}{b}女孩限定{/b}{/color}的物品。注意查看物品栏中的描述。"

# game/BKhelp.rpy:851
translate schinese help_MC_00da599b:

    # sill "You can learn new {b}Spells{/b} and {b}Talents{/b} from your character class when increasing your level."
    sill "当你升级时,你可以学习新的职业{b}法术{/b}和{b}天赋{/b}。"

# game/BKhelp.rpy:853
translate schinese help_MC_b3706761:

    # sill "Spells must be cast and cost mana. Talents are always active."
    sill "法术必须主动释放并消耗法力值。天赋则始终处于激活状态。"

# game/BKhelp.rpy:855
translate schinese help_MC_045ee96f:

    # sill "After you learn a Spell, you can use it any time from the {b}Spellbook{/b}. Press 'K' for fast access to the Spellbook."
    sill "学会一个法术后,你可以随时在{b}法术书{/b}中使用它。按'K'键可快速访问法术书。"

# game/BKhelp.rpy:857
translate schinese help_MC_e6d656f6:

    # sill "Casting spells costs {b}Mana{/b}. Mana Points (MP) depend on your Spirit skill. They are replenished every day."
    sill "施法会消耗{b}法力值{/b}。法力值(MP)取决于你的精神属性。它们会在每天回满。"

# game/BKhelp.rpy:859
translate schinese help_MC_a7da7429:

    # "{b}Left click{/b} on a spell in the spells tab to cast it."
    "在法术栏中{b}左键单击{/b}一个法术来施放它。"

# game/BKhelp.rpy:861
translate schinese help_MC_099c7f62:

    # sill "Did you know that you can {b}auto-cast{/b} spells? If you automate a spell, you will keep casting it every day - provided you have enough leftover MP."
    sill "你知道你可以{b}自动施法{/b}吗?如果你想自动释放某个法术,只要你有足够的法力值,它就会每天持续自动施放。"

# game/BKhelp.rpy:864
translate schinese help_MC_cd3fe0cf:

    # "{b}Right click{/b} on a spell in the spells tab to cast it. You can choose between auto-casting at night or in the morning by right-clicking again."
    "在法术栏中{b}右键单击{/b}一个法术。你可以选择在晚上或清晨自动施放。"

# game/BKhelp.rpy:866
translate schinese help_MC_9f6110f2:

    # sill "But no matter what you do, you may only cast one spell of a given type every day. For instance, wizards can only have one aura active at any given time."
    sill "但无论你做什么,你一天内只能施放一次某种类型的法术。比如法师职业一天只能有一个光环法术生效。"

# game/BKhelp.rpy:869
translate schinese help_MC_746f86b6:

    # sill "Some spells are only active for one night, and some last until a specific event occurs. {b}Hover{/b} over a spell to see its description."
    sill "有些法术的效果只能持续一天,有些则持续到特定事件发生。将鼠标{b}悬停{/b}在法术上查看描述。"

# game/BKhelp.rpy:885
translate schinese help_girls_0a07f586:

    # sill "{b}The Girls' rooms{/b}. This is where you can check your girls' information, interact with them, choose their job and schedule, and more." nointeract
    sill "{b}女孩房间{/b}。这里你可以查看女孩信息、与她们互动、安排她们的排班和休息日等。" nointeract

# game/BKhelp.rpy:889
translate schinese help_girls_7a317d3c:

    # sill happy "You sure can! This is your privilege as the slave's master."
    sill happy "当然可以!这是你作为奴隶主的特权。"

# game/BKhelp.rpy:891
translate schinese help_girls_414576d0:

    # sill "Just click on her {b}name{/b}, and choose a new one"
    sill "只需要点击她的{b}名字{/b}，然后就可以改名了"

# game/BKhelp.rpy:894
translate schinese help_girls_38b9d150:

    # sill sad "You can't rename me, though. I like my name!"
    sill sad "不过你无法给我改名。我喜欢我的名字!"

# game/BKhelp.rpy:896
translate schinese help_girls_1687264a:

    # you "Wait a minute. Didn't you say it was my privilege to rename you, as your owner?"
    you "等等。你不是说让我有权给你改名,因为我是你的主人吗?"

# game/BKhelp.rpy:898
translate schinese help_girls_df66ced2:

    # sill sad "B-But..."
    sill sad "但是..."

# game/BKhelp.rpy:904
translate schinese help_girls_3cae61af:

    # you "Go back to Sill."
    you "改回希露。"

# game/BKhelp.rpy:906
translate schinese help_girls_c5dcb8e1:

    # sill "Th... Thank you! Finally!"
    sill "太...太感谢了!终于!"

# game/BKhelp.rpy:931
translate schinese help_girls_1e495c41:

    # you "Your new name is... [new_name]"
    you "你的新名字是...[new_name]"

# game/BKhelp.rpy:933
translate schinese help_girls_3a58de2f:

    # sill "Wh... What?"
    sill "什...什么?"

# game/BKhelp.rpy:935
translate schinese help_girls_206e8fd0:

    # you "You heard me... [new_name]."
    you "你听到了...[new_name]。"

# game/BKhelp.rpy:941
translate schinese help_girls_51f6aa12:

    # sill sad "Oh no... T_T"
    sill sad "噢不...T_T"

# game/BKhelp.rpy:948
translate schinese help_girls_126b20c1:

    # sill "Your girl's {b}defense{/b} is the number seen in the shield icon on top of your girl's profile. You can improve it by giving her certain perks or items."
    sill "女孩的{b}自卫能力{/b}是她个人资料顶部盾牌图标上显示的数值。你可以通过给予她某些特性或物品来提升这一指标。"

# game/BKhelp.rpy:950
translate schinese help_girls_f2b39454:

    # sill "Defense allows her to hold her own in a fight, because some customers do get rough. Being assaulted might not only hurt her, but damage her trust and her mood."
    sill "自卫能力可以帮助她在战斗中保护自己,有些客户可能会很粗暴。遭受攻击不仅会伤害她,还可能降低她的信任和情绪。"

# game/BKhelp.rpy:953
translate schinese help_girls_aa00ccab:

    # sill sad "But if you give her a weapon, watch out! If she rebels, she might well use it against you."
    sill sad "不过如果你给她一件武器,要小心!如果她反抗你,她可能会用你给她的武器对付你。"

# game/BKhelp.rpy:955
translate schinese help_girls_ce881a8c:

    # sill happy "Your girl's {b}Mood{/b} can be seen as a circle on top of your girl's profile. Its color indicates how happy or discontent your girl is."
    sill happy "女孩的{b}情绪{/b}可以在她个人资料顶部看到一个圆圈。其颜色表示她当前的愉悦或不满程度。"

# game/BKhelp.rpy:957
translate schinese help_girls_55756123:

    # sill "Keep your girl happy so that she doesn't rebel and progresses faster! Happiness comes from many factors, such as resting time, accommodation, security, and how much love and respect your girl has for you."
    sill "保持女孩的情绪高涨,这样她才不会反抗你并能更快地进步!幸福感来自于许多因素,如休息时间、住宿条件、安全感,以及她对你的好感度与尊重。"

# game/BKhelp.rpy:960
translate schinese help_girls_b61c19a1:

    # sill "However, you can also keep a girl sufficiently scared so that she won't refuse work or run away from you. Fear can be a powerful tool."
    sill "不过,你也可以适度地让她感到害怕,这样她就不会拒绝工作或是从你这里逃走。恐惧也能成为一种强大的工具。"

# game/BKhelp.rpy:962
translate schinese help_girls_e8a949cb:

    # sill sad "But please, there is no need to act like this with me! I'm always good!!!"
    sill sad "但是请记住,对我来说这是不必要的!我一直都很听话!!!"

# game/BKhelp.rpy:964
translate schinese help_girls_ed4d6ad7:

    # sill "Let me tell you about {b}Love{/b} and {b}Fear{/b}."
    sill "让我告诉你关于{b}好感度{/b}和{b}恐惧值{/b}的事。"

# game/BKhelp.rpy:966
translate schinese help_girls_cde91123:

    # sill "Depending on how you treat them, girls will come to {b}love{/b} or {b}hate you{/b}. They will also {b}fear{/b} or {b}trust{/b} you."
    sill "根据你对待方式的不同,女孩会{b}喜欢{/b}或{b}憎恨{/b}你。她们也会{b}害怕{/b}或{b}信任{/b}你。"

# game/BKhelp.rpy:968
translate schinese help_girls_e2ea8320:

    # sill "But be careful... Some girls will take advantage of your leniency if you are too nice... Not me, of course!!!"
    sill "不过要小心...如果你太善良,有些女孩会利用你的宽容。当然,除了我!"

# game/BKhelp.rpy:970
translate schinese help_girls_ed5462ef:

    # sill "Your {b}Alignment{/b} will have consequences on how fast your girls' {b}Love{/b} and {b}Fear{/b} evolve."
    sill "你的{b}阵营{/b}会影响女孩{b}好感度{/b}与{b}恐惧值{/b}的变化速度。"

# game/BKhelp.rpy:972
translate schinese help_girls_49418e04:

    # sill "Unlike Mood, Love and Fear change slowly. Keep it in mind before you decide how to treat your girls."
    sill "与情绪不同,好感度与恐惧值的变化是缓慢的。在决定如何对待女孩之前要记住这一点。"

# game/BKhelp.rpy:981
translate schinese help_girls_02e1052c:

    # sill "What are you interested in?" nointeract
    sill "你对什么感兴趣?" nointeract

# game/BKhelp.rpy:987
translate schinese help_girls_e4e73ca0:

    # "You can use the shortcuts to sort your girls by name, job, level or rank (right-click on a sort method to sort backwards)."
    "你可以使用快捷方式按名字、工作、等级或排名对女孩进行排序(右击某个排序方式可以反向排序)。"

# game/BKhelp.rpy:991
translate schinese help_girls_96e2c0c0:

    # sill "As your girl works, goes on quests or attends classes she will become more experienced. Once she has enough {b}experience{/b} (XP), she will be ready to reach a new level."
    sill "当你的女孩工作、进行任务或参加培训时,她会获得更多经验值。一旦她积攒了足够的{b}经验值{/b}(XP),她就可以升级了。"

# game/BKhelp.rpy:994
translate schinese help_girls_fba3a29a:

    # sill "New levels will allow you to increase your girls main skills with {b}skill points{/b}."
    sill "每次升级都会让你获得{b}属性点{/b},用于提升女孩的主要技能。"

# game/BKhelp.rpy:996
translate schinese help_girls_62b8f040:

    # sill "Also, every level, you will receive {b}perk points{/b}. Perk points allow you to unlock some useful perks in a girl's {b}zodiac{/b} trees."
    sill "此外,每次升级你还会获得{b}天赋点{/b}。天赋点可以用来解锁女孩{b}天赋树{/b}上的一些有用的天赋。"

# game/BKhelp.rpy:998
translate schinese help_girls_2cbb8e75:

    # sill "The maximum level a girl can reach is capped by her current {b}rank{/b}, however. Don't forget to rank up!"
    sill "女孩能够达到的最高等级受她当前{b}阶级{/b}的限制。所以别忘了提升阶级!"

# game/BKhelp.rpy:1006
translate schinese help_girls_b28a8a80:

    # sill "Every time your girl accomplishes a specific job or sex act, she will receive {b}job points{/b} (JP). Job points will allow her to automatically increase her job level."
    sill "每当你的女孩完成一项特定工作或性行为时,她都会获得{b}职业点数{/b}(JP)。职业点数可以帮助她提升相关的职业等级。"

# game/BKhelp.rpy:1009
translate schinese help_girls_3bf256e7:

    # sill "Improving job level will in turn raise a girl's relevant {b}skills{/b} by a good amount, and improve {b}customer satisfaction{/b} for this particular job or sex act."
    sill "提升职业等级将大幅提高女孩相关的{b}技能{/b}属性,并提高这项工作或性行为的{b}客户满意度{/b}。"

# game/BKhelp.rpy:1011
translate schinese help_girls_938349fe:

    # sill "The maximum job level a girl can reach is capped by her current {b}rank{/b}, however. But she can always learn a new job!"
    sill "但一个女孩所能达到的最高职业等级是受她当前的{b}等级{/b}限制的。不过她可以去做其他工作！"

# game/BKhelp.rpy:1013
translate schinese help_girls_e4e73ca0_1:

    # "You can use the shortcuts to sort your girls by name, job, level or rank (right-click on a sort method to sort backwards)."
    "您可以使用快捷方式按名字、职业、等级或等级（右击某一排序方式即可倒序）对您的女孩进行排序。"

# game/BKhelp.rpy:1023
translate schinese help_girls_1ba0aa5a:

    # sill "What do you want to know about?" nointeract
    sill "您想了解什么？" nointeract

# game/BKhelp.rpy:1027
translate schinese help_girls_8570726b:

    # sill "The first button is the {b}job{/b} button. It allows you to change a girl's job. Keep in mind that she may refuse to be a whore if her obedience and/or libido is too low."
    sill "第一个按钮是{b}工作{/b}按钮。它可以让您更改一个女孩的工作项目。注意,如果她的服从度和/或性欲太低,她可能会拒绝成为妓女。"

# game/BKhelp.rpy:1029
translate schinese help_girls_e6f76cb7:

    # sill sad "You cannot change a girl's job while she is away, hurt, sick or exhausted. Sorry."
    sill sad "当一个女孩外出、受伤、生病或精疲力尽时,您是无法更改她的工作的。很抱歉。"

# game/BKhelp.rpy:1033
translate schinese help_girls_bb2ea651:

    # sill "You can set a weekly schedule for your girls using the {b}schedule{/b} button."
    sill "您可以使用{b}排班表{/b}功能为您的女孩安排每周的工作计划。"

# game/BKhelp.rpy:1035
translate schinese help_girls_7fe79f90:

    # sill "Girls cannot work every day, or they will deplete their energy and become sick. A girl can work a half shift on certain days if you want to spare some of her energy."
    sill "女孩们不能永不停歇地工作,否则她们的精力会耗尽,染上疾病。如果您想节省一些女孩的精力,可以让她们只上半天班。"

# game/BKhelp.rpy:1040
translate schinese help_girls_ea270c35:

    # sill "The {b}interact{/b} button allows you to visit your girl and talk to her. You might find it useful to know your girls better, and maybe try to influence their behavior."
    sill "{b}互动{/b}按钮可让您探视您的女孩并与她们交谈。了解您的女孩可能会很有用,也许还能影响她们的行为。"

# game/BKhelp.rpy:1044
translate schinese help_girls_ae6acd8e:

    # sill "Use the {b}equip{/b} button to have a girl take, give, use or equip items. Some effects are permanent: think carefully before you use an item!"
    sill "使用{b}装备{/b}按钮让女孩拿取、退还、使用或装备物品。某些效果是永久性的:请谨慎考虑是否使用物品!"

# game/BKhelp.rpy:1048
translate schinese help_girls_8489c85b:

    # sill "Oh, that one is easy. Just click the {b}sell{/b} button to get rid of a girl. But not me, of course."
    sill "哦,那很简单。只需点击{b}出售{/b}按钮就可以和女孩说永别了。当然,不包括我。"

# game/BKhelp.rpy:1052
translate schinese help_girls_1c5940c4:

    # sill "The {b}level up{/b} or {b}rank up{/b} buttons become available once your girl is ready to advance. Otherwise, you can use the {b}perks{/b} button to check on your girl's current perks"
    sill "一旦您的女孩准备好晋级,{b}升级{/b}或{b}阶级提升{/b}按钮就会显示出来。与此同时,您可以在{b}特性{/b}窗口查看您的女孩当前的特性"

# game/BKhelp.rpy:1057
translate schinese help_girls_761e6c23:

    # sill "The {b}stats{/b} button is handy if you want to keep track of the latest information about your girl's performance."
    sill "如果您想了解女孩最近的表现情况,使用{b}数据{/b}按钮会很方便。"

# game/BKhelp.rpy:1073
translate schinese help_girls_e4e73ca0_2:

    # "You can use the shortcuts to sort your girls by name, job, level or rank (right-click on a sort method to sort backwards)."
    "您可以使用快捷方式按名字、职业、等级或等级（右击某一排序方式即可倒序）对您的女孩进行排序。"

# game/BKhelp.rpy:1089
translate schinese help_whores_3c2dd192:

    # sill sad "Turning a regular girl into a whore is not as easy as it seems. Girls need training before they will accept to be whores."
    sill sad "将普通奴隶变成妓女并非易事。女孩们需要接受培训才能成为妓女。"

# game/BKhelp.rpy:1091
translate schinese help_whores_8f5570aa:

    # sill happy "There are several ways to train your girls, however."
    sill happy "不过,有好几种方法可以训练您的女孩。"

# game/BKhelp.rpy:1093
translate schinese help_whores_a4f945a8:

    # sill "First, you can train girls yourself. To do this, choose to 'interact' with them."
    sill "首先,您可以亲自训练女孩。要做到这一点,请与她们\"互动\"。"

# game/BKhelp.rpy:1095
translate schinese help_whores_a3048321:

    # sill "The typical way of training sex slaves is to go soft on them at first, lecturing them about sex and making them comfortable with their body, until they are ready for more advanced sex acts."
    sill "培训性奴的传统方式就是先温和地对待她们,讲授有关性的知识,让她们做好心理准备,直到她们准备好接受更刺激的性行为。"

# game/BKhelp.rpy:1098
translate schinese help_whores_4a609ba6:

    # sill sad "But some brothel masters choose to use fear and coercion instead... Some even brutally rape their slaves into submission."
    sill sad "但是一些青楼的主人选择使用恐吓和强制手段....甚至残酷地强奸她们使之服从。"

# game/BKhelp.rpy:1100
translate schinese help_whores_d6dcee3c:

    # sill happy "Other choose to use magic... In the end, it's up to you to decide how you prefer to train your girls."
    sill happy "其他人则选择使用魔法催眠....归根结底,如何训练您的女孩取决于您自己。"

# game/BKhelp.rpy:1102
translate schinese help_whores_b8b5b024:

    # sill "If you do not train your girls yourself, there are other ways to get them to become whores."
    sill "如果您自己不训练女孩,也有其他方法让她们成为妓女。"

# game/BKhelp.rpy:1104
translate schinese help_whores_3b10d222:

    # sill "Working in the brothel will make your girls more receptive to sex work over time. Sometimes, interacting with customers will make them more interested in sex... Until they are ready for the next step."
    sill "在青楼工作会使您的女孩慢慢接受性行为。有时,与客户的互动也会让她们对性行为产生更多兴趣....直到她们准备好迈入下一步。"

# game/BKhelp.rpy:1107
translate schinese help_whores_2cc36857:

    # sill "This is quite a slow process, however. Obedient, libidinous, or sensitive girls are more likely to progress that way."
    sill "不过,这个过程相当缓慢。服从、性欲旺盛或敏感的女孩更有可能通过这种方式取得进步。"

# game/BKhelp.rpy:1109
translate schinese help_whores_ec8f5d4e:

    # sill "Alternatively, you can buy trained girls in the slavemarket. Those girls have been trained for sex already, although sometimes it's hard to know just how effective this training was."
    sill "另外,您也可以在奴隶市场购买受过训练的女孩。这些女孩已经接受过性方面的培训,尽管培训时间不能和水平划等号。"

# game/BKhelp.rpy:1112
translate schinese help_whores_bc05ae36:

    # sill "Trained girls are more expensive to buy, of course. On the plus side, they also sell for more."
    sill "受过训练的女孩当然也更加昂贵。不过好处是,她们的售卖价格也更高。"

# game/BKhelp.rpy:1114
translate schinese help_whores_d15b16e7:

    # sill "At first, you will only have access to girls with a low level of training, but the quality of available girls will improve as you unlock higher brothel licenses."
    sill "刚开始,您只能接触到培训程度较低的女孩,但随着您解锁更高级的经营许可证,可供选择的奴隶质量也会提高。"

# game/BKhelp.rpy:1117
translate schinese help_whores_c4a5057c:

    # sill "Finally, please know that all girls have different tastes in sex acts... They might even have some strange fetishes. If you discover them, you can make your training more effective."
    sill "最后,请注意每个女孩对性行为都有不同的偏好...她们甚至可能有一些奇怪的性癖。如果您发现了它们,您的训练就会更加有效。"

# game/BKhelp.rpy:1129
translate schinese skills_introduction_02e1052c:

    # sill "What are you interested in?" nointeract
    sill "您对什么感兴趣?" nointeract

# game/BKhelp.rpy:1133
translate schinese skills_introduction_02a5be0a:

    # sill "There are two types of skills for your girls: main skills and sex skills. Working jobs may increase, and sometimes decrease, specific stats."
    sill "您的女孩有两类属性:主要属性和性技能属性。从事不同的工作可能会提高或降低特定的属性。"

# game/BKhelp.rpy:1135
translate schinese skills_introduction_90d6f80a:

    # sill "Main skills are generic skills that every girl can improve working on any job."
    sill "主要属性是每个女孩都可以通过从事工作来提高的通用属性。"

# game/BKhelp.rpy:1137
translate schinese skills_introduction_386a04d1:

    # sill "Sex skills are skills that can only be improved by doing specific sex acts. They are the most important skills for a whore."
    sill "性技能是只能通过从事特定性行为来提高的技能。它们是妓女最重要的技能。"

# game/BKhelp.rpy:1139
translate schinese skills_introduction_0e5e29c0:

    # sill "Your girl will choose which sex acts to perform on her own, but you can make her perform only specific acts if you tick or untick the box next to each sex skill. Keep in mind, however, that some girls may refuse certain sex acts if their libido and/or obedience are too low."
    sill "你的女孩会自己选择要对她施加的性行为,但如果你勾选或取消勾选性技能旁的复选框,你就可以让她只执行特定的性行为。不过请记住,如果某些女孩的性欲和/或服从度太低,她们可能会拒绝进行某些性行为。"

# game/BKhelp.rpy:1142
translate schinese skills_introduction_a86f430b:

    # sill "You can hover over a specific skill to learn more about it."
    sill "你可以把鼠标悬停在某个具体的技能上,了解更多相关信息。"

# game/BKhelp.rpy:1144
translate schinese skills_introduction_c3969182:

    # sill "The maximum skill a girl can have is limited by her current rank, however. Be sure to rank up your girls!"
    sill "一个女孩的属性上限受制于她当前的等级,所以一定要尽可能的提升女孩的等级!"

# game/BKhelp.rpy:1148
translate schinese skills_introduction_0fb90cee:

    # sill "Every girl has {b}3 traits{/b} that she is born with. You cannot change those."
    sill "每个女孩天生都自带{b}3个特质{/b},你无法改变这些特质。"

# game/BKhelp.rpy:1150
translate schinese skills_introduction_9df928f2:

    # sill "Some traits can have powerful effects on your girl, but beware! No one is perfect, so your girls will usually have a negative trait as well."
    sill "某些特质可能会对你的女孩产生强大的影响,要多留意!人无完人,所以你的女孩通常也会有一个负面特质。"

# game/BKhelp.rpy:1155
translate schinese skills_introduction_5d06222b:

    # sill "You can freely adjust {b}upkeep{/b}. Upkeep covers how much money you spend every night keeping your girl suitably fed and groomed."
    sill "你可以自由调整{b}保养费{/b}。保养费包含每晚的餐补和化妆打扮的开销。"

# game/BKhelp.rpy:1157
translate schinese skills_introduction_510ae8f6:

    # sill "Slave or not, your girls have some expensive tastes, you know! Giving your girls extra money will keep them content. Watch that you don't let their upkeep sink too low, however! They will be mad at you."
    sill "不管她是不是奴隶,女孩们总是喜欢奢侈的生活,你懂的!提高工资会让她们更加满足。不过,也要注意别让她们的保养费降得太低,否则她们会生你的气。"

# game/BKhelp.rpy:1169
translate schinese bis_introduction_ca0dddba:

    # sill happy "Master! It seems you have trained your first bisexual girl."
    sill happy "主人!看来你已经训练出了第一个接受双飞玩法的女孩。"

# game/BKhelp.rpy:1172
translate schinese bis_introduction_958ca6ab:

    # sill "Would you like to learn more about how {b}bisexual{/b} whores work?" nointeract
    sill "你想了解{b}双飞{/b}是怎么一回事吗?" nointeract

# game/BKhelp.rpy:1174
translate schinese bis_introduction_ff9ff0da:

    # sill "Bisexual girls are able to have a threesome with a customer. For that, you will need {b}at least two bisexual whores{/b} active at the same time."
    sill "双飞可以让一个客人同时和两个女孩做爱。为此,你需要{b}至少有两个工作的，接受双飞玩法女孩{/b}。"

# game/BKhelp.rpy:1178
translate schinese bis_introduction_9ba21507:

    # sill "During a threesome, the lucky customer will be extra happy and pay {b}[bonus] of the usual tip{/b}."
    sill "在三人行中,幸运的客户会格外高兴,并支付{b}[bonus]倍{/b}的小费。"

# game/BKhelp.rpy:1180
translate schinese bis_introduction_d680755a:

    # sill "Each girl will use an {b}interaction{/b}. But your girls will also share the, ahem, load, so their {b}energy{/b} expenditure will be halved."
    sill "每个女孩都会消耗一次{b}服务{/b}次数。但你的女孩们也会彼此分担,所以她们的{b}精力{/b}消耗会减半。"

# game/BKhelp.rpy:1182
translate schinese bis_introduction_4724d70a:

    # sill "Bisexual girls are useful if you want to get the {b}maximum value{/b} out of a few customers!"
    sill "提供双飞服务的女孩很赚钱,如果你想从少数客户那里榨取{b}最大利润{/b}的话!"

# game/BKhelp.rpy:1184
translate schinese bis_introduction_807ea81c:

    # you "I see. Thanks!"
    you "我明白了,谢谢!"

# game/BKhelp.rpy:1187
translate schinese bis_introduction_6e4cab89:

    # sill "All right."
    sill "好的。"

# game/BKhelp.rpy:1194
translate schinese group_introduction_b67fee2c:

    # sill happy "Master! It seems you have trained your first group girl."
    sill happy "主人!看来你已经训练出了第一个接受群交的女孩。"

# game/BKhelp.rpy:1197
translate schinese group_introduction_54670519:

    # sill "Would you like to learn more about how {b}group{/b} works for whores?" nointeract
    sill "你想了解{b}群交{/b}是怎么一回事吗?" nointeract

# game/BKhelp.rpy:1199
translate schinese group_introduction_b9132224:

    # sill "Group girls are able to have orgies with several customers. For that, you will need {b}at least two willing customers{/b} that want the same sex act."
    sill "群交女孩能够同时与多个客户进行群交。为此,你需要{b}至少有两个愿意进行相同性行为的客户{/b}。"

# game/BKhelp.rpy:1203
translate schinese group_introduction_b6488f89:

    # sill "During an orgy, customers will have fun but are not ready to pay your girl as much. Each customer will pay {b}[bonus] of the usual tip{/b}."
    sill "在群交过程中,客户们会玩得很嗨,但却不愿意给你的女孩支付太多小费。每个客户只会支付{b}[bonus]倍{/b}的小费。"

# game/BKhelp.rpy:1205
translate schinese group_introduction_2f427d86:

    # sill "Of course, because a girl can service several customers with only one {b}interaction{/b}, it's still a good deal for you."
    sill "当然,因为一个女孩可以只消耗一次{b}互动{/b}却能服务多个客户,这对你来说依然是个不错的买卖。"

# game/BKhelp.rpy:1207
translate schinese group_introduction_f87a6bc3:

    # sill "You have to watch the energy level of your group girls, though. They will spend double or triple the {b}energy{/b} depending on how many customers there are."
    sill "不过你得注意你的群交女孩的精力水平。她们会根据参与的客户数量,消耗双倍或三倍的{b}精力{/b}。"

# game/BKhelp.rpy:1209
translate schinese group_introduction_da9455b8:

    # sill "Group girls are useful if you want to serve the {b}maximum number of customers{/b}!"
    sill "群交女孩很有用,如果你想{b}服务尽可能多的客户{/b}!"

# game/BKhelp.rpy:1211
translate schinese group_introduction_fb6e86b3:

    # you "I'll keep that in mind. Thanks!"
    you "我会牢记在心的,谢谢!"

# game/BKhelp.rpy:1214
translate schinese group_introduction_7062e2e4:

    # sill "Okay. Call me if you need anything..."
    sill "好的。有需要随时叫我..."

# game/BKhelp.rpy:1220
translate schinese jobs_introduction_71a28bb0:

    # sill "Waitresses, dancers, masseuses and geishas entertain several customers at once. Whores usually entertain one customer at a time, but they can have several customers per night."
    sill "服务生、脱衣舞女、按摩技师和表演艺妓可以同时接待多名客户。妓女一次只能服务一名客人,但她一晚上可以接待多个客户。"

# game/BKhelp.rpy:1225
translate schinese jobs_introduction_3226f923:

    # sill "What job do you want to know about?" nointeract
    sill "你想了解哪一种工作?" nointeract

# game/BKhelp.rpy:1229
translate schinese jobs_introduction_4436f236:

    # sill "The {b}tavern{/b} will allow you to train your girl as a {b}waitress{/b}. With time, they will start wearing sexy uniforms and providing all kinds of 'entertainment' to the customers."
    sill "{b}零点酒吧{/b}可以让女孩们作为{b}女服务生{/b}给客人端茶送酒。随着等级提高,她们的制服会越来越暴露、性感,客人可以在她们服务的时候随便揩油。"

# game/BKhelp.rpy:1232
translate schinese jobs_introduction_602834d7:

    # sill "Waitresses need {b}charm{/b} to keep the customers entertained. They also need a strong {b}constitution{/b}: working tables is not easy, you know! Finally, it cannot hurt if they are {b}beautiful{/b}, and have a good {b}body{/b}."
    sill "服务生需要{b}魅力{/b}来引起客户的兴趣。她们还需要强大的{b}体质{/b}:端盘子可不是件容易的事!最后,如果她们{b}外貌{/b}迷人并有不错的{b}身材{/b},那就再好不过了。"

# game/BKhelp.rpy:1237
translate schinese jobs_introduction_7f010990:

    # sill "The {b}strip club{/b} will allow you to train her as a {b}dancer{/b}. With time, they will remove more and more clothing, and take the customers to a room for a 'private dance'."
    sill "{b}激情夜店{/b}可以让女孩们作为{b}脱衣舞女{/b}在客人面前表演舞蹈。随着等级提高,她们会脱掉越来越多的衣服,还可以在包间为客人提供私人的舞蹈动作指导。"

# game/BKhelp.rpy:1240
translate schinese jobs_introduction_f0f985d0:

    # sill "You should pick girls with a good {b}body{/b} to be dancers. A {b}libidinous{/b} girl is always better, the customers can sense it. {b}Refinement{/b} and {b}charm{/b} are also good qualities for a dancer."
    sill "你应该选择{b}身材{/b}好的女孩来当舞女。{b}性欲{/b}旺盛的女孩最好不过,客户能感受得到她们表演的热情。{b}优雅{/b}和{b}魅力{/b}对舞女来说也是很重要的品质。"

# game/BKhelp.rpy:1245
translate schinese jobs_introduction_284e0e40:

    # sill "The {b}onsen{/b} will allow you to train her as a {b}masseuse{/b}. With time, they will provide more erotic massages to the customers, and eventually give them 'full service'"
    sill "{b}露天温泉{/b}可以让女孩们作为{b}按摩技师{/b}为客人提供按摩服务。随着等级提高,她们会给客户提供更多部位的按摩,最终提供全套服务。"

# game/BKhelp.rpy:1248
translate schinese jobs_introduction_80db185e:

    # sill "Masseuses should be {b}beautiful{/b} girls, to attract customers to the onsen. {b}Sensitive{/b} girls fare better as they can make the customer more comfortable. A good {b}body{/b} and a little {b}refinement{/b} are also important."
    sill "按摩技师应该是{b}外貌{/b}迷人的女孩,以吸引顾客到温泉消费。{b}敏感{/b}的女孩表现更好,因为她们能使顾客更加放松。良好的{b}身材{/b}和{b}优雅{/b}的气质也同样重要。"

# game/BKhelp.rpy:1253
translate schinese jobs_introduction_26ccde83:

    # sill "The {b}okiya{/b} will allow you to train her as a {b}geisha{/b}. With time, they'll learn a thousand ways to please their customers, and how to take care of their more 'special' requests."
    sill "{b}艺妓置屋{/b}可以让女孩们作为{b}表演艺妓{/b}在这里表演才艺。随着等级提高,她们将学会各种各样取悦客户的方式,以及如何应付他们的‘特殊’要求。"

# game/BKhelp.rpy:1256
translate schinese jobs_introduction_5374094c:

    # sill "Geishas should be {b}refined{/b} girls. {b}Obedience{/b} is a prized quality, to make the customers feel important. {b}Beauty{/b} and {b}charm{/b} also help make a perfect geisha."
    sill "表演艺妓应该是气质{b}优雅{/b}的女孩。{b}服从{/b}是一种备受重视的品质,可以让顾客感受到自己的重要性。{b}外貌{/b}和{b}魅力{/b}也有助于成就一个完美的艺妓。"

# game/BKhelp.rpy:1261
translate schinese jobs_introduction_c8a8066e:

    # sill "Girls need to be trained before they will accept to be whores. Forcing them never gives good results. Girls with a high libido or obedience can be trained faster."
    sill "女孩需要接受培训才能成为妓女。强扭的瓜不甜。性欲旺盛或者服从性强的女孩可以更容易地接受培训。"

# game/BKhelp.rpy:1263
translate schinese jobs_introduction_924e2632:

    # sill "There are {b}4 different sex acts{/b} a whore can perform. You can select which acts they may or may not perform by ticking or unticking the box next to each sex skill."
    sill "妓女可以和客人进行{b}4种不同{/b}的性行为。您可以通过勾选或取消每种性技能旁边的复选框来选择她们可以或不可以执行的行为。"

# game/BKhelp.rpy:1266
translate schinese jobs_introduction_cb224f7c:

    # sill "{b}Service{/b} includes all kinds of foreplay, including masturbation, handjobs and blowjobs. A good {b}service{/b} skill is required to perform service, of course, as well as good {b}sensitivity{/b}. {b}Charm{/b} and the {b}fetish{/b} skill also help."
    sill "{b}侍奉{/b}包括各种前戏,例如手淫、手交和口交。当然,良好的{b}侍奉{/b}技能,以及{b}敏感{/b}都是必需的。{b}魅力{/b}和{b}调教{/b}属性也有帮助。"

# game/BKhelp.rpy:1269
translate schinese jobs_introduction_f4b64573:

    # sill "{b}Sex{/b} means vaginal intercourse, of course. A high {b}sex{/b} skill and {b}libido{/b} gives the best results. {b}Beauty{/b} and the {b}service{/b} skill also boost sex."
    sill "{b}性交{/b}当然指正常的阴道性交。高{b}性交{/b}和{b}性欲{/b}会带来最佳效果。{b}美貌{/b}和{b}侍奉{/b}属性也有助于性行为。"

# game/BKhelp.rpy:1271
translate schinese jobs_introduction_ef2e62a3:

    # sill "{b}Anal{/b} means anal sex. A high {b}anal{/b} skill together with a good {b}constitution{/b} is a must. A good {b}body{/b} and the {b}sex{/b} skill also help."
    sill "{b}肛交{/b}指用肛门性交。{b}肛交{/b}属性和良好的{b}体质{/b}是必需的。良好的{b}身材{/b}和{b}性交{/b}属性也有帮助。"

# game/BKhelp.rpy:1273
translate schinese jobs_introduction_f7b43498:

    # sill "{b}Fetish{/b} includes BDSM, spanking, fisting, and all the weirder sexual practices. A girl needs a good {b}fetish{/b} and {b}obedience{/b} skill to succeed. {b}Refinement{/b} and {b}anal{/b} skill also factor in fetish acts."
    sill "{b}调教{/b}包括BDSM、打屁股、拳交以及更加刺激的性行为。女孩需要有一定的{b}调教{/b}和{b}服从{/b}属性才能有所发挥。{b}优雅{/b}和{b}肛交{/b}属性也至关重要。"

# game/BKhelp.rpy:1276
translate schinese jobs_introduction_efdea71a:

    # sill "Later, you can also train your girls to accept {b}bisexual{/b} or {b}group{/b} sex."
    sill "之后,你还可以培训你的女孩接受{b}双飞{/b}或{b}群交{/b}玩法。"

# game/BKhelp.rpy:1294
translate schinese help_brothel_1858a842:

    # sill "{b}Your Brothel{/b}. This is where you can check your brothel information, buy new rooms, and hire freelancers." nointeract
    sill "你的{b}青楼{/b}。在这里你可以查看你的青楼信息,扩建新的房间,雇佣外包人员。" nointeract

# game/BKhelp.rpy:1298
translate schinese help_brothel_54755cbb:

    # sill "The {b}Rooms{/b} you own are highlighted on the brothel tab. You can also see the number and type of bedrooms you have."
    sill "你拥有的{b}房间{/b}在青楼菜单上会高亮显示。你也可以看到你拥有的房间数量和等级。"

# game/BKhelp.rpy:1300
translate schinese help_brothel_e919f168:

    # sill "You can hover over the {b}Advertising{/b}, {b}Security{/b} and {b}Maintenance{/b} labels to learn more about your brothel's current state."
    sill "您可以将鼠标悬停在{b}广告{/b}、{b}安保{/b}和{b}清洁{/b}菜单上,了解您的青楼当前的状态。"

# game/BKhelp.rpy:1304
translate schinese help_brothel_678986c0:

    # sill "From the Brothel, you can also access the {b}Carpenter's Wagon{/b} and {b}Customer options{/b}."
    sill "在青楼信息界面还可以点击{b}工匠改造{/b}和{b}经营策略{/b}。"

# game/BKhelp.rpy:1308
translate schinese help_brothel_08baf912:

    # sill "There are two types of rooms: bedrooms and common rooms."
    sill "共有两种类型的房间:卧室和经营场所。"

# game/BKhelp.rpy:1310
translate schinese help_brothel_4a0fd479:

    # sill "Each girl must have her own {b}bedroom{/b}. They're not sharing! More comfortable bedrooms increase both girl mood and customer satisfaction."
    sill "每个女孩都必须拥有属于自己的{b}卧室{/b}。她们是不会住在一起的!更舒适的卧室会提高女孩的情绪和顾客的满意度。"

# game/BKhelp.rpy:1312
translate schinese help_brothel_22ccf984:

    # sill "You can click on the bedroom {b}picture{/b} to increase the number of bedrooms. Keep in mind that space is limited by your brothel's size, however. Click on the {b}upgrade{/b} button to improve the quality of all of your bedrooms."
    sill "你可以通过点击卧室{b}图标{/b}来增加卧室数量。但请记住,青楼的空间有限，扩建也是有限度的。点击{b}升级{/b}可以同时提升所有卧室的等级。"

# game/BKhelp.rpy:1315
translate schinese help_brothel_5ce9b38c:

    # sill "{b}Common rooms{/b} are where your girls perform their jobs when they aren't whoring. You need a specific room for each job type. A common room can host any number of girls at the same time."
    sill "{b}经营场所{/b}是你的女孩进行普通工作的地方。你需要为每种工作类型准备一个特定的场地。一个经营场所可以同时容纳任意数量的女孩。"

# game/BKhelp.rpy:1318
translate schinese help_brothel_ced859d6:

    # sill "A word of warning: The more common rooms you build, the more expensive they get! Think before you buy. Contractors are so unreliable these days..."
    sill "记住一点:你建造的经营场所越多,扩建的价格就越昂贵!在升级请前三思而行。这些承包商可真是得寸进尺..."

# game/BKhelp.rpy:1322
translate schinese help_brothel_a54ec990:

    # sill "You can hire {b}3 types of freelancers{/b} to help with your business: advertising girls, goons and cleaners."
    sill "你可以雇佣{b}3种类型的自由职业者{/b}来帮助你的生意:广告宣传人员、保镖和清洁工。"

# game/BKhelp.rpy:1324
translate schinese help_brothel_83b488c6:

    # sill "{b}Advertising girls{/b} increase your brothel reputation, which in turn will bring in more customers every night."
    sill "{b}广告宣传{/b}可以提升你的青楼声望,从而每晚吸引更多顾客。"

# game/BKhelp.rpy:1326
translate schinese help_brothel_d834ccda:

    # sill "In addition, advertising can give a temporary boost to how many customers will come ({b}customer attraction{/b}), and increase {b}customer budget{/b}, allowing you to make the most of a customer's visit. You need advertising to get people to visit your brothel, at least at the beginning!"
    sill "此外,广告还可以暂时提高每晚来消费的顾客数量({b}顾客吸引力{/b}),并增加{b}顾客预算{/b},让你能够最大限度地榨干顾客的钱包。至少在前期,你需要打广告来让更多人来你的青楼消费!"

# game/BKhelp.rpy:1331
translate schinese help_brothel_3c23b7ef:

    # sill "Would you like to know more about advertising?" nointeract
    sill "你想了解更多关于广告的信息吗?" nointeract

# game/BKhelp.rpy:1338
translate schinese help_brothel_0cfa0c52:

    # sill "{b}Goons{/b} improve your brothel security by beating the unpleasantness out of rowdy customers. Trouble is sure to show up at your door some day, make sure to have a few of them handy!"
    sill "{b}保镖{/b}通过镇住喧闹的顾客来改善你的青楼环境。麻烦总有一天会找上门来,所以准备好几个打手以防万一!"

# game/BKhelp.rpy:1342
translate schinese help_brothel_c09afa4d:

    # sill "Would you like to know more about security?" nointeract
    sill "你想了解更多关于安保的信息吗?" nointeract

# game/BKhelp.rpy:1349
translate schinese help_brothel_9d160085:

    # sill "{b}Cleaners{/b} are maids that take care of the maintenance of your brothel. Low maintenance may cause your girls to fall sick and turn away customers."
    sill "{b}清洁工{/b}是负责维护你的青楼的女佣。环境脏乱可能会导致你的女孩生病并拒绝接待顾客。"

# game/BKhelp.rpy:1351
translate schinese help_brothel_fce3bf44:

    # sill sad "Please hire some cleaners, I don't want to do all the dirty work alone!!!"
    sill sad "请雇佣一些清洁工,我可不想一个人打扫所有的卫生!"

# game/BKhelp.rpy:1353
translate schinese help_brothel_92c4b3db:

    # sill happy "Please note that freelancers get more expensive as you move your operations to fancier city districts. But they are also more efficient."
    sill happy "请注意,随着你的业务发展，搬到更高级的城市区域,自由职业者的费用会越来越高。但他们的效率也会提高。"

# game/BKhelp.rpy:1357
translate schinese help_brothel_6843404d:

    # sill "{b}Trainers{/b} can help run your brothel more efficiently! You may meet some interesting people in Zan, which will be able to help managing your girls. Only one trainer can be active at all times."
    sill "{b}训练师{/b}可以帮助你更有效地运营青楼!在泽恩,你可能会遇到一些有趣的人,他们能帮助你管理女孩们。你只能激活一位训练师的效果。"

# game/BKhelp.rpy:1360
translate schinese help_brothel_74043be9:

    # sill "There is no right or wrong trainer, it all depends on your management style."
    sill "没有最好用或最无用的训练师,根据你的经营风格选择适合你的。"

# game/BKhelp.rpy:1370
translate schinese help_advertising_53898c4f:

    # sill "{b}Advertising{/b} helps grow the renown of your brothel and draws more and wealthier {b}customers{/b} to your brothel."
    sill "{b}广告{/b}有助于提升青楼的声望,吸引更多更富裕的{b}客人{/b}到你的青楼消费。"

# game/BKhelp.rpy:1372
translate schinese help_advertising_714a1312:

    # sill "Advertising has three different effects: it boosts {b}brothel reputation{/b}, {b}customer attraction{/b} and {b}customer budgets{/b}."
    sill "广告有三种不同的效果:它能提升{b}青楼声望{/b}、{b}客户吸引力{/b}和{b}客户预算{/b}。"

# game/BKhelp.rpy:1374
translate schinese help_advertising_133d9e77:

    # sill "Each of these effects is strengthened by {b}advertising power{/b}."
    sill "这些效果都随{b}广告效果{/b}的增强而得到加强。"

# game/BKhelp.rpy:1382
translate schinese help_advertising_menu_b463b8ec:

    # sill "Some {b}customers{/b} will come to your brothel every night. At least one will show up no matter what, but your {b}brothel reputation{/b} and {b}customer attraction{/b} can increase that manyfold."
    sill "有些{b}客人{/b}每晚都会来你的青楼。至少也会有一个,但你的{b}青楼声望{/b}和{b}客户吸引力{/b}可以大大增加客人的数量。"

# game/BKhelp.rpy:1384
translate schinese help_advertising_menu_c046054f:

    # sill "There are different {b}customer populations{/b} in Zan, each with a different level of difficulty and rewards, as well as some special effect."
    sill "在泽恩有不同的{b}客人群体{/b},每个群体的服务要求和消费水平都不同,还有一些顾客有特殊效果。"

# game/BKhelp.rpy:1386
translate schinese help_advertising_menu_f889f1f4:

    # sill "You will mostly serve only {b}beggars{/b} at first, though. But as your brothel increases in rank and you reach higher districts, more customer populations will become available."
    sill "一开始你接待的客人以{b}乞丐{/b}为主,但随着你的青楼等级提高,搬到更高等级的区域,会有更多的客户群体可供选择。"

# game/BKhelp.rpy:1388
translate schinese help_advertising_menu_c9b423ad:

    # sill "When that happens, you can choose which populations to attract in the {b}customer options{/b} tab. Not only that, but you can choose to allow or restrict customer populations on a girl by girl basis if you need to."
    sill "当这种情况发生时,你可以在{b}经营策略{/b}里选择吸引哪些群体。不仅如此,如果需要,你还可以选择是否允许某些客人群体进入,这是针对每个女孩的选项调整。"

# game/BKhelp.rpy:1390
translate schinese help_advertising_menu_e9a0c292:

    # sill "Harder customers also have a higher {b}customer budget{/b}. Keep that in mind as your brothel grows to make the most money out of it."
    sill "更难满足的客人也有更高的{b}客户预算{/b}。随着青楼的发展,请记住这一点以获得最大收益。"

# game/BKhelp.rpy:1394
translate schinese help_advertising_menu_463efcf4:

    # sill "{b}Brothel reputation{/b} receives a boost from advertising every night. It is also affected negatively or positively by your {b}customers' satisfaction{/b}."
    sill "{b}青楼声望{/b}每晚都会从广告中获得提升。它也会受到你{b}客人满意度{/b}的正负面影响。"

# game/BKhelp.rpy:1396
translate schinese help_advertising_menu_39ad60d5:

    # sill "{b}Brothel reputation{/b} is nice because it draws customers to your brothel for free, unlike advertising. It does {b}decay{/b} over time, though, especially in higher districts."
    sill "{b}青楼声望{/b}很重要,因为声望越高来青楼消费的客人越多,不像广告是临时的效果。不过它会随时间而{b}衰减{/b},尤其是在更高等级的区域。"

# game/BKhelp.rpy:1398
translate schinese help_advertising_menu_c166846a:

    # sill "Of course, it will cost more brothel reputation to attract higher rank customers than the usual riff-raff. If you need more customers than your current reputation allows, blocking some higher rank populations from coming in {b}customer options{/b} could be a way to do it."
    sill "当然,吸引更高级的客人所需要的声望要比流氓乞丐更多。如果你想要客人的数量尽可能多一些,在{b}经营策略{/b}中屏蔽一些较高等级的客户群体可能会有用。"

# game/BKhelp.rpy:1400
translate schinese help_advertising_menu_fe19d71f:

    # sill "And a word of warning: {b}Unhappy customers{/b} may really drag your reputation down, especially at higher ranks. It is best to focus on drawing only as many customers as you can serve to the brothel to avoid grumbling."
    sill "关于客人,还有一件事要注意:{b}不高兴的客人{/b}可能会大幅降低你的声望,尤其是高等级的客人。最好专注于服务好和你能力相匹配的客人,以避免他们抱怨。"

# game/BKhelp.rpy:1404
translate schinese help_advertising_menu_60dc3fd0:

    # sill "{b}Customer attraction{/b} is a temporary boost your advertising girls will bring to the total number of customers that come to the brothel."
    sill "{b}客户吸引力{/b}是你的广告临时增加的客人数量。"

# game/BKhelp.rpy:1406
translate schinese help_advertising_menu_39508306:

    # sill "If you lower advertising, though, you will lose those extra customers. This is the main difference between {b}brothel reputation{/b} and {b}customer attraction{/b}."
    sill "不过,如果你减少广告投放,你就会失去这些额外的客人。这就是{b}青楼声望{/b}和{b}客户吸引力{/b}的主要区别。"

# game/BKhelp.rpy:1408
translate schinese help_advertising_menu_15a8ef4b:

    # sill "Ultimately, if you want many customers to come, it is good to keep advertising high."
    sill "总之,如果你想吸引大量客人,保持大量的广告投放是个好主意。"

# game/BKhelp.rpy:1410
translate schinese help_advertising_menu_2f8db18d:

    # sill "One day, you may even be able to unlock a way to increase {b}customer attraction{/b}, at the expense of {b}customer budgets{/b}."
    sill "有一天,你甚至可能解锁另一种提高{b}客户吸引力{/b}的方式,代价是降低{b}客户预算{/b}。"

# game/BKhelp.rpy:1413
translate schinese help_advertising_menu_3a3136fa:

    # sill "Each customer has a {b}maximum budget{/b} he's ready to spend on activities at your brothel."
    sill "每个客人都有一个{b}最高预算{/b},这是他们预计在你的青楼里消费的金额上限。"

# game/BKhelp.rpy:1415
translate schinese help_advertising_menu_cbb3a1de:

    # sill "Customers have a separate budget for {b}entertainment{/b}, such as hanging by the tavern or club, and {b}whoring{/b}. Their budget for whoring is higher, naturally. But if you want to get the most value out of a customer, entertaining him first is a good idea."
    sill "客人还有单独的{b}娱乐{/b}预算,例如在酒吧喝酒或在舞厅闲逛,还有{b}性爱{/b}预算。显然,性爱预算的占比更高。但如果你想从一个客人赚到更多的钱,先招待他是个好主意。"

# game/BKhelp.rpy:1418
translate schinese help_advertising_menu_e75811ac:

    # sill "Advertising increases {b}customer budgets{/b}. In rare cases, it is also possible to convince customers to go over their allowance. Some girl perks help with that."
    sill "广告会增加{b}客户预算{/b}。在极少数情况下,也有可能让客人透支消费。某些女孩的天赋也可以产生这种效果。"

# game/BKhelp.rpy:1420
translate schinese help_advertising_menu_9ee4e8dd:

    # sill "Of course, this is only their {b}maximum{/b} allowance. If your girls perform badly, they will pay a lot less regardless of their budget."
    sill "当然,这只是他们的{b}最高{/b}预算。如果你的女孩表现不佳,他们实际支付的金额会大大减少,不管预算有多高。"

# game/BKhelp.rpy:1422
translate schinese help_advertising_menu_6001b4b3:

    # sill "Ultimately, if you want customers to spend a lot, it is good to keep advertising high."
    sill "总之,如果你想让客人多花钱,保持大量的广告投放是个好主意。"

# game/BKhelp.rpy:1424
translate schinese help_advertising_menu_bb561c66:

    # sill "One day, you may even be able to unlock a way to increase {b}customer budgets{/b}, at the expense of {b}customer attraction{/b}."
    sill "未来可能还有其他方法提高{b}客户预算{/b},作为代价要牺牲一些{b}客户吸引力{/b}。"

# game/BKhelp.rpy:1428
translate schinese help_advertising_menu_82913fb0:

    # sill "{b}Advertising power{/b} is influenced by your advertising girls' current {b}outfits{/b}."
    sill "{b}广告效果{/b}受到你的广告女郎当前{b}穿着{/b}的影响。"

# game/BKhelp.rpy:1430
translate schinese help_advertising_menu_df2089fa:

    # sill "There may be a way to unlock new outfits in the future."
    sill "未来也许能解锁更多服装。"

# game/BKhelp.rpy:1434
translate schinese help_advertising_menu_0c9bddb4:

    # you "I'm good now, thanks."
    you "好的,我明白了,谢谢。"

# game/BKhelp.rpy:1443
translate schinese help_wagon_bdaee514:

    # carpenter "This baby here is where I ply my trade, boss."
    carpenter "这里就是我干活的地方了,老板。"

# game/BKhelp.rpy:1445
translate schinese help_wagon_3ace83c3:

    # carpenter "{b}Furniture-making{/b}, of course. I also do decorations. Heck, I can even arrange a whole room real nice if you have the resources."
    carpenter "当然是{b}制作家具{/b}了。我也会做些装饰品。事实上,如果你有材料,我还可以帮你装修整个房间。"

# game/BKhelp.rpy:1447
translate schinese help_wagon_7de54d46:

    # carpenter "I need {b}Resources{/b} to build stuff. That was our deal, remember?"
    carpenter "我需要{b}资源{/b}来制作东西。我们一开始就说好了,记得吗?"

# game/BKhelp.rpy:1449
translate schinese help_wagon_c1d74bcf:

    # carpenter "Resources can be found all over town, I hear."
    carpenter "据说城里很多地方都可以弄到资源。"

# game/BKhelp.rpy:1451
translate schinese help_wagon_92544efd:

    # carpenter "When you give me a new task, I might take a few days to complete it. An' I don't like to be interrupted. Make sure to get yer priorities straight."
    carpenter "当你派发一个新活时,我可能需要几天才能完成。而且我不喜欢被打断。请务必考虑好顺序。"

# game/BKhelp.rpy:1453
translate schinese help_wagon_a9e5dec2:

    # carpenter "After you build something, it cannot be undone. But some furniture will open new options for you, or even be switched on/off. Check the {b}Customer options{/b} tab to learn more."
    carpenter "建造完就不能撤销了。但有些家具会为你提供新的功能选项,甚至可以启用或禁用效果。请查看{b}经营策略{/b}菜单了解更多信息。"

# game/BKhelp.rpy:1459
translate schinese help_customers_fa2a1a82:

    # carpenter "Heya boss, it's me."
    carpenter "嘿呀~老板,是我。"

# game/BKhelp.rpy:1461
translate schinese help_customers_b19eac2c:

    # carpenter "You might've noticed, building new furniture has opened new {b}Customer options{/b} for ya."
    carpenter "你可能发现了,刚刚完成的家具为你提供了新的{b}经营策略{/b}。"

# game/BKhelp.rpy:1463
translate schinese help_customers_5ee181c4:

    # carpenter "It's real simple: choosing which furniture and decoration to display will attract different categories of customers."
    carpenter "这很容易理解:摆放不同的家具和装饰会吸引不同类型的顾客。"

# game/BKhelp.rpy:1465
translate schinese help_customers_78e58e25:

    # carpenter "Customers come from one of the many populations in Zan, from the rabble to the high 'n' mighty. Check out their characteristics by hovering over their portrait: some might surprise you."
    carpenter "顾客来自泽恩的各种人群,从平民到权贵。将鼠标悬停在她们头像上可以看到他们的特点,有些可能会让你大吃一惊。"

# game/BKhelp.rpy:1468
translate schinese help_customers_8cb7b3c2:

    # carpenter "They also have different budgets and whatnot. Your pink-haired girlfriend tried to explain all this to me once, it made my head hurt."
    carpenter "他们的预算也各不相同。你那个粉毛女友曾试图向我解释这些,听的我头都疼了。"

# game/BKhelp.rpy:1470
translate schinese help_customers_d4095bcf:

    # carpenter "There are more subtle ways customers differ, though. Some just like different things for entertainment, in and out of the sack."
    carpenter "不过顾客之间的差异还体现在更加细微的地方。不同的群体喜欢不同的娱乐活动,包括床上的。"

# game/BKhelp.rpy:1472
translate schinese help_customers_1aa53b4a:

    # carpenter "I've designed some furniture that can attract different types so that you can tailor it to your liking."
    carpenter "我已经设计了一些能吸引不同类型顾客的家具,供你挑选。"

# game/BKhelp.rpy:1474
translate schinese help_customers_642f217c:

    # carpenter "Just remind 'em, though: the next one who tries to pinch my ass gets his skull split with a 10-pound hammer."
    carpenter "不过要提醒他们,要是谁还敢摸我的屁股,我就拿起10磅重的锤子砸烂他的脑袋。"

# game/BKhelp.rpy:1484
translate schinese help_districts_4ac14c86:

    # sill "This is {b}Zan{/b}, the City of Jade! From here, you can visit all of the districts you have unlocked." nointeract
    sill "这里是{b}泽恩{/b},玉之城!从这里你可以前往已解锁的各个区域。" nointeract

# game/BKhelp.rpy:1488
translate schinese help_districts_ec26eb9d:

    # sill "Zan is home to {b}six districts{/b}, from the sprawling Slums outside the city walls to the heart of the city's power: the King's Hold."
    sill "泽恩共有{b}六大区域{/b},从城墙外的贫民窟到城内权力中心——国王所在的宫殿。"

# game/BKhelp.rpy:1490
translate schinese help_districts_f9b2c1b2:

    # sill "You can visit the districts that are currently unlocked, in order to meet new people or pursue some private business."
    sill "你现在可以前往已解锁的区域,认识一些新朋友或处理一些私事。"

# game/BKhelp.rpy:1492
translate schinese help_districts_e68659ac:

    # sill "In time, you will be able to unlock new districts and relocate your brothel to fancier areas."
    sill "随着游戏的进展,你将能够解锁更多的区域,并把青楼迁移到更高档的地区。"

# game/BKhelp.rpy:1496
translate schinese help_districts_12dc4af0:

    # sill "You need a proper brothel {b}license{/b} to move out of the Slums. Accessing the more upscale districts requires an even higher license."
    sill "要从贫民窟搬出,你需要取得正式的{b}营业执照{/b}。进入更高档的区域则需要更高级的执照。"

# game/BKhelp.rpy:1498
translate schinese help_districts_e9183595:

    # sill "When you advance in the game, you will be able to {b}relocate{/b} your brothel to a fancier district. Relocating your brothel will allow you to improve the quality and size of your operation, while increasing both the difficulty and rewards of the game. You may relocate only once every chapter."
    sill "随着游戏的推进,你将能够{b}迁移{/b}青楼到更高档的区域。青楼迁移后,你可以提升经营的质量和规模,但同时也会提高游戏难度和回报。每个章节你只能迁移一次。"

# game/BKhelp.rpy:1502
translate schinese help_districts_ad1d2b41:

    # sill "New licenses and brothel relocation will become available as you advance in the game."
    sill "新的执照和青楼迁移机会将随着游戏进度而开放。"

# game/BKhelp.rpy:1513
translate schinese help_visit_district_a2722007:

    # sill happy "From here, you can see the various locations available in {b}[district.name]{/b}. Click on their picture to visit them."
    sill happy "在这里你可以看到{b}[district.name]{/b}区域内的各个地点。点击图标即可前往。"

# game/BKhelp.rpy:1524
translate schinese help_visit_location_01e0386a:

    # sill "You are visiting the {b}[selected_location.name]{/b}. From here, you can meet and chat with people, or take a random tour of the area." nointeract
    sill "你正在访问{b}[selected_location.name]{/b}。在这里你可以与他人交谈,或随意游览一番。" nointeract

# game/BKhelp.rpy:1528
translate schinese help_visit_location_c777e57b:

    # sill sad "Aw, Master, you're such a playboy!!!"
    sill sad "主人,你可真是个花花公子啊!!!"

# game/BKhelp.rpy:1530
translate schinese help_visit_location_6a49d4d9:

    # sill "There are many girls idling about in Zan. You know, in this economy..."
    sill "泽恩到处都有闲逛的女孩。你知道的,经济不景气..."

# game/BKhelp.rpy:1532
translate schinese help_visit_location_75de1963:

    # sill happy "Some may be happy to talk to you. Who knows, in time you might even convince them to join your workforce?"
    sill happy "有些女孩可能愿意与你聊聊天。谁知道,随着时间推移,你甚至能说服她们加入你的团队?"

# game/BKhelp.rpy:1534
translate schinese help_visit_location_b67c19ba:

    # sill "But it will take some hard convincing. Or charms... And even then, there is no way to know in advance if the girl is any good. You might be pleasantly surprised, though."
    sill "不过这得花不少功夫。或者利用你的魅力...即便如此,你也无法提前知道她们的属性。不过说不定你会有意外的收获。"

# game/BKhelp.rpy:1539
translate schinese help_visit_location_0ee20028:

    # sill "Touring the area may allow you to learn some interesting rumors or meet new people..."
    sill "在这里闲逛可能会听到一些有趣的流言,或者认识一些新朋友..."

# game/BKhelp.rpy:1541
translate schinese help_visit_location_f311755a:

    # sill sad "But be careful, Master! The streets aren't safe, you know."
    sill sad "不过当心,主人!街道可不太安全,你知道的。"

# game/BKhelp.rpy:1548
translate schinese help_slavemarket_ecee1ea5:

    # sill happy "This is the {b}slave market{/b}!"
    sill happy "这里是{b}奴隶市场{/b}!"

# game/BKhelp.rpy:1550
translate schinese help_slavemarket_e9b64117:

    # sill sad "I am in no hurry to go back there..."
    sill sad "我可不想回到这里..."

# game/BKhelp.rpy:1552
translate schinese help_slavemarket_e130cc8e:

    # sill happy "At the slavemarket, you can {b}buy{/b} new girls to work for you. Watch out for their stats and traits before you decide, however."
    sill happy "在奴隶市场上,你可以{b}购买{/b}新的奴隶为你工作。不过在你决定之前,请先观察她们的属性和特质。"

# game/BKhelp.rpy:1556
translate schinese help_slavemarket_8d90cc80:

    # sill "Do you want to know more?" nointeract
    sill "你想了解更多吗?" nointeract

# game/BKhelp.rpy:1568
translate schinese help_slavemarket_d8d2e58e:

    # sill "Sexual experience... Well, you know... *blush*"
    sill "性经验...嗯,你知道的...*脸红*"

# game/BKhelp.rpy:1570
translate schinese help_slavemarket_a0887105:

    # sill "The slaves sold here are sex slaves. Most of them have seen some 'use' by the slavers, or their former masters. There is the odd virgin, though, but even then, she has often been 'used' in other ways..."
    sill "这里出售的奴隶都是性奴隶。她们大多数已经被奴隶贩子或前任主人'使用'过了。虽然也有少数是处女,但即便如此,她们也常常被'以其他方式使用'过..."

# game/BKhelp.rpy:1573
translate schinese help_slavemarket_a6731bb8:

    # sill "Experienced girls are more expensive. They might not have been trained the way you'd like, though."
    sill "经验丰富的女孩价格更高。不过,她们可能没有被调教成你想要的样子。"

# game/BKhelp.rpy:1575
translate schinese help_slavemarket_b6c2fe47:

    # sill "You can get a general idea about a slave's sexual training by looking at her experience level and stats."
    sill "你可以通过查看奴隶的经验等级和属性,大概了解她的训练情况。"

# game/BKhelp.rpy:1581
translate schinese help_slavemarket_7b0bc4af:

    # "You can use the shortcuts to sort market girls by name, level or sexual experience (right-click on a sort method to sort backwards)."
    "你可以使用快捷方式按名称、等级或训练等级对市场上的女孩进行排序(右击排序方式可反向排序)。"

# game/BKhelp.rpy:1588
translate schinese help_shop_035de95c:

    # sill happy "This is the {b}item shop{/b}! From here, you can buy anything you like for you or your girls."
    sill happy "这里是{b}商店{/b}!在这里,你可以买到你需要的或女孩想要的大多数东西。"

# game/BKhelp.rpy:1590
translate schinese help_shop_88299ad9:

    # sill sad "I do not like that shopkeeper, though! She cannot keep her eyes off you..."
    sill sad "不过,我不太喜欢那个老板娘!她总是用奇怪的目光盯着你看..."

# game/BKhelp.rpy:1592
translate schinese help_shop_29aeabda:

    # sill happy "There are 3 types of items: {color=[c_steel]}personal items{/color}, {color=[c_pink]}girl items{/color}, and {color=[c_orange]}gifts{/color}. Watch for the color on the item profile. Hover your mouse on each item or click it to learn more."
    sill happy "物品分为{color=[c_steel]}主角物品{/color}、{color=[c_pink]}女孩物品{/color}和{color=[c_orange]}礼物{/color}这三类。查看道具详情时会标明类型。将鼠标悬停在每件道具上或点击它都可以了解更多信息。"

# game/BKhelp.rpy:1595
translate schinese help_shop_4bdd0c04:

    # sill "{b}{color=[c_steel]}Personal items{/color}{/b} are intended for your use."
    sill "{b}{color=[c_steel]}主角物品{/color}{/b}是供你使用的。"

# game/BKhelp.rpy:1597
translate schinese help_shop_9adaadcf:

    # sill "{b}{color=[c_pink]}Girl items{/color}{/b} are intended for your girls."
    sill "{b}{color=[c_pink]}女孩物品{/color}{/b}是供你的女孩使用的。"

# game/BKhelp.rpy:1599
translate schinese help_shop_52090d2f:

    # sill "{b}{color=[c_orange]}Gifts{/color}{/b} can be given to your girls or to strangers you meet in the city. You might want to learn more about a person's taste before you give them a present, however. Not everyone likes everything!"
    sill "{b}{color=[c_orange]}礼物{/color}{/b}可以送给你的女孩或你在城里遇到的陌生人。不过,在送出去之前,最好先了解一下对方的品味。并非所有人都喜欢同样的东西!"

# game/BKhelp.rpy:1602
translate schinese help_shop_4e75be8b:

    # "You can use the shortcuts to sort or filter items by name, type, or cost (right-click on a sort method to sort backwards)."
    "你可以使用快捷方式按名称、类型或价格对道具进行排序或筛选(右击排序方式可反向排序)。"

# game/BKhelp.rpy:1613
translate schinese help_postings_9be38b31:

    # sill "The {b}posting{/b} board. This is a good place to look for classes or quests!" nointeract
    sill "{b}公告大厅{/b}。这里是寻找培训课程或委托任务的好地方!" nointeract

# game/BKhelp.rpy:1617
translate schinese help_postings_6e866826:

    # sill "{b}Classes{/b} can improve your girls' {b}skills{/b} as well as slightly raise their {b}reputation{/b} and {b}experience{/b}."
    sill "{b}培训课程{/b}可以提升你的女孩的{b}属性{/b},同时稍微提升她们的{b}声望{/b}和{b}经验{/b}。"

# game/BKhelp.rpy:1619
translate schinese help_postings_a640924b:

    # sill "Committing to a class means that your girl will be away for a few days. Several girls can enroll in the same class, up to a maximum."
    sill "参加培训课程意味着你的女孩将会离开一段时间。同一个课程可以同时培训好几个女孩。"

# game/BKhelp.rpy:1621
translate schinese help_postings_a6c2425f:

    # sill "Please be aware that classes can only improve a girl's skill up to a point. To raise her skills further, you might need more advanced classes."
    sill "请注意,培训课程只能提升女孩的属性到一定程度。如果想要进一步提升她们的技能,你可能需要更高级的课程。"

# game/BKhelp.rpy:1625
translate schinese help_postings_db095dea:

    # sill "{b}Quests{/b} are special requests posted by locals that your girl can fulfill. Questing rewards include {b}gold{/b}, {b}skills{/b}, {b}experience{/b} and {b}reputation{/b}."
    sill "{b}委托任务{/b}是当地人发布的特殊请求,你的女孩可以去完成委托。完成后可以获得{b}金币{/b}、{b}属性提升{/b}、{b}经验增长{/b}和{b}声望{/b}。"

# game/BKhelp.rpy:1627
translate schinese help_postings_633d6d8c:

    # sill sad "Some of those quests look quite dodgy! Committing to a quest means that your girl will be away for a few days."
    sill sad "有些任务看起来确实很可疑!完成任务意味着你的女孩也将离开一段时间。"

# game/BKhelp.rpy:1629
translate schinese help_postings_f234ad6c:

    # sill happy "Quests are a good way to increase a girl's reputation so you can rank her up."
    sill happy "任务是提升女孩声望,让她们升级的好方法。"

# game/BKhelp.rpy:1631
translate schinese help_postings_e90378a9:

    # sill "Watch for specific {b}traits{/b} that the customers love or hate. This might increase or decrease the reward for the quest by a large amount."
    sill "留意客户喜欢或讨厌的特定{b}特质{/b},这可能会大幅增加或减少任务的报酬。"

# game/BKhelp.rpy:1642
translate schinese help_how_to_win_387a3310:

    # sill "To win the game, you must reach the last chapter. As the story of Brothel King develops, proper endings will be added to the game."
    sill "要通关游戏,你必须完成最后一章的主线任务。随着《青楼之王》故事的发展,游戏将抵达结局。"

# game/BKhelp.rpy:1644
translate schinese help_how_to_win_eb06c747:

    # sill "For each chapter, you may have to reach certain goals or complete elements of the story to advance."
    sill "每个章节,你都需要达成某些目标或完成故事中的某些任务才能推进。"

# game/BKhelp.rpy:1648
translate schinese help_how_to_win_adcf0a29:

    # sill "Please hover on the bottom right message in the home screen to know your current goals."
    sill "请将鼠标悬停在主界面右下角的菜单上,了解您当前的目标。"

# game/BKhelp.rpy:1659
translate schinese help_about_game_54f1f4da:

    # sill "What would you like to know about this game?" nointeract
    sill "还想了解其他关于游戏的信息吗?" nointeract

# game/BKhelp.rpy:1700
translate schinese help_about_game_72bc183d:

    # "Thanks to all the people on the [URL] forum for their support and all the good ideas, girl packs and mods they contributed."
    "[URL]论坛上的所有人,感谢你们的支持和所有优秀的创意、女孩包和模组贡献。"

# game/BKhelp.rpy:1702
translate schinese help_about_game_142a2cc1:

    # "Special thanks to OhWee for making some great screens (including the load/save screen and the input screen), and Deimos96 for making the cool Evil Power cards' UI."
    "特别感谢OhWee制作了一些出色的截图(包括读取/保存界面和输入界面),以及Deimos96为邪恶力量卡UI制作的酷炫设计。"

# game/BKhelp.rpy:1704
translate schinese help_about_game_2f85ff93:

    # "Special thanks to kite80, Dragonblood, Chris12, OhWee, Xela, and all the testers, for helping me with develop the story, improve gameplay, and learn some coding along the way."
    "特别感谢kite80、Dragonblood、Chris12、OhWee、Xela以及所有测试人员,感谢你们在游戏剧情、玩法改善和编程方面给予的帮助。"

# game/BKhelp.rpy:1707
translate schinese help_about_game_fd2461a7:

    # "Contact me at [URL] for feedback, criticism, bug reports, etc."
    "如有反馈、批评、错误报告等,请联系我 [URLcn]。"

# game/BKhelp.rpy:1720
translate schinese help_night_events_3d6e67e7:

    # sill "{b}Night events{/b} are where the action is!" nointeract
    sill "{b}夜间事件{/b}才是游戏的核心玩法!" nointeract

# game/BKhelp.rpy:1724
translate schinese help_night_events_c5b22008:

    # sill "Every night, customers will flock to your brothel in the hope of getting some action, both in the form of entertainment and sex. How many customers actually show up depends on your {b}brothel reputation{/b}."
    sill "每晚,顾客都会涌向你的青楼,希望获得娱乐和性方面的满足。实际的顾客数量取决于你的{b}青楼声望{/b}。"

# game/BKhelp.rpy:1727
translate schinese help_night_events_afc703b9:

    # sill "Each customer has his own preferences in terms of entertainment and sexual practices. Meeting your customers expectations is key to their satisfaction."
    sill "每位顾客都有对娱乐和性行为方式的偏好。满足顾客的期望是提升满意度的关键。"

# game/BKhelp.rpy:1730
translate schinese help_night_events_4914165e:

    # sill "Eventually, your brothel's reputation will rise or fall depending on how many customers have been entertained and sexually gratified."
    sill "最终,你青楼的声望将根据已接待的和对性爱满足的顾客数量而变动。"

# game/BKhelp.rpy:1732
translate schinese help_night_events_a112b296:

    # sill "If no girls are working at the brothel on a given night, the brothel will close. Security and advertising babes will go home. You will still have to pay for upkeep and maintenance."
    sill "如果没有女孩能够工作,青楼将暂停营业。保安和广告女郎都不会工作。但你仍需支付保养费用。"

# game/BKhelp.rpy:1737
translate schinese help_night_events_ce34d1be:

    # sill "{b}Advertising{/b} is pretty straightforward. Get a bunch of young, hot girls out there with signs, flyers or bodily tattoos with the brothel's name on it, and its reputation will increase. Reputation begets more customers."
    sill "{b}广告{/b}的效果非常直接。让一群年轻火辣的女孩在外面挥舞标语、发传单或纹身宣传青楼的名字,这将提升青楼声望。高声望会带来更多的顾客。"

# game/BKhelp.rpy:1740
translate schinese help_night_events_bb01cc54:

    # sill "If your {b}brothel security{/b} is insufficient, your brothel may suffer from attacks during the night. Attacks can deplete your money, hurt your girls and damage their mood, so watch out!"
    sill "如果你的{b}青楼安保{/b}不足,青楼可能遭受夜间袭击。袭击会劫掠你的金币,伤害女孩并降低她们的情绪,所以要小心! "

# game/BKhelp.rpy:1744
translate schinese help_night_events_c09afa4d:

    # sill "Would you like to know more about security?" nointeract
    sill "你想了解更多关于安保的信息吗?" nointeract

# game/BKhelp.rpy:1751
translate schinese help_night_events_a9c6adda:

    # sill "Customers are messy and will dirty up the place every night as they go about their business. {b}Maintenance{/b} is essential is you don't want your brothel to become a hotbed for disease. By the way, sex is dirtier than entertainment."
    sill "顾客很邋遢,每晚离开后都会弄脏整个场地。{b}清洁{/b}是必需的,否则青楼会沦为疾病的温床。顺带一提,性行为比一般娱乐要更加脏乱。"

# game/BKhelp.rpy:1754
translate schinese help_night_events_a112b296_1:

    # sill "If no girls are working at the brothel on a given night, the brothel will close. Security and advertising babes will go home. You will still have to pay for upkeep and maintenance."
    sill "如果没有女孩能够工作,青楼将暂停营业。保安和广告女郎都不会工作。但你仍需支付保养费用。"

# game/BKhelp.rpy:1760
translate schinese help_night_events_ab834815:

    # sill "Every night, the available working girls will {b}perform{/b} for the visiting customers."
    sill "每天晚上,工作的女孩会{b}服务{/b}前来消费的顾客。"

# game/BKhelp.rpy:1762
translate schinese help_night_events_37e4edbe:

    # sill "Some girls might rebel and go on strike. This happens when their mood and/or obedience is too low."
    sill "有些女孩可能会罢工。如果她们的情绪和/或服从度太低。"

# game/BKhelp.rpy:1764
translate schinese help_night_events_0ec7626f:

    # sill "The girls that do work will service the customers by doing their job or performing sex acts. Each customer will choose the form of entertainment and sex act that matches his tastes, if a girl is available."
    sill "女孩会为顾客提供服务,或是性行为。每位顾客都会选择符合自己品位的娱乐和性行为,前提是有女孩可供选择。"

# game/BKhelp.rpy:1767
translate schinese help_night_events_f8681acd:

    # sill "Performance depends on a number of factors: customer difficulty (which is higher in fancier districts), girl skills and job level, bedroom quality, special items, and so on."
    sill "服务质量取决于诸多因素:顾客的要求(顾客等级越高要求更高)、女孩的属性和职业等级、卧室等级、特殊道具等。"

# game/BKhelp.rpy:1770
translate schinese help_night_events_aa569c0f:

    # sill "A good performance will massively increase the rewards from the customers, especially the tipping."
    sill "出色的表现将大幅提高从顾客那里获得的收入,尤其是小费。"

# game/BKhelp.rpy:1780
translate schinese security_introduction_7bf4aa11:

    # sill sad "Master! Something happened last night!"
    sill sad "主人!昨晚出事了!"

# game/BKhelp.rpy:1794
translate schinese help_security_dcd77966:

    # sill happy "Security events happen when the {b}threat{/b} to your brothel is high, or when {b}crazy customers{/b} visit your brothel."
    sill happy "当你的{b}威胁等级{/b}较高,或{b}疯狂的顾客{/b}来访时,就会发生安全事故。"

# game/BKhelp.rpy:1796
translate schinese help_security_ba7a2419:

    # sill "Brothel {b}Threat{/b} is based on your gold and the number of working girls in your brothel. Higher ranking girls attract more threat."
    sill "青楼的{b}威胁等级{/b}取决于你的金钱总量和在职的女孩数量。等级更高的女孩会引来更多威胁。"

# game/BKhelp.rpy:1798
translate schinese help_security_acb38ad7:

    # sill "Security is based on your {b}available goons{/b}."
    sill "安保等级取决于你的{b}保安数量{/b}。"

# game/BKhelp.rpy:1800
translate schinese help_security_987e3a23:

    # sill "Your {b}Strength{/b} counts too, but only if you have {b}remaining AP at the end of the day{/b}, or the 'Defender' talent."
    sill "你的{b}力量{/b}属性也很重要,但前提是你当天还有足够的{b}剩余行动点数{/b},或者拥有'守护者'天赋。"

# game/BKhelp.rpy:1802
translate schinese help_security_b7f273ff:

    # sill "Threat builds up more or less quickly depending on your level of security. Over time, {b}security events{/b} will happen, although it will happen much less often if you have adequate security. A good level of security will help mitigate the results of security events as well."
    sill "威胁等级的增长速度取决于你的安保水平。随着时间推移,会发生{b}安全事故{/b},不过如果安保力量足够,发生的频率会大大降低。良好的安保水平也有助于降低安全事故的损失。"

# game/BKhelp.rpy:1804
translate schinese help_security_e992cd18:

    # sill "Be careful! Security events may {b}escalate{/b} over time, until something really bad happens."
    sill "小心!安全事故可能会{b}升级{/b},直到发生非常糟糕的事。"

# game/BKhelp.rpy:1806
translate schinese help_security_413c41d6:

    # sill "Another risk to your brothel is {b}crazy customers{/b}. Crazy customers may attack your girls or brothel. Because they are customers, it's hard to prevent them from coming in, but your security level may help avoid negative consequences. Arming your girls is also a good option."
    sill "另一个威胁是{b}疯狂的顾客{/b}。他们可能袭击你的女孩或青楼。由于他们是顾客,你很难阻止他们进入,但你的安保水平可能会减少损失。为女孩配备防身的武器也是个不错的选择。"

# game/BKhelp.rpy:1808
translate schinese help_security_6304b56b:

    # sill "Some customer populations, like thugs, are well-known for causing trouble. So watch out!"
    sill "一些顾客群体,比如暴徒,以惹事闻名,所以要当心! "

# game/BKhelp.rpy:1814
translate schinese help_rank_introduction_3372946f:

    # sill happy "Rank is a very important factor for a sex slave!"
    sill happy "等级对性奴来说非常重要!"

# game/BKhelp.rpy:1816
translate schinese help_rank_introduction_388fa8cc:

    # sill "In Zan, regular citizens are only allowed to own common sex slaves. Owning sex slaves of a higher rank requires a suitable brothel license. As you unlock better licenses, you will be able to upgrade your girls' rank further."
    sill "在泽恩,普通公民只允许拥有普通等级的性奴。拥有更高等级的性奴需要对应的青楼营业执照。随着你解锁更高级的营业执照,你才能进一步提升女孩的等级。"

# game/BKhelp.rpy:1819
translate schinese help_rank_introduction_6896bafd:

    # sill "Common slaves are known as rank 'C'. Better sex slaves can reach rank 'B', 'A', then 'S' for the most special sex slaves. Some say there is a even higher rank, accessible only to the most legendary sex slaves... But if it exists, it is a well-kept secret."
    sill "普通奴隶被称为'C'阶。更优秀的性奴可达到'B'、'A'阶,最高为'S'阶。有人说还有更高的等级,只有最传奇的性奴才能拥有...但如果真的存在,那也是不传之秘。"

# game/BKhelp.rpy:1822
translate schinese help_rank_introduction_30a168db:

    # sill "Reaching a new rank is difficult. First, a girl must have reached the {b}maximum level{/b} for her rank (for rank 'C', she must be level 5). She must also have a high enough {b}reputation{/b} with her customers."
    sill "晋升阶级十分困难。首先,女孩必须达到自身的{b}等级上限{/b}(例如'C'阶,必须达到5级)。她还需要在顾客中拥有足够高的{b}声望{/b}。"

# game/BKhelp.rpy:1825
translate schinese help_rank_introduction_8f8ee891:

    # sill "Girls can earn reputation when performing well with customers, and when accomplishing special quests."
    sill "女孩可以通过服务中出色的表现以及完成特殊任务来获得声望。"

# game/BKhelp.rpy:1827
translate schinese help_rank_introduction_48fe6358:

    # sill "A higher rank allows you to train a girl's stats and job skills further, increase her level cap, and affects many other aspects of the game."
    sill "阶级提升后,你将可以进一步提升她们的属性和职业等级,提高她们的等级上限,并影响游戏的许多其他方面。"

# game/BKhelp.rpy:1829
translate schinese help_rank_introduction_9c2c4486:

    # sill "But ranking up costs money, and higher rank girls are also more expensive to maintain. They are also more fickle: all this glory is getting to their head."
    sill "但是提升阶级需要花钱,阶级高的女孩保养费也更贵。她们也更加善变:荣耀已经冲昏了她们的头脑。"

# game/BKhelp.rpy:1833
translate schinese help_rank_introduction_a7b662e2:

    # sill "With your current license, you can train girls up to rank [rk]."
    sill "根据您目前的营业执照,您可以将女孩训练至[rk]阶。"

# game/BKhelp.rpy:1841
translate schinese help_farm_question_4a71d5b9:

    # gizel "Do you need some help with the farm?" nointeract
    gizel "你想知道如何使用奴隶农场吗?" nointeract

# game/BKhelp.rpy:1851
translate schinese help_farm_5c3737c1:

    # gizel normal "The 'farm' is the unoriginal name you stupid humans give this place."
    gizel normal "所谓的'农场'不过是你们这些愚蠢的人类给这个地方起的愚蠢名字。"

# game/BKhelp.rpy:1853
translate schinese help_farm_9b6a2942:

    # gizel "Of course, it is much more than that. It is a shining place of power acting like a beacon through space and time, sitting on a whirlpool of dark magical energy..."
    gizel "当然,这里不仅仅是个农场。这是一个光辉灿烂的力量之地,如同一座位于时空漩涡之上的灯塔,充满着黑暗魔法的能量...."

# game/BKhelp.rpy:1855
translate schinese help_farm_d99951a5:

    # you "Yeah yeah. To the point, Giz'."
    you "是是是。让我们直奔主题吧,吉泽'。"

# game/BKhelp.rpy:1857
translate schinese help_farm_99f22c61:

    # gizel upset "Hmpf! You're just a foolish barbarian. Anyway. I'll just stick to concepts you can easily understand."
    gizel upset "哼!真是个愚蠢的野蛮人。总之,我会用你们容易理解的概念来解释。"

# game/BKhelp.rpy:1859
translate schinese help_farm_a03c3eaf:

    # you "*sigh* You do that..."
    you "*叹息*就按你说的来吧...."

# game/BKhelp.rpy:1866
translate schinese help_farm_menu_21bd3cff:

    # gizel "What do you want to know, then?" nointeract
    gizel "那么你想了解什么?" nointeract

# game/BKhelp.rpy:1870
translate schinese help_farm_menu_5da4d4c8:

    # gizel "You can send girls here at the {b}farm{/b}, where they will remain in my care for a fixed duration, or until you want them back."
    gizel "你可以将女孩们送到{b}奴隶农场{/b},她们将由我管教,除非你想让她们提前回去。"

# game/BKhelp.rpy:1872
translate schinese help_farm_menu_5d1c7953:

    # gizel "The girls will be held in {b}pens{/b} here with my animals and minions for company. While they're here, they won't be using their room in the brothel, so you are free to assign it to someone else."
    gizel "女孩们将被关在这里的{b}牢房{/b}中,和我的宠物和仆从为伴。当她们住在这里的时候,青楼里的房间就空出来了,所以你可以把房间分配给别人。"

# game/BKhelp.rpy:1874
translate schinese help_farm_menu_de6f9d87:

    # gizel "For training, I will use the farm {b}facilities{/b} and my sweet {b}minions{/b}. Each facility is suitable for only one type of minions."
    gizel "为了训练她们,我将利用农场的{b}设施{/b}和我亲爱的{b}仆从{/b}。每种仆从都有其对应的设施。"

# game/BKhelp.rpy:1876
translate schinese help_farm_menu_8076bef7:

    # "You can use the shortcuts to sort your girls by name, level or rank (right-click on a sort method to sort backwards)."
    "你可以使用快捷方式按照名称、等级或等级对女孩进行排序(右键单击排序方法可以反向排序)。"

# game/BKhelp.rpy:1880
translate schinese help_farm_menu_4bd7aa9f:

    # gizel normal "Certainly. {b}Pens{/b} are where your girls are being kept while they are at the farm. {b}Facilities{/b} are where I host my beloved minions."
    gizel normal "当然。{b}牢房{/b}就是你的女孩在训练期间居住的地方。{b}设施{/b}则是我尊贵的仆从们的居所。"

# game/BKhelp.rpy:1882
translate schinese help_farm_menu_8892b641:

    # gizel "Because the farm was in such a state of disrepair when we got it, not all pens and facilities will be immediately available, however."
    gizel "我们接手农场的时候这里一片狼藉,并不是所有的牢房和设施都能立即投入使用。"

# game/BKhelp.rpy:1884
translate schinese help_farm_menu_76939fcc:

    # gizel "If you have the coin, you can build new {b}pens{/b} to hold more girls at the farm. The farm's {b}facilities{/b} can be also rebuilt and improved on, which will increase the number of minions they can hold."
    gizel "如果你有钱,你可以建造新的{b}牢房{/b}来容纳更多的女孩。农场的{b}设施{/b}也可以进行重建和升级,设施等级越高能容纳的仆从也就越多。"

# game/BKhelp.rpy:1886
translate schinese help_farm_menu_ee85818f:

    # gizel "Our activities here are a bit unusual, I guess, and might attract attention from the wrong crowd... Like those Arios-worshipping apes. So let's keep our little operation quiet."
    gizel "我们在这里做的事非同寻常,我想,可能会引起其他人的注意....比如那些崇拜太阳神的猴子们。所以还是低调行事比较好。"

# game/BKhelp.rpy:1890
translate schinese help_farm_menu_cedb2a82:

    # gizel "Right now, we can build a maximum of [p] pens and improve our facilities up to level [district.rank], before the neighbors get suspicious."
    gizel "现在,我们最多可以建造[p]个牢房,设施最高只能升到[district.rank]级,再扩张就要引起别人的怀疑了。"

# game/BKhelp.rpy:1893
translate schinese help_farm_menu_24fdfbf8:

    # extend " This is the farm's maximum capacity, so we cannot improve our facilities beyond that."
    extend "这已经达到农场的上限了,我们没办法继续升级设施了。"

# game/BKhelp.rpy:1895
translate schinese help_farm_menu_6c52d52b:

    # extend " We can develop the farm further if you obtain a higher brothel license and some protection in high places."
    extend "如果你获得了更高级的营业执照,并得到高层的庇护,我们就可以进一步开发农场。"

# game/BKhelp.rpy:1899
translate schinese help_farm_menu_8dfec8f5:

    # gizel smirk "Ah, the minions! My little babies, my loves..."
    gizel smirk "啊,仆从们!我的小宝贝们,我的心头肉...."

# game/BKhelp.rpy:1901
translate schinese help_farm_menu_6413bebc:

    # gizel normal "There are various kinds of minions, using various facilities."
    gizel normal "这里有各种各样的仆从,他们住在不同的设施。"

# game/BKhelp.rpy:1903
translate schinese help_farm_menu_3413667e:

    # gizel "Minions are useful for training. It takes at least one minion to train a girl, so make sure you have enough minions to do the work."
    gizel "仆从们对训练很有用。一个女孩至少需要一个仆从辅助训练,所以请确保你有足够的仆从来进行训练。"

# game/BKhelp.rpy:1905
translate schinese help_farm_menu_548c97d0:

    # gizel "You can find new minions in town, I guess, but I'm not sure where. I don't go out much."
    gizel "我猜城里可以弄到新的仆从,但我也不确定具体在哪里。我不怎么出门。"

# game/BKhelp.rpy:1907
translate schinese help_farm_menu_bf18b887:

    # gizel "The {b}Stallions{/b}, such as my beloved Bob, are held in the {b}Stables{/b}, in the barn."
    gizel "{b}种马{/b},比如我可爱的鲍勃,被关在马厩里的{b}马棚{/b}。"

# game/BKhelp.rpy:1909
translate schinese help_farm_menu_887e01c7:

    # gizel "Stallions are magically brainwashed sex-slaves from the Blood Islands, bred selectively for their unnaturally large dicks..."
    gizel "种马是从鲜血群岛带来的, 经过选择性培育的，被魔法洗脑的性奴隶,拥有后天改造的巨大阴茎...."

# game/BKhelp.rpy:1911
translate schinese help_farm_menu_0980844a:

    # you "Freaky..."
    you "太可怕了...."

# game/BKhelp.rpy:1914
translate schinese help_farm_menu_3d1d6e8c:

    # gizel "The {b}Beasts{/b} stay in the {b}Pig stall{/b}."
    gizel "{b}野兽{/b}则待在{b}猪圈{/b}里。"

# game/BKhelp.rpy:1916
translate schinese help_farm_menu_1ac0e76c:

    # gizel "Beasts are simply animals, but I have fun messing around with their natural instincts with magic. They're not just pigs too: their structure is so basic I can morph them into different animals when the need arises."
    gizel "野兽只是普通的动物,但我会用魔法玩弄它们的天性。它们并非只是猪,它们的生理构造十分简单,我可以在需要的时候把它们变成各种各样的动物。"

# game/BKhelp.rpy:1919
translate schinese help_farm_menu_3ca9315d:

    # you "Cool..."
    you "太酷啦..."

# game/BKhelp.rpy:1922
translate schinese help_farm_menu_c07ab10a:

    # gizel "There's an old pig stall in the courtyard... I guess we could use it to hold some {b}animals{/b}."
    gizel "院子里还有一个旧猪圈...我想我们可以用它来饲养一些{b}动物{/b}。"

# game/BKhelp.rpy:1925
translate schinese help_farm_menu_dd27cb3d:

    # gizel "The {b}Monsters{/b} dwell underground in the {b}Monster den{/b}, where else?"
    gizel "{b}怪物{/b}住在地下的{b}怪物巢穴{/b},不然你以为它们还能住在哪?"

# game/BKhelp.rpy:1927
translate schinese help_farm_menu_0e44a238:

    # gizel "Monsters come in many shapes and forms, but they are all hungry for juicy nipples and wet holes... I wouldn't have them any other way!"
    gizel "怪物有各种形态,但它们都渴望美味的乳头和湿润的穴道...我希望它们一直保持这样!"

# game/BKhelp.rpy:1929
translate schinese help_farm_menu_a4916498:

    # you "Spooky..."
    you "很吓人..."

# game/BKhelp.rpy:1932
translate schinese help_farm_menu_20a19d35:

    # gizel "I spotted the entrance to an old cellar in the back, which seems linked to an ancient and intricate cave network... A perfect place to breed {b}monsters{/b}, I should say."
    gizel "我在后面发现了一个入口，里面有个地形复杂的地窖... 这简直就是培养{b}怪物{/b}的最佳场所,不得不说。"

# game/BKhelp.rpy:1935
translate schinese help_farm_menu_3697bc00:

    # gizel "Finally, there are the {b}Machines{/b}, which are kept in the {b}Workshop{/b} at the back."
    gizel "最后,还有那些安置在{b}工坊{/b}里的{b}机器{/b}。"

# game/BKhelp.rpy:1937
translate schinese help_farm_menu_26f0bedf:

    # gizel "Those machines aren't the crude tools your backward species use either... They are artefacts of great magical power, designed with one thing in mind: pleasure. Or pain. What was the difference you people make between both, again? This always confused me!"
    gizel "这些机器可不是你们这些落后物种使用的简陋工具...它们是拥有强大魔力的神器,它们被设计出来就是为了提供快乐或痛苦。你们人类到底如何区分这两者,我一直很疑惑!"

# game/BKhelp.rpy:1940
translate schinese help_farm_menu_d4655048:

    # you "Nerdy..."
    you "真是个书呆子..."

# game/BKhelp.rpy:1942
translate schinese help_farm_menu_7c84a6b7:

    # gizel "Finally, there's the old toolshed in the farmhouse, I could repurpose it to hold various artefacts and {b}machines{/b}... I have plenty of ideas..."
    gizel "最后,还有农舍里的老旧工具间,我可以把它改造成存放各种设备和{b}机器{/b}的储物间... 我还有很多计划..."

# game/BKhelp.rpy:1944
translate schinese help_farm_menu_0281c96a:

    # gizel smirk "My minions will take good care of your slaves! *smirk*"
    gizel smirk "我的仆从会好好照顾你的奴隶的! *得意地笑*"

# game/BKhelp.rpy:1946
translate schinese help_farm_menu_7b73b7a4:

    # gizel normal "As they play around with your girls, minions will earn {b}experience{/b}. Eventually, they might become more powerful. And more power means one thing of course: BIGGER DICKS!!!"
    gizel normal "当它们与你的女孩玩耍时,仆从们也会获得{b}经验{/b}。渐渐地,他们会越来越强大。而强大意味着:更粗更硬的生殖器!!!"

# game/BKhelp.rpy:1950
translate schinese help_farm_menu_28a79ba4:

    # gizel normal "But watch out for the minions and their health. If you take good care of them, and deliver them a steady flow of dumb sluts to play with, they will serve you better over time."
    gizel normal "但要注意仆从们的健康。如果你好好照顾它们,给它们源源不断的女孩玩弄,它们会更加服从于你。"

# game/BKhelp.rpy:1953
translate schinese help_farm_menu_22d27be2:

    # gizel "Finally, you should note that for {b}group sex training{/b}, I'll need more than one minion of the same type. But that's obvious, right?"
    gizel "最后,你应该了解一下{b}群体性训练{/b},我需要不同类型的仆从帮助训练。这显而易见,不是吗?"

# game/BKhelp.rpy:1957
translate schinese help_farm_menu_3a1896f9:

    # gizel normal "Right, let's get to the main reason we're here. {b}Training{/b}."
    gizel normal "好了,让我们回到正题。{b}训练{/b}。"

# game/BKhelp.rpy:1959
translate schinese help_farm_menu_9abd1e8c:

    # gizel "Basically, there are two kinds of things I can do with your girls while they're at the farm."
    gizel "一般来说,我可以安排你的女孩做两种事,第一种是让她们在农场里接受训练。"

# game/BKhelp.rpy:1961
translate schinese help_farm_menu_9e185176:

    # gizel "First, I can simply keep them locked in a pen. This is what I call {b}holding{/b}. Nothing will happen to them then, however, so that's not FUN."
    gizel "第一种,我可以把她们单纯地关在牢房里。也就是所谓的{b}囚禁{/b}。在这种情况下,什么都不会发生,所以也很无聊。"

# game/BKhelp.rpy:1963
translate schinese help_farm_menu_e29ab913:

    # gizel "Fortunately, I can spice things up by setting {b}holding rules{/b}."
    gizel "幸运的是,我可以通过设定{b}囚禁规则{/b}来让事情变得更有意思。"

# game/BKhelp.rpy:1967
translate schinese help_farm_menu_ae465693:

    # gizel normal "Next, you can let my minions play with your girls to {b}train{/b} them."
    gizel normal "第二种,我可以让仆从与你的女孩一起{b}训练{/b}。"

# game/BKhelp.rpy:1969
translate schinese help_farm_menu_14d39697:

    # gizel "I can train your girls for all kinds of sex acts with my minions here. Depending on the {b}rules{/b} you choose for their training, I can go soft or hard on the little bitches... But who doesn't like it hard?"
    gizel "我可以训练你的女孩们去练习各种性行为。根据你选择的{b}训练模式{/b},我可以温柔或强硬地对待这些小婊子...但谁不喜欢粗暴一些呢?"

# game/BKhelp.rpy:1974
translate schinese help_farm_menu_ecd045bd:

    # gizel "Because we work and play hard here at the farm, your girls will occasionally {b}get tired{/b}, being wimps and all."
    gizel "因为要在农场里工作和努力训练,你的女孩们偶尔会{b}感到疲惫{/b},毕竟她们都是一些软弱的家伙。"

# game/BKhelp.rpy:2000
translate schinese help_farm_rules_training_b1341e48:

    # gizel normal "{b}Training rules{/b} are very important. Most importantly, they affect the amount of FUN {b}I{/b} get while training your dumb sluts."
    gizel normal "{b}训练模式{/b}非常重要。最重要的是,它们会影响到{b}我{/b}在训练你这群愚蠢婊子时获得的乐趣。"

# game/BKhelp.rpy:2002
translate schinese help_farm_rules_training_e1666289:

    # gizel "The most boring and useless option is to go '{b}soft{/b}' on the bitches. When going soft, I won't make them do anything they don't already agree to."
    gizel "{b}温和{/b}对待这些婊子是最无聊和无用的选项。采用温和模式时,我不会强迫她们做任何她们不愿做的事情。"

# game/BKhelp.rpy:2004
translate schinese help_farm_rules_training_a954f985:

    # gizel upset "Can't you see how lame it is! LAME! Never choose this option, ever! *mad*"
    gizel upset "难道你看不出这有多无聊吗!无聊透顶!永远不要选择这个选项! *生气*"

# game/BKhelp.rpy:2006
translate schinese help_farm_rules_training_1e1333da:

    # gizel normal "The second option is to go '{b}tough{/b}' on them. When being trained hard, I'll ignore most of the girls' complaints and threaten the dumb slaves into submission. I'll push them hard, but I won't go all out on them just yet, introducing them to harder sex acts gradually."
    gizel normal "我也可以对她们{b}强硬{/b}一些。在接受强硬训练时,我会无视女孩们大部分的抱怨,并威胁这些愚蠢的奴隶屈服。我会狠狠地刺激她们,但不会把她们逼到绝路,循序渐进地引入更加极端的性行为。"

# game/BKhelp.rpy:2009
translate schinese help_farm_rules_training_8b201e22:

    # gizel "Finally, you can leave it all to me... This is what I call the '{b}hardcore{/b}', or most FUN, option."
    gizel "如果这都不行,你可以把她完全交给我来负责...就是所谓的'{b}硬核{/b}模式'，最有趣的模式。"

# game/BKhelp.rpy:2011
translate schinese help_farm_rules_training_6e31ce98:

    # gizel smirk "In hardcore mode, I will ignore any and all complaints your girls have, even if they try to fight it, and inflict on them whatever hardcore sex act I wish on a whim. They will be harshly punished if they disobey."
    gizel smirk "在硬核模式下,我将无视你的女孩们的所有抱怨,即使她们试图反抗,我也会随心所欲地让她们练习各种极端的性行为。如果她们不听话,我会严惩不贷。"

# game/BKhelp.rpy:2013
translate schinese help_farm_rules_training_e688a828:

    # gizel "Now isn't that FUN? Muhahahaha!"
    gizel "这不是很有趣吗?哈哈哈哈!"

# game/BKhelp.rpy:2015
translate schinese help_farm_rules_training_3e7fc5d7:

    # gizel upset "But let's be careful, because some hard-headed sluts might try to fight back... Make sure you disarm the bitches before sending them here, or we'll be in trouble."
    gizel upset "不过我们还是要小心一点,有些顽固的婊子可能会试图反击...确保在把她们送来之前解除她们的武装,否则我们会陷入麻烦。"

# game/BKhelp.rpy:2021
translate schinese help_farm_rules_resting_0ca98645:

    # gizel normal "I've never understood this concept of {b}rest{/b} myself, being blessed with endless libido and stamina."
    gizel normal "我不需要了解{b}休息{/b}这个概念,因为我拥有无休止的性欲和耐力。"

# game/BKhelp.rpy:2023
translate schinese help_farm_rules_resting_b54917fd:

    # gizel "But petty lifeforms like yours seem to need it, otherwise they just become exhausted or even injured. Lame."
    gizel "但是像你这样孱弱的生命体似乎需要休息,否则就会筋疲力尽甚至受伤。真是弱小。"

# game/BKhelp.rpy:2025
translate schinese help_farm_rules_resting_d7764556:

    # gizel "So if you want, I can keep an eye on your girls' level of {b}energy{/b}, and remove them from working or training if they become too weak."
    gizel "如果你需要的话,我可以时刻监视女孩们的{b}精力{/b}水平,如果她们变得太虚弱的话我将她们从工作或训练中撤下。"

# game/BKhelp.rpy:2027
translate schinese help_farm_rules_resting_9e02141f:

    # gizel "I'll set them to rest automatically before they become exhausted. Of course, I could always misjudge and injure a slave or two if I'm not paying attention, but who CARES!"
    gizel "我会在她们筋疲力尽之前让她们去休息。当然,如果我粗心大意的话,可能会误伤一两个奴隶,但是谁在乎呢!"

# game/BKhelp.rpy:2045
translate schinese help_farm_rules_holding_10d01bcf:

    # gizel normal "While your girls are not in training, you can choose what you want them to do during their stay ({b}holding{/b})."
    gizel normal "当女孩们不训练时,你可以让她们在({b}囚禁{/b})期间做些什么。"

# game/BKhelp.rpy:2047
translate schinese help_farm_rules_holding_f68194b4:

    # gizel "Because an idle slave is prone to become a fat and lazy cow, I recommend keeping them busy at all times!"
    gizel "一个闲置的奴隶容易变成一只又肥又懒的奶牛,所以我建议让她们保持运动!"

# game/BKhelp.rpy:2049
translate schinese help_farm_rules_holding_34be0daf:

    # gizel "While you keep them busy, their {b}skills{/b} will improve... But they will get tired, and whine. They always do."
    gizel "在她们保持忙碌的同时,她们的{b}属性{/b}也会提高...但是她们会变得疲惫,并且会发牢骚。她们总是这样。"

# game/BKhelp.rpy:2051
translate schinese help_farm_rules_holding_af6e58e4:

    # gizel "Alternatively, you can set them to {b}rest{/b}, and the spoiled sluts will do nothing but recover their {b}energy{/b}."
    gizel "或者,你也可以让她们{b}休息{/b},那些被宠坏的婊子们就什么也不做,好好恢复她们的{b}精力{/b}。"

# game/BKhelp.rpy:2053
translate schinese help_farm_rules_holding_e7b5dc08:

    # gizel upset "Why not give them a nice massage, and a pouch of gold too, why don't you? *hiss*"
    gizel upset "你干脆再给她们做一次全身按摩,再发一袋金币得了? *嘶*"

# game/BKhelp.rpy:2060
translate schinese help_zodiac_da122e3e:

    # sill happy "I read a fascinating story... Astrology is very popular in Zan, and {b}Zodiac saints{/b} play a foremost role at every level of society."
    sill happy "我看了一个有趣的故事...泽恩有很多传奇人物被人们称为圣人,{b}圣人{/b}在各行各业都引领着人们前进，包括奴隶。"

# game/BKhelp.rpy:2062
translate schinese help_zodiac_f945b6dd:

    # sill "Different classes of people have different belief systems. They have hundreds of popular spirits and other minor deities to choose from..."
    sill "每个行业都有其领军人物。三百六十行，行行出状元，服务业里也出现过闻名天下的人..."

# game/BKhelp.rpy:2064
translate schinese help_zodiac_31388708:

    # you "Oh, really? Does that sit well with the major religions?"
    you "哦,真的吗?但宗教允许人们崇拜这些传奇人物吗?"

# game/BKhelp.rpy:2066
translate schinese help_zodiac_f10dd37b:

    # sill "Well, the established religions don't really bother with mere superstitions such as the Zodiac saints... They find a way to include the saints in their own particular pantheon, in one way or another."
    sill "嗯,宗教其实无所谓人们崇拜凡人...相反的，他们会设法将这些传奇归为神明赐福的信徒,这样也能扩大宗教的影响力。"

# game/BKhelp.rpy:2068
translate schinese help_zodiac_ebd70b3d:

    # sill "For instance, the women of Zan follow {b}the Eight{/b} for good fortune, especially slaves and whores. The Eight are saintly women-like figures that appear in the heavens when the conditions are right."
    sill "就拿青楼来说,泽恩有八位{b}传奇{/b}的女圣人。那八位圣人被认为是帮助人类繁衍和存续的化身,她们在大陆上播撒欲望和幸福。"

# game/BKhelp.rpy:2070
translate schinese help_zodiac_d2faaa6a:

    # sill "By dedicating ourselves to a particular {b}Zodiac saint{/b}, we believe that we can improve and be better at serving our purpose."
    sill "通过信仰{b}八位传奇{/b}可以让姑娘们得到提升,她们坚信自己可以更进一步,朝着传奇的方向努力。"

# game/BKhelp.rpy:2072
translate schinese help_zodiac_5ca4478a:

    # you "Sounds nice."
    you "听起来不错。"

# game/BKhelp.rpy:2074
translate schinese help_zodiac_c16fe67f:

    # sill "Every slave girl and prostitute follows a few of those Saints. A new girl will usually follow one or two of those depending on her natural inclinations."
    sill "每个女奴和妓女都信仰着几个这样的圣人。女孩的特质预示着她和对应的圣人十分契合。"

# game/BKhelp.rpy:2076
translate schinese help_zodiac_2e23e72b:

    # sill "With experience, girls can learn to pay their respects to different deities or deepen their relationship with one they already follow. The saints are rumored to grant {b}perks{/b} to their followers, each according to their own character."
    sill "随着等级的提高,女孩们可以效仿不同的圣人,或者加深对某一位的信仰。据说每位圣人都会根据自己的特质,赐予信徒{b}祝福{/b}。"

# game/BKhelp.rpy:2078
translate schinese help_zodiac_d5223255:

    # sill "No girl can get all the benefits of all zodiac signs at once, though. They have to carefully choose their path."
    sill "不过,女孩们不可能同时获得所有的祝福。她们必须谨慎选择自己的信仰。"

# game/BKhelp.rpy:2080
translate schinese help_zodiac_2de68c61:

    # "Every girl starts with {b}one or two zodiac signs{/b} unlocked according to her base traits."
    "每个女孩一开始都会根据她的基本特质解锁一到两个{b}天赋{/b}。"

# game/BKhelp.rpy:2082
translate schinese help_zodiac_71de0118:

    # "If a girl has only one zodiac sign unlocked, she receives the first perk for that sign for free."
    "如果一个女孩只解锁了一个天赋,她将会自动获得这个天赋的第一个祝福。"

# game/BKhelp.rpy:2084
translate schinese help_zodiac_8c4857ab:

    # "Every level, a girl receives {b}one or two perk points{/b}. Perk points can be used to unlock a new zodiac sign, or learn a new perk."
    "每升1级,女孩都会获得一到两个{b}天赋点{/b}。天赋点可以用于解锁新的祝福,或解锁一项新的天赋。"

# game/BKhelp.rpy:2086
translate schinese help_zodiac_05a4a2d6:

    # "Higher-end perks can only be bought after the girl improves her rank. To learn more, click on the level up or perks button in the girls tab."
    "更高级的祝福只有在女孩提升阶级后才能激活。如果想了解详细信息,请点击女孩页面上的升级或天赋按钮查询。"

# game/BKhelp.rpy:2099
translate schinese cheat_menu_22b4f2e2:

    # sill "WARNING! These cheats are for testing purposes, and may easily break your game! Only call this menu from the Home screen." nointeract
    sill "警告!这些作弊功能仅用于测试,这会破坏你的游戏体验!你只能在主页面使用作弊功能。" nointeract

# game/BKhelp.rpy:2462
translate schinese cheat_menu_43260813:

    # "This allows you to check if some girl pics have no usable tags (excluding the 'unused' tag)."
    "这可以检查是否有某些女孩图片没有可用的标签(不包括'未使用'标签)。"

# game/BKhelp.rpy:2644
translate schinese check_missing_pictures_5e0ae11a:

    # "Step 1: Checking profile and portrait pictures"
    "步骤1:检查头像和肖像图片"

# game/BKhelp.rpy:2660
translate schinese check_missing_pictures_4bf6234b:

    # "Step2: Checking job/sex pictures"
    "步骤2:检查职业/性别图片"

# game/BKhelp.rpy:2678
translate schinese check_missing_pictures_4f1baf65:

    # "Step3: Checking naked pictures"
    "步骤3: 检查裸体肖像"

# game/BKhelp.rpy:2693
translate schinese check_missing_pictures_c9cd67c9:

    # "Step4: Checking group and bisexual sex pictures"
    "步骤4: 检查群交和双飞词条图片"

# game/BKhelp.rpy:2711
translate schinese check_missing_pictures_51aadab5:

    # "Step1: Checking farm pictures"
    "步骤1: 检查农场训练图片"

# game/BKhelp.rpy:2728
translate schinese check_missing_pictures_e5aeea0e:

    # "Step2: Checking optional fixation pictures"
    "步骤2: 检查可选择性癖图片"

# game/BKhelp.rpy:2745
translate schinese check_missing_pictures_549bfd01:

    # "End of picture check."
    "图片检查结束。"

# game/BKhelp.rpy:2912
translate schinese test_perks_launch_66442e80:

    # "Generating girls...{nw}"
    "正在生成女孩...{nw}"

# game/BKhelp.rpy:2934
translate schinese test_perks_launch_9142d387:

    # "Running tests...{nw}"
    "正在运行测试...{nw}"

# game/BKhelp.rpy:2957
translate schinese test_perks_launch_52661ef2:

    # "Simulating month [month] out of [duration]...{nw}"
    "模拟第[month]个月,共[duration]个月...{nw}"

# game/BKhelp.rpy:3041
translate schinese test_perks_launch_53f68f38:

    # "Yielding test results...{nw}"
    "输出测试结果...{nw}"

# game/BKhelp.rpy:3052
translate schinese test_perks_launch_b7930d9a:

    # "Ending test..."
    "结束测试..."

translate schinese strings:

    # game/BKhelp.rpy:459
    old "Reset food effects"
    new "重置食物效果"

    # game/BKhelp.rpy:459
    old "Activate picture count in gallery (girl packs)"
    new "在图库中启用图片计数(女孩包)"

    # game/BKhelp.rpy:459
    old "Deactivate picture count in gallery"
    new "停用图库中的图片计数"

    # game/BKhelp.rpy:472
    old "Would you like to reset all existing counters to zero?"
    new "您是否要将所有现有计数器重置为零?"

    # game/BKhelp.rpy:524
    old "Choose an option"
    new "选择一个选项"

    # game/BKhelp.rpy:524
    old "Manage ignored pictures"
    new "管理被忽略的图片"

    # game/BKhelp.rpy:524
    old "Repair girl/MC pictures"
    new "修复女孩/主角图片"

    # game/BKhelp.rpy:531
    old "Pictures will be removed from the picture set on your next restart or by using the refresh option below."
    new "下次重启或使用下面的刷新选项时,这些图片将从图片集中删除。"

    # game/BKhelp.rpy:531
    old "Refresh pictures"
    new "刷新图片"

    # game/BKhelp.rpy:531
    old "Edit ignored pictures"
    new "编辑被忽略的图片"

    # game/BKhelp.rpy:531
    old "Clear ignored pictures"
    new "清除被忽略的图片"

    # game/BKhelp.rpy:531
    old "Print list of ignored pictures to file"
    new "将被忽略的图片列表列出到文件"

    # game/BKhelp.rpy:777
    old "Can I rename my character?"
    new "我可以重命名我的角色吗?"

    # game/BKhelp.rpy:777
    old "Tell me about my character and skills"
    new "告诉我关于我的角色和技能的信息"

    # game/BKhelp.rpy:777
    old "How can I change my portrait?"
    new "如何更换我的头像?"

    # game/BKhelp.rpy:777
    old "Tell me about prestige and skill points"
    new "告诉我关于声望和技能点的信息"

    # game/BKhelp.rpy:777
    old "Tell me how to use items"
    new "告诉我如何使用物品"

    # game/BKhelp.rpy:777
    old "Tell me how to learn and use spells"
    new "告诉我如何学习和使用法术"

    # game/BKhelp.rpy:883
    old "Can I change a girl's name?"
    new "我可以更改女孩的名字吗?"

    # game/BKhelp.rpy:883
    old "Tell me about defense, mood, love and fear"
    new "告诉我关于防御、情绪、爱情和恐惧的作用"

    # game/BKhelp.rpy:883
    old "Tell me about ranks, levels and job skills"
    new "告诉我关于等级、职业技能的知识"

    # game/BKhelp.rpy:883
    old "Tell me about the available actions"
    new "告诉我我可以做哪些事"

    # game/BKhelp.rpy:883
    old "Tell me about girl skills and traits"
    new "告诉我关于女孩技能和特质的作用"

    # game/BKhelp.rpy:883
    old "Tell me more about jobs"
    new "告诉我更多关于工作的信息"

    # game/BKhelp.rpy:883
    old "How can I train a girl to become a whore?"
    new "我如何培训一个女孩成为妓女?"

    # game/BKhelp.rpy:900
    old "Rename her 'Sill'"
    new "将她重命名为'希露'"

    # game/BKhelp.rpy:900
    old "Rename her 'Rose'"
    new "将她重命名为'萝丝'"

    # game/BKhelp.rpy:900
    old "Rename her 'Lolita'"
    new "将她重命名为'小萝莉'"

    # game/BKhelp.rpy:900
    old "Rename her 'Pinky'"
    new "将她重命名为'粉毛'"

    # game/BKhelp.rpy:900
    old "Rename her 'Peggy'"
    new "将她重命名为'佩琪'"

    # game/BKhelp.rpy:900
    old "Rename her 'Bitch'"
    new "将她重命名为'婊子'"

    # game/BKhelp.rpy:900
    old "Don't rename her"
    new "不为她重新命名"

    # game/BKhelp.rpy:979
    old "Rank and reputation"
    new "等级和声望"

    # game/BKhelp.rpy:979
    old "Leveling and experience"
    new "升级和经验"

    # game/BKhelp.rpy:979
    old "Zodiac signs and perks"
    new "天赋和特殊技能"

    # game/BKhelp.rpy:979
    old "Job skill and job points"
    new "工作技能和工作点数"

    # game/BKhelp.rpy:1022
    old "Changing a girl's job"
    new "改变女孩的工作"

    # game/BKhelp.rpy:1022
    old "Changing a girl's schedule"
    new "改变女孩的工作排班表"

    # game/BKhelp.rpy:1022
    old "Interacting with a girl"
    new "与女孩的互动"

    # game/BKhelp.rpy:1022
    old "Equipping and using items"
    new "装备和使用道具"

    # game/BKhelp.rpy:1022
    old "Selling a girl"
    new "出售女孩"

    # game/BKhelp.rpy:1022
    old "Perks, leveling and ranking up"
    new "特殊技能、升级和晋升"

    # game/BKhelp.rpy:1022
    old "Tracking a girl's performance"
    new "跟踪女孩的表现"

    # game/BKhelp.rpy:1128
    old "Tell me about skills"
    new "告诉我关于技能的信息"

    # game/BKhelp.rpy:1128
    old "Tell me about traits"
    new "告诉我关于特质的信息"

    # game/BKhelp.rpy:1128
    old "Tell me about upkeep"
    new "告诉我关于维护的信息"

    # game/BKhelp.rpy:1171
    old "Yes, fill me in"
    new "是的,请告诉我"

    # game/BKhelp.rpy:1223
    old "Waitress"
    new "女服务员"

    # game/BKhelp.rpy:1223
    old "Dancer"
    new "脱衣舞娘"

    # game/BKhelp.rpy:1223
    old "Masseuse"
    new "按摩技师"

    # game/BKhelp.rpy:1223
    old "Geisha"
    new "表演艺伎"

    # game/BKhelp.rpy:1223
    old "Whore"
    new "妓女"

    # game/BKhelp.rpy:1292
    old "How can I check my brothel's information?"
    new "我如何查看我的青楼信息?"

    # game/BKhelp.rpy:1292
    old "Tell me more about rooms"
    new "告诉我更多关于房间的信息"

    # game/BKhelp.rpy:1292
    old "Tell me more about freelancers"
    new "告诉我更多关于自由职业者的信息"

    # game/BKhelp.rpy:1292
    old "Tell me about trainers"
    new "告诉我关于调教师的信息"

    # game/BKhelp.rpy:1292
    old "Tell me about the Carpenter's Wagon and Customer options"
    new "告诉我关于工匠改造和经营策略的信息"

    # game/BKhelp.rpy:1377
    old "Tell me about..."
    new "告诉我关于..."

    # game/BKhelp.rpy:1377
    old "Customers"
    new "客户"

    # game/BKhelp.rpy:1377
    old "Brothel reputation"
    new "青楼声望"

    # game/BKhelp.rpy:1377
    old "Customer attraction"
    new "吸引客户"

    # game/BKhelp.rpy:1377
    old "Customer budgets"
    new "客户预算"

    # game/BKhelp.rpy:1377
    old "Advertising power"
    new "广告力度"

    # game/BKhelp.rpy:1482
    old "Tell me about districts"
    new "告诉我关于区域的信息"

    # game/BKhelp.rpy:1482
    old "Tell me about unlocking new districts"
    new "告诉我解锁新区域的信息"

    # game/BKhelp.rpy:1522
    old "Tell me about meeting girls"
    new "告诉我如何招募女孩"

    # game/BKhelp.rpy:1522
    old "Tell me about looking around"
    new "告诉我如何巡视"

    # game/BKhelp.rpy:1554
    old "Tell me about sexual experience"
    new "告诉我关于性经验的信息"

    # game/BKhelp.rpy:1554
    old "No, I'm good"
    new "不,不必了"

    # game/BKhelp.rpy:1611
    old "Tell me about classes"
    new "告诉我关于培训课程的信息"

    # game/BKhelp.rpy:1611
    old "Tell me about quests"
    new "告诉我关于任务的信息"

    # game/BKhelp.rpy:1657
    old "DISCLAIMER"
    new "免责声明"

    # game/BKhelp.rpy:1657
    old "What is this game?"
    new "这是什么游戏?"

    # game/BKhelp.rpy:1657
    old "How can I help?"
    new "我能为您做些什么?"

    # game/BKhelp.rpy:1657
    old "Special thanks"
    new "特别感谢"

    # game/BKhelp.rpy:1657
    old "I have a bug to report/gameplay suggestion"
    new "我有一个错误要报告/一个游戏建议"

    # game/BKhelp.rpy:1718
    old "Tell me about brothel reputation and customers"
    new "告诉我关于青楼风评和客户的信息"

    # game/BKhelp.rpy:1718
    old "Tell me about advertising, security and maintenance"
    new "告诉我关于广告、安保和清洁的信息"

    # game/BKhelp.rpy:1718
    old "Tell me about girl performances"
    new "告诉我关于女孩表演的信息"

    # game/BKhelp.rpy:1782
    old "You've just had your first security event. Would you like to learn more about security events?"
    new "您刚刚经历了第一次安全事件。您想了解更多关于安全事件的信息吗?"

    # game/BKhelp.rpy:1865
    old "Tell me about the farm"
    new "告诉我关于奴隶农场的信息"

    # game/BKhelp.rpy:1865
    old "Tell me about pens and facilities"
    new "告诉我关于地牢和设施的信息"

    # game/BKhelp.rpy:1865
    old "Tell me about minions and facilities"
    new "告诉我关于仆从和设施的信息"

    # game/BKhelp.rpy:1865
    old "Tell me about girl training and rules"
    new "告诉我训练员工的规则和方法"

    # game/BKhelp.rpy:1865
    old "Go back"
    new "返回"

    # game/BKhelp.rpy:2097
    old "Test event"
    new "测试事件"

    # game/BKhelp.rpy:2097
    old "Cheat modifier"
    new "调整游戏难度"

    # game/BKhelp.rpy:2097
    #old "Gold"
    #new "获得金币"

    # game/BKhelp.rpy:2097
    #old "Main character"
    #new "修改主角状态"

    # game/BKhelp.rpy:2097
    #old "Girls"
    #new "修改员工属性"

    # game/BKhelp.rpy:2097
    #old "Free girls"
    #new "调整陌生女孩"

    # game/BKhelp.rpy:2097
    #old "Items"
    #new "获取物品道具"

    # game/BKhelp.rpy:2097
    #old "Tags"
    #new "修改标签"

    # game/BKhelp.rpy:2097
    #old "Others"
    #new "其他"

    # game/BKhelp.rpy:2194
    old "The cheat modifier is a float number affecting stat gains, xp, jp, reputation and gold earned by your girls. Use it to adjust the difficulty level (e.g.: a modifier of 1.5 gives your girl a 50% increase); a modifier of 0.75 decreases gains by 25%)"
    new "游戏难度影响你的女孩获得的统计数据、经验值、技能点、声望和金币。你可以通过改变它来调整难度(例如:1.5倍的倍率会使你的女孩获得50%的增益);0.75倍的倍率会减少25%的收益。"

    # game/BKhelp.rpy:2194
    old "Set global cheat modifier"
    new "全局作弊修改"

    # game/BKhelp.rpy:2194
    old "Set modifiers separately"
    new "自定义作弊修改"

    # game/BKhelp.rpy:2215
    old "Get 1,000 gold"
    new "获得1,000金币"

    # game/BKhelp.rpy:2215
    old "Get 10,000 gold"
    new "获得10,000金币"

    # game/BKhelp.rpy:2215
    old "Get 100,000 gold"
    new "获得100,000金币"

    # game/BKhelp.rpy:2215
    old "Custom amount"
    new "获得自定义数量的金币"

    # game/BKhelp.rpy:2235
    old "Level MC"
    new "提升主角等级"

    # game/BKhelp.rpy:2235
    old "Reset interactions"
    new "重置互动次数"

    # game/BKhelp.rpy:2235
    old "Check MC effects"
    new "检查增益效果"

    # game/BKhelp.rpy:2262
    old "Get naked"
    new "脱光衣服"

    # game/BKhelp.rpy:2262
    old "Give girls perk points"
    new "获得天赋点数"

    # game/BKhelp.rpy:2262
    old "Give girls upgrade points"
    new "获得属性点数"

    # game/BKhelp.rpy:2262
    old "Rank girls up"
    new "提升员工阶级"

    # game/BKhelp.rpy:2262
    old "Level girls up"
    new "提升员工等级"

    # game/BKhelp.rpy:2262
    old "Job skill up"
    new "提升职业等级"

    # game/BKhelp.rpy:2262
    old "Raise libido and obedience"
    new "提高性欲和服从"

    # game/BKhelp.rpy:2262
    old "Raise sexual preferences"
    new "提高性行为倾向"

    # game/BKhelp.rpy:2262
    old "Set all girls to rest"
    new "让所有员工休息"

    # game/BKhelp.rpy:2262
    old "Sell all girls"
    new "出售所有员工"

    # game/BKhelp.rpy:2262
    old "Give all girls a random job"
    new "给予所有女孩一份随机的工作"

    # game/BKhelp.rpy:2262
    old "Fully rest girls"
    new "让女孩恢复至满状态"

    # game/BKhelp.rpy:2262
    old "Reset girl interactions"
    new "重置女孩互动次数"

    # game/BKhelp.rpy:2262
    old "Change love"
    new "改变好感度"

    # game/BKhelp.rpy:2262
    old "Change fear"
    new "改变恐惧值"

    # game/BKhelp.rpy:2262
    old "Force girl to run away"
    new "强制让女孩逃跑"

    # game/BKhelp.rpy:2374
    old "Get new free girls"
    new "补充陌生女孩"

    # game/BKhelp.rpy:2374
    old "Update free girls"
    new "刷新陌生女孩"

    # game/BKhelp.rpy:2374
    old "Cycle free girls"
    new "轮换陌生女孩"

    # game/BKhelp.rpy:2374
    old "List free girls"
    new "列出陌生女孩"

    # game/BKhelp.rpy:2407
    old "Refresh shops"
    new "刷新商店"

    # game/BKhelp.rpy:2407
    old "Get all items"
    new "获得所有物品"

    # game/BKhelp.rpy:2407
    old "Get item"
    new "获得指定物品"

    # game/BKhelp.rpy:2407
    old "Clear items"
    new "清除物品"

    # game/BKhelp.rpy:2407
    old "Build all furniture"
    new "完成所有设施升级"

    # game/BKhelp.rpy:2449
    old "Check girls for missing pictures (main)"
    new "检查女孩包是否有缺失的图片(主要)"

    # game/BKhelp.rpy:2449
    old "Check girls for missing pictures (optional)"
    new "检查女孩包是否有缺失的图片(可选)"

    # game/BKhelp.rpy:2449
    old "Check untagged girl pics"
    new "检查未标记的女孩图片"

    # game/BKhelp.rpy:2449
    old "Check tags for duplicates"
    new "检查标签是否有重复"

    # game/BKhelp.rpy:2480
    old "Test brothel events"
    new "测试青楼事件"

    # game/BKhelp.rpy:2480
    old "Change brothel reputation"
    new "改变青楼声望"

    # game/BKhelp.rpy:2480
    old "Refresh slave market"
    new "刷新奴隶市场"

    # game/BKhelp.rpy:2480
    old "Refresh postings"
    new "刷新任务和课程"

    # game/BKhelp.rpy:2480
    old "Toggle secret locations"
    new "切换隐藏地点"

    # game/BKhelp.rpy:2480
    old "Skip time"
    new "跳过时间"

    # game/BKhelp.rpy:2480
    old "Advance to chapter"
    new "自由选择主线进度"

    # game/BKhelp.rpy:2480
    old "Change goal"
    new "更改目标"

    # game/BKhelp.rpy:2480
    old "Check goals"
    new "检查目标"

    # game/BKhelp.rpy:2480
    old "List available story events"
    new "列出可用的故事事件"

    # game/BKhelp.rpy:2480
    old "Check active brothel effects"
    new "检查激活的青楼效果"

    # game/BKhelp.rpy:2480
    old "Add minion"
    new "添加仆从"

    # game/BKhelp.rpy:2480
    old "Get resources"
    new "获取资源"

    # game/BKhelp.rpy:2480
    old "Wound minions"
    new "伤害仆从"

    # game/BKhelp.rpy:2480
    old "Test special characters"
    new "测试特殊角色"

    # game/BKhelp.rpy:2525
    old "What type of goal are you going to achieve?"
    new "您打算实现什么类型的目标?"

    # game/BKhelp.rpy:2525
    old "Raise gold"
    new "提高金钱"

    # game/BKhelp.rpy:2525
    old "Rank up your girls"
    new "提升女孩阶级"

    # game/BKhelp.rpy:2525
    old "Raise brothel reputation"
    new "提高青楼风评"

    # game/BKhelp.rpy:2525
    old "Raise player prestige"
    new "提高玩家声望"

    # game/BKhelp.rpy:2583
    old "Stables"
    new "种马马厩"

    # game/BKhelp.rpy:2583
    old "Pig stall"
    new "发情猪圈"

    # game/BKhelp.rpy:2583
    old "Monster den"
    new "怪物巢穴"

    # game/BKhelp.rpy:2583
    old "Workshop"
    new "机械工房"

    # game/BKhelp.rpy:2632
    old "Check all girl packs"
    new "检查所有女孩包"

    # game/BKhelp.rpy:2632
    old "Check a specific girl pack"
    new "检查特定女孩包"

    # game/BKhelp.rpy:2890
    old "Testing perks: [perk_text]"
    new "测试天赋: [perk_text]"

    # game/BKhelp.rpy:2890
    old "Girls: [girl_nb]"
    new "女孩: [girl_nb]"

    # game/BKhelp.rpy:2890
    old "Duration: [duration] months"
    new "持续时间: [duration] 月"

    # game/BKhelp.rpy:2890
    old "Job: [jobs]"
    new "工作: [jobs]"

    # game/BKhelp.rpy:2890
    old "Show events: [show_ev]"
    new "显示事件: [show_ev]"

    # game/BKhelp.rpy:2890
    #old "GO"
    #new "开始"

    # game/BKhelp.rpy:360
    old "BROKIPEDIA"
    new "青楼百科全书"

    # game/BKhelp.rpy:380
    #old "Close"
    #new "关闭"

    # game/BKhelp.rpy:420
    old "Previous notifications"
    new "之前的通知"

    # game/BKhelp.rpy:429
    old "How can I help you?"
    new "我能帮到您什么?"

    # game/BKhelp.rpy:2227
    old "How much?"
    new "多少?"

    # game/BKhelp.rpy:2424
    old "Item name contains"
    new "物品名称包含"

    # game/BKhelp.rpy:3069
    old "Test results - "
    new "测试结果 - "

    # game/BKhelp.rpy:3089
    old "Gold made-Test"
    new "获得的金钱-测试"

    # game/BKhelp.rpy:3099
    old "Gold made-Ctrl"
    new "金币制作-Ctrl"

    # game/BKhelp.rpy:3109
    old "Gold adv."
    new "金币进阶"

    # game/BKhelp.rpy:3118
    old "Stats-Test"
    new "属性-测试"

    # game/BKhelp.rpy:3127
    old "Stats-Ctrl"
    new "属性-Ctrl"

    # game/BKhelp.rpy:3136
    old "Stat adv."
    new "属性进阶"

    # game/BKhelp.rpy:3144
    old "Level-Test"
    new "等级-测试"

    # game/BKhelp.rpy:3153
    old "Level-Ctrl"
    new "等级-Ctrl"

    # game/BKhelp.rpy:3166
    old "Rank-Test"
    new "排名-测试"

    # game/BKhelp.rpy:3175
    old "Rank-Ctrl"
    new "排名-Ctrl"

    # game/BKhelp.rpy:3188
    old "Rep-Test"
    new "声望-测试"

    # game/BKhelp.rpy:3197
    old "Rep-Ctrl"
    new "声望-Ctrl"

    # game/BKhelp.rpy:3209
    old "RESTART"
    new "重新开始"

    # game/BKhelp.rpy:3210
    old "NEW TEST"
    new "新的测试"

    # game/BKhelp.rpy:3211
    #old "STOP"
    #new "停止"


###########################################################
#  ________  ________  ________        ___  ________      #
# |\   __  \|\   __  \|\   __  \      |\  \|\_____  \     #
# \ \  \|\  \ \  \|\  \ \  \|\  \     \ \  \\|___/  /|    #
#  \ \   ____\ \   _  _\ \  \\\  \  __ \ \  \   /  / /    #
#   \ \  \___|\ \  \\  \\ \  \\\  \|\  \\_\  \ /  /_/__   #
#    \ \__\    \ \__\\ _\\ \_______\ \________\\________\ #
#     \|__|     \|__|\|__|\|_______|\|________|\|_______| #
#                                                         #
#  This rpy file is generated by the project:             #
#  https://github.com/abse4411/projz_renpy_translation    #
###########################################################
# TODO: Translation updated at 2024-07-31 02:24

# game/BKhelp.rpy:396
translate schinese help_powers_0ae9bcd0:

    # ""
    ""

# game/BKhelp.rpy:775
translate schinese help_MC_fc6cac84:

    # sill happy "{nw}"
    sill happy "{nw}"

# game/BKhelp.rpy:881
translate schinese help_girls_fc6cac84:

    # sill happy "{nw}"
    sill happy "{nw}"

# game/BKhelp.rpy:1290
translate schinese help_brothel_fc6cac84:

    # sill happy "{nw}"
    sill happy "{nw}"

# game/BKhelp.rpy:1434
translate schinese help_advertising_menu_004618db:

    # you "我知道了, 谢谢你。"
    you "我知道了, 谢谢你。"

# game/BKhelp.rpy:1480
translate schinese help_districts_fc6cac84:

    # sill happy "{nw}"
    sill happy "{nw}"

# game/BKhelp.rpy:1520
translate schinese help_visit_location_fc6cac84:

    # sill happy "{nw}"
    sill happy "{nw}"

# game/BKhelp.rpy:1609
translate schinese help_postings_fc6cac84:

    # sill happy "{nw}"
    sill happy "{nw}"

# game/BKhelp.rpy:1861
translate schinese help_farm_c57ebb83:

    # gizel normal "{nw}"
    gizel normal "{nw}"

# game/BKhelp.rpy:1948
translate schinese help_farm_menu_dae0ec55:

    # you "..."
    you "..."

# game/BKhelp.rpy:2095
translate schinese cheat_menu_a355d470:

    # sill sad "{nw}"
    sill sad "{nw}"

# game/BKhelp.rpy:2099
translate schinese cheat_menu_001b47f3:

    # sill "警告! 作弊功能只是用于测试, 否则会毁了你的游戏体验! 只有在主界面才能使用作弊按钮。" nointeract
    sill "警告! 作弊功能只是用于测试, 否则会毁了你的游戏体验! 只有在主界面才能使用作弊按钮。" nointeract

# game/BKhelp.rpy:2612
translate schinese cheat_menu_3eba7cfb:

    # "{font=[gui.fuhao]}✓{/font} [emo_heart] [emo_lightning]"
    "{font=[gui.fuhao]}✓{/font} [emo_heart] [emo_lightning]"

translate schinese strings:

    # game/BKhelp.rpy:459
    old "重置食物效果"
    new "重置食物效果"

    # game/BKhelp.rpy:472
    old "是的"
    new "是的"

    # game/BKhelp.rpy:472
    old "算了"
    new "算了"

    # game/BKhelp.rpy:524
    old "管理忽略的图片"
    new "管理忽略的图片"

    # game/BKhelp.rpy:524
    old "修复女孩/主角图片"
    new "修复女孩/主角图片"

    # game/BKhelp.rpy:531
    old "刷新图片"
    new "刷新图片"

    # game/BKhelp.rpy:531
    old "编辑忽略的图片"
    new "编辑忽略的图片"

    # game/BKhelp.rpy:531
    old "清楚忽略的图片"
    new "清楚忽略的图片"

    # game/BKhelp.rpy:531
    old "将忽略的图片整理列出"
    new "将忽略的图片整理列出"

    # game/BKhelp.rpy:777
    old "我可以重命名主角吗?"
    new "我可以重命名主角吗?"

    # game/BKhelp.rpy:777
    old "介绍一下主角的职业和技能"
    new "介绍一下主角的职业和技能"

    # game/BKhelp.rpy:777
    old "怎么切换我的肖像?"
    new "怎么切换我的立绘?"

    # game/BKhelp.rpy:777
    old "介绍一下声望和技能点"
    new "介绍一下声望和技能点"

    # game/BKhelp.rpy:777
    old "介绍一下如何使用道具"
    new "介绍一下如何使用道具"

    # game/BKhelp.rpy:777
    old "介绍一下如何学习并使用法术"
    new "介绍一下如何学习并使用法术"

    # game/BKhelp.rpy:777
    old "没事了"
    new "没事了"

    # game/BKhelp.rpy:883
    old "我可以修改女孩的名字吗?"
    new "我可以修改女孩的名字吗?"

    # game/BKhelp.rpy:883
    old "介绍一下女孩的技能和天赋"
    new "介绍一下女孩的技能和天赋"

    # game/BKhelp.rpy:883
    old "介绍一下女孩的职业"
    new "介绍一下女孩的职业"

    # game/BKhelp.rpy:883
    old "如何训练女孩成为妓女?"
    new "如何训练女孩成为妓女?"

    # game/BKhelp.rpy:900
    old "把她的名字改为'希露'"
    new "把她的名字改为'希露'"

    # game/BKhelp.rpy:900
    old "把她的名字改为'露丝'"
    new "把她的名字改为'露丝'"

    # game/BKhelp.rpy:900
    old "把她的名字改为'萝莉'"
    new "把她的名字改为'萝莉'"

    # game/BKhelp.rpy:900
    old "把她的名字改为'粉毛'"
    new "把她的名字改为'粉毛'"

    # game/BKhelp.rpy:900
    old "把她的名字改为'母狗'"
    new "把她的名字改为'母狗'"

    # game/BKhelp.rpy:900
    old "把她的名字改为'婊子'"
    new "把她的名字改为'婊子'"

    # game/BKhelp.rpy:900
    old "不修改她的名字"
    new "不修改她的名字"

    # game/BKhelp.rpy:979
    old "阶级和声望"
    new "阶级和声望"

    # game/BKhelp.rpy:979
    old "等级和经验"
    new "等级和经验"

    # game/BKhelp.rpy:979
    old "佐迪亚克标志和天赋"
    new "佐迪亚克标志和天赋"

    # game/BKhelp.rpy:979
    old "职业技能和技能点"
    new "职业技能和技能点"

    # game/BKhelp.rpy:979
    old "返回"
    new "返回"

    # game/BKhelp.rpy:1022
    old "如何更换女孩的工作"
    new "如何更换女孩的工作"

    # game/BKhelp.rpy:1022
    old "如何修改女孩的排班"
    new "如何修改女孩的排班"

    # game/BKhelp.rpy:1022
    old "如何与女孩互动"
    new "如何与女孩互动"

    # game/BKhelp.rpy:1022
    old "如何装备和使用道具"
    new "如何装备和使用道具"

    # game/BKhelp.rpy:1022
    old "如何出售女孩"
    new "如何出售女孩"

    # game/BKhelp.rpy:1022
    old "天赋，等级和升阶"
    new "天赋，等级和升阶"

    # game/BKhelp.rpy:1022
    old "如何查看女孩的近期表现"
    new "如何查看女孩的近期表现"

    # game/BKhelp.rpy:1128
    old "介绍一下技能"
    new "介绍一下技能"

    # game/BKhelp.rpy:1128
    old "介绍一下特质"
    new "介绍一下特质"

    # game/BKhelp.rpy:1128
    old "介绍一下保养费"
    new "介绍一下保养费"

    # game/BKhelp.rpy:1171
    old "当然，告诉我"
    new "当然，告诉我"

    # game/BKhelp.rpy:1171
    old "不用了"
    new "不用了"

    # game/BKhelp.rpy:1223
    old "女服务员"
    new "女服务员"

    # game/BKhelp.rpy:1223
    old "脱衣舞娘"
    new "脱衣舞娘"

    # game/BKhelp.rpy:1223
    old "按摩技师"
    new "按摩技师"

    # game/BKhelp.rpy:1223
    old "表演艺伎"
    new "表演艺伎"

    # game/BKhelp.rpy:1223
    old "卖淫妓女"
    new "卖淫妓女"

    # game/BKhelp.rpy:1292
    old "如何查看青楼的信息?"
    new "如何查看青楼的现状?"

    # game/BKhelp.rpy:1292
    old "介绍一下青楼的房间"
    new "介绍一下青楼的房间"

    # game/BKhelp.rpy:1292
    old "介绍一下外包人员"
    new "介绍一下外包人员"

    # game/BKhelp.rpy:1292
    old "关于调教师"
    new "关于调教师"

    # game/BKhelp.rpy:1330
    old "不了"
    new "不了"

    # game/BKhelp.rpy:1377
    old "我想要知道..."
    new "我想要知道..."

    # game/BKhelp.rpy:1377
    old "关于顾客"
    new "关于顾客"

    # game/BKhelp.rpy:1377
    old "关于青楼的评价"
    new "关于青楼的评价"

    # game/BKhelp.rpy:1377
    old "关于顾客的吸引力"
    new "关于顾客的吸引力"

    # game/BKhelp.rpy:1377
    old "关于顾客的预算"
    new "关于顾客的预算"

    # game/BKhelp.rpy:1377
    old "广告宣传的作用"
    new "广告宣传的作用"

    # game/BKhelp.rpy:1377
    old "没有要问的了"
    new "没有要问的了"

    # game/BKhelp.rpy:1482
    old "关于城市里的地区"
    new "关于城市里的地区"

    # game/BKhelp.rpy:1482
    old "如何解锁新的地区"
    new "如何解锁新的地区"

    # game/BKhelp.rpy:1522
    old "如何邂逅陌生女孩"
    new "如何邂逅陌生女孩"

    # game/BKhelp.rpy:1522
    old "如何探索城市地区"
    new "如何探索城市地区"

    # game/BKhelp.rpy:1554
    old "介绍一下女孩的技能和特质"
    new "介绍一下女孩的技能和特质"

    # game/BKhelp.rpy:1554
    old "介绍一下女孩的职业内容"
    new "介绍一下女孩的职业"

    # game/BKhelp.rpy:1554
    old "介绍一下性培训经验等级"
    new "介绍一下性训练等级"

    # game/BKhelp.rpy:1554
    old "不用了，没什么要问的"
    new "我没什么要问的了"

    # game/BKhelp.rpy:1611
    old "关于培训课程"
    new "关于培训课程"

    # game/BKhelp.rpy:1611
    old "关于委托任务"
    new "关于委托任务"

    # game/BKhelp.rpy:1657
    old "免责声明"
    new "免责声明"

    # game/BKhelp.rpy:1657
    old "这是个什么样的游戏?"
    new "这是个什么样的游戏?"

    # game/BKhelp.rpy:1657
    old "如何支持游戏?"
    new "我能通过什么方式支持游戏?"

    # game/BKhelp.rpy:1657
    old "特别鸣谢"
    new "特别鸣谢"

    # game/BKhelp.rpy:1718
    old "介绍一下青楼的名声和顾客"
    new "介绍一下青楼的名声和顾客"

    # game/BKhelp.rpy:1718
    old "介绍一下广告，保安和清洁工"
    new "介绍一下广告，保安和清洁工"

    # game/BKhelp.rpy:1718
    old "介绍一下女孩的表现"
    new "介绍一下女孩的表现"

    # game/BKhelp.rpy:1782
    old "好的"
    new "好的"

    # game/BKhelp.rpy:1865
    old "介绍一下农场"
    new "介绍一下农场"

    # game/BKhelp.rpy:1865
    old "介绍一下牢房和设施"
    new "介绍一下牢房和设施"

    # game/BKhelp.rpy:1865
    old "介绍一下仆从和设施"
    new "介绍一下仆从和设施"

    # game/BKhelp.rpy:1865
    old "介绍一下女孩训练的规则"
    new "介绍一下女孩训练的规则"

    # game/BKhelp.rpy:2097
    old "测试游戏事件"
    new "测试游戏事件"

    # game/BKhelp.rpy:2097
    old "调整游戏难度"
    new "调整游戏难度"

    # game/BKhelp.rpy:2097
    old "获得大量金币"
    new "获得大量金币"

    # game/BKhelp.rpy:2097
    old "修改主角属性"
    new "修改主角属性"

    # game/BKhelp.rpy:2097
    old "修改青楼女孩们的属性"
    new "修改青楼女孩们的属性"

    # game/BKhelp.rpy:2097
    old "调整城市里的陌生女孩"
    new "调整城市里的陌生女孩"

    # game/BKhelp.rpy:2097
    old "刷新商店及获得道具"
    new "刷新商店及获得道具"

    # game/BKhelp.rpy:2097
    old "检查图片标签"
    new "检查图片标签"

    # game/BKhelp.rpy:2097
    old "其他作弊功能"
    new "其他作弊功能"

    # game/BKhelp.rpy:2097
    old "返回游戏界面"
    new "返回游戏界面"

    # game/BKhelp.rpy:2194
    old "调整全局的难度系数"
    new "调整全局的难度系数"

    # game/BKhelp.rpy:2194
    old "自定义各项数值系数"
    new "自定义各项数值系数"

    # game/BKhelp.rpy:2215
    old "获得一千金币"
    new "获得一千金币"

    # game/BKhelp.rpy:2215
    old "获得一万金币"
    new "获得一万金币"

    # game/BKhelp.rpy:2215
    old "获得十万金币"
    new "获得十万金币"

    # game/BKhelp.rpy:2215
    old "获得自定义数量金币"
    new "获得自定义数量金币"

    # game/BKhelp.rpy:2215
    old "返回上一级"
    new "返回上一级"

    # game/BKhelp.rpy:2235
    old "提升主角等级"
    new "提升主角等级"

    # game/BKhelp.rpy:2235
    old "重置互动次数"
    new "重置互动次数"

    # game/BKhelp.rpy:2235
    old "检查增益效果"
    new "检查增益效果"

    # game/BKhelp.rpy:2262
    old "让所有的女孩都脱光衣服"
    new "让所有的女孩都脱光衣服"

    # game/BKhelp.rpy:2262
    old "让所有女孩获得天赋点数"
    new "让所有女孩获得天赋点数"

    # game/BKhelp.rpy:2262
    old "让所有女孩获得属性点数"
    new "让所有女孩获得属性点数"

    # game/BKhelp.rpy:2262
    old "晋升所有女孩的角色阶级"
    new "晋升所有女孩的角色阶级"

    # game/BKhelp.rpy:2262
    old "提升所有女孩的角色等级"
    new "提升所有女孩的角色等级"

    # game/BKhelp.rpy:2262
    old "提高所有女孩的职业等级"
    new "提高所有女孩的职业等级"

    # game/BKhelp.rpy:2262
    old "提高女孩们的性欲和服从"
    new "提高女孩们的性欲和服从"

    # game/BKhelp.rpy:2262
    old "提高所有女孩性行为倾向"
    new "提高所有女孩性行为倾向"

    # game/BKhelp.rpy:2262
    old "让所有人停止工作去休息"
    new "让所有人停止工作去休息"

    # game/BKhelp.rpy:2262
    old "出售掉青楼里所有的女孩"
    new "出售掉青楼里所有的女孩"

    # game/BKhelp.rpy:2262
    old "给所有人一份随机的工作"
    new "给所有人一份随机的工作"

    # game/BKhelp.rpy:2262
    old "让女孩们得到充分的休息"
    new "让女孩们得到充分的休息"

    # game/BKhelp.rpy:2262
    old "重置所有女孩的互动次数"
    new "重置所有女孩的互动次数"

    # game/BKhelp.rpy:2262
    old "修改所有女孩的好感数值"
    new "修改所有女孩的好感数值"

    # game/BKhelp.rpy:2262
    old "修改所有女孩的恐惧数值"
    new "修改所有女孩的恐惧数值"

    # game/BKhelp.rpy:2262
    old "强制让某个女孩逃出青楼"
    new "强制让某个女孩逃出青楼"

    # game/BKhelp.rpy:2262
    old "返回上一级菜单"
    new "返回上一级菜单"

    # game/BKhelp.rpy:2374
    old "补充新的陌生女孩"
    new "补充新的陌生女孩"

    # game/BKhelp.rpy:2374
    old "刷新新的陌生女孩"
    new "刷新新的陌生女孩"

    # game/BKhelp.rpy:2374
    old "轮换一批陌生女孩"
    new "轮换一批陌生女孩"

    # game/BKhelp.rpy:2374
    old "重置陌生女孩互动"
    new "重置陌生女孩互动"

    # game/BKhelp.rpy:2374
    old "列出所有陌生女孩"
    new "列出所有陌生女孩"

    # game/BKhelp.rpy:2407
    old "刷新商店货架"
    new "刷新商店货架"

    # game/BKhelp.rpy:2407
    old "获得所有道具"
    new "获得所有道具"

    # game/BKhelp.rpy:2407
    old "获得指定道具"
    new "获得指定道具"

    # game/BKhelp.rpy:2407
    old "清空所有道具"
    new "清空所有道具"

    # game/BKhelp.rpy:2407
    old "完成所有改造"
    new "完成所有改造"

    # game/BKhelp.rpy:2449
    old "检查缺失的图片 (main)"
    new "检查缺失的图片 (main)"

    # game/BKhelp.rpy:2449
    old "检查缺失的图片 (optional)"
    new "检查缺失的图片 (optional)"

    # game/BKhelp.rpy:2449
    old "检查没有标签的图片"
    new "检查没有标签的图片"

    # game/BKhelp.rpy:2449
    old "检查有无重复的标签"
    new "检查有无重复的标签"

    # game/BKhelp.rpy:2481
    old "测试青楼事件"
    new "测试青楼事件"

    # game/BKhelp.rpy:2481
    old "改变青楼声望"
    new "改变青楼声望"

    # game/BKhelp.rpy:2481
    old "刷新奴隶市场"
    new "刷新奴隶市场"

    # game/BKhelp.rpy:2481
    old "刷新公告大厅"
    new "刷新公告大厅"

    # game/BKhelp.rpy:2481
    old "跳过指定天数"
    new "跳过指定天数"

    # game/BKhelp.rpy:2481
    old "跳转主线章节"
    new "跳转主线章节"

    # game/BKhelp.rpy:2481
    old "改变游戏目标"
    new "改变游戏目标"

    # game/BKhelp.rpy:2481
    old "检查游戏目标"
    new "检查游戏目标"

    # game/BKhelp.rpy:2481
    old "列出所有可用的剧情事件"
    new "列出所有可用的剧情事件"

    # game/BKhelp.rpy:2481
    old "检查激活的青楼增益效果"
    new "检查激活的青楼增益效果"

    # game/BKhelp.rpy:2481
    old "添加指定种类的仆从"
    new "添加指定种类的仆从"

    # game/BKhelp.rpy:2481
    old "获得指定数量的资源"
    new "获得指定数量的资源"

    # game/BKhelp.rpy:2481
    old "让你的仆从受伤"
    new "让你的仆从受伤"

    # game/BKhelp.rpy:2481
    old "测试特殊的角色"
    new "测试特殊的角色"

    # game/BKhelp.rpy:2526
    old "你想在游戏中达成什么样的目标?"
    new "你想在游戏中达成什么样的目标?"

    # game/BKhelp.rpy:2526
    old "赚到足够多的钱"
    new "赚到足够多的钱"

    # game/BKhelp.rpy:2526
    old "晋升你的女孩们"
    new "晋升你的女孩们"

    # game/BKhelp.rpy:2526
    old "提高青楼的声望"
    new "提高青楼的声望"

    # game/BKhelp.rpy:2526
    old "提高主角的声望"
    new "提高主角的声望"

    # game/BKhelp.rpy:2633
    old "选择一个选项"
    new "选择一个选项"

    # game/BKhelp.rpy:2633
    old "检查所有的女孩图包"
    new "检查所有的女孩图包"

    # game/BKhelp.rpy:2633
    old "检查特定的女孩图包"
    new "检查特定的女孩图包"

    # game/BKhelp.rpy:2891
    old "女孩: [girl_nb]"
    new "女孩: [girl_nb]"

    # game/BKhelp.rpy:2891
    old "持续: [duration] months"
    new "持续: [duration] months"

    # game/BKhelp.rpy:2891
    old "职业: [jobs]"
    new "职业: [jobs]"

    # game/BKhelp.rpy:2891
    old "展示事件: [show_ev]"
    new "展示事件: [show_ev]"

    # game/BKhelp.rpy:2891
    old "开始测试"
    new "开始测试"

