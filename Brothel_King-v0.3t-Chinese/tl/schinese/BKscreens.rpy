# TODO: Translation updated at 2024-04-27 18:30

# game/BKscreens.rpy:8914
translate schinese packstates_menu_da3bbd8f:

    # "Oftentimes, a Girlpack creator may wish to change some of the picture names to better fit Brothel King's tagging system. The packstate feature helps updating girlpacks without having to download hundreds of pictures all over again."
    "很多时候,Girlpack创作者可能想要改变一些图片名称,以更好地适应Brothel King的标签系统。Packstate功能可以帮助更新Girlpack,无需再次下载数百张图片。"

# game/BKscreens.rpy:8915
translate schinese packstates_menu_83fb1d69:

    # "" "{b}Packstates{/b} contain all the necessary information to keep the tags of a Girlpack up to date. These files need to be put into the {color=[c_magenta]}/game/[packdir]{/color} directory and named exactly like the girlpack they are for."
    "" "{b}Packstates{/b}包含了保持Girlpack标签最新所需的所有信息。这些文件需要放在{color=[c_magenta]}/game/[packdir]{/color}目录中,并与Girlpack完全同名。"

# game/BKscreens.rpy:8916
translate schinese packstates_menu_35c34b3e:

    # "" "During the renaming process, a {b}log file{/b} is created in the BrothelKing main directory\n({color=[c_magenta]}packstate_log.txt{/color}), showing in detail everything that was changed."
    "" "在重命名过程中,会在BrothelKing主目录中创建一个{b}日志文件{/b}\n({color=[c_magenta]}packstate_log.txt{/color}),详细记录所有更改情况。"

# game/BKscreens.rpy:8917
translate schinese packstates_menu_88f8438c:

    # "" "You can also do a {b}simulation{/b}. This creates the same log file, but without actually renaming any files. This lets you check which changes would be made, without any risk."
    "" "你也可以进行{b}模拟{/b}。这会创建相同的日志文件,但不会实际重命名任何文件。这让你可以检查会做哪些更改,而不存在任何风险。"

# game/BKscreens.rpy:8918
translate schinese packstates_menu_9b65325a:

    # "" "No files are actually deleted. Instead, unwanted files are tagged as {b}_TRASH{/b}. These files will not get used by BrothelKing, so you can safely just leave them there, or delete them for real. You may notice the duplicate tag on some _TRASH files - these are duplicates of existing images."
    "" "不会真正删除任何文件。相反,不需要的文件将被标记为{b}_TRASH{/b}。这些文件不会被BrothelKing使用,所以你可以放心地将它们留在那里或彻底删除。你可能会注意到一些_TRASH文件上有重复标签 - 这些是现有图像的副本。"

# game/BKscreens.rpy:8919
translate schinese packstates_menu_f158589e:

    # "" "If you have added your own images, they will get tagged as {b}_UNRECOGNIZED{/b} by default. They will still get used by BrothelKing."
    "" "如果你添加了自己的图像,它们将默认被标记为{b}_UNRECOGNIZED{/b}。它们仍将被BrothelKing使用。"

# game/BKscreens.rpy:8920
translate schinese packstates_menu_4afe306a:

    # "Please note that {b}duplicates{/b} only get detected for recognized images. You can change this behavior in the {b}packstates menu{/b} (Ignore will not even rename them, Hide will prevent them from showing in the game)."
    "请注意,{b}重复项{/b}只会针对已识别的图像进行检测。你可以在{b}packstates菜单{/b}中更改此行为(忽略将不会重命名它们,隐藏将阻止它们在游戏中显示)。"

# game/BKscreens.rpy:8921
translate schinese packstates_menu_53287101:

    # "" "That's all! Why not try a {b}simulation{/b} and see if the {color=[c_magenta]}/packstate_log.txt{/color} shows any useful changes?"
    "" "就是这些!为什么不尝试一下{b}模拟{/b},看看{color=[c_magenta]}/packstate_log.txt{/color}是否显示任何有用的更改?"

translate schinese strings:

    # game/BKscreens.rpy:388
    old "Day: [calendar.day] ("
    new "天: [calendar.day]  ("

    # game/BKscreens.rpy:413
    old "AP: Your remaining actions for today."
    new "AP: 您今日剩余的行动次数。"

    # game/BKscreens.rpy:423
    old "MP: Your current mana."
    new "MP: 您当前的魔力值。"

    # game/BKscreens.rpy:700
    old "is a level {0} {1}."
    new "她是一个{0}级的{1}。"

    # game/BKscreens.rpy:703
    old "{/b} is resting."
    new "{/b}正在休息。"

    # game/BKscreens.rpy:729
    old ". Click for details."
    new "。点击查看详情。"

    # game/BKscreens.rpy:776
    old "She has "
    new "她还有"

    # game/BKscreens.rpy:776
    old " energy left out of "
    new " 点体力剩余，总共"

    # game/BKscreens.rpy:1134
    old "Open%s's schedule"
    new "打开 %s 的排班表"

    # game/BKscreens.rpy:1141
    old "Open %s's entry in your magical notebook"
    new "在你的魔法笔记本里打开 %s 的条目"

    # game/BKscreens.rpy:1462
    old "Click to change her name"
    new "为她重新取名"

    # game/BKscreens.rpy:1497
    old "{b}Helpless"
    new "{b}手无寸铁"

    # game/BKscreens.rpy:1499
    old "{b}Mostly harmless"
    new "{b}娇花易折"

    # game/BKscreens.rpy:1501
    old "{b}Competent"
    new "{b}红颜侠客"

    # game/BKscreens.rpy:1503
    old "{b}Dangerous"
    new "{b}绝色杀手"

    # game/BKscreens.rpy:1505
    old "{b}Deadly"
    new "{b}致命诱惑"

    # game/BKscreens.rpy:1507
    old "{/b}\nHow well she can defend herself. Raise this by giving her a weapon."
    new "{/b}\n她的自卫能力如何。给她一把武器可以提高自卫能力。"

    # game/BKscreens.rpy:1566
    old "Rank "
    new "阶级 "

    # game/BKscreens.rpy:1581
    old "Level "
    new "等级 "

    # game/BKscreens.rpy:1705
    old "She has {b}"
    new "她还剩余{b}"

    # game/BKscreens.rpy:1705
    old "{/b} energy remaining. Her maximum energy is {b}"
    new "{/b}点体力。她的最大体力为{b}"

    # game/BKscreens.rpy:1705
    old "{/b} (increase constitution for higher energy)."
    new "{/b}(提高体质可增加最大体力)。"

    # game/BKscreens.rpy:1798
    old " Prior training may make a girl more suitable for sex acts."
    new "之前接受的训练可能会使女孩更适合从事性行为。"

    # # game/BKscreens.rpy:1799
    # old "Prior training received:   {color="
    # new "性奴隶训练等级:   {color="

    # game/BKscreens.rpy:1845
    old "This will activate {b}"
    new "这将激活{b}"

    # game/BKscreens.rpy:1845
    old "{/b} for this girl."
    new "{/b}的能力。"

    # game/BKscreens.rpy:1990
    old "Auto upkeep setting is {b}{color=[c_green]}on{/color}{/b} {i}(shortcut: {u}Shift+a{/u}){/i}"
    new "自动调整保养费用功能状态为{b}{color=[c_green]}激活{/color}{/b} \n({i}快捷键: {u}Shift+a{/u}{/i})"

    # game/BKscreens.rpy:1992
    old "Auto upkeep setting is {b}{color=[c_red]}off{/color}{/b} {i}(shortcut: {u}Shift+a{/u}){/i}"
    new "自动调整保养费用功能状态为{b}{color=[c_red]}停用{/color}{/b} \n({i}快捷键: {u}Shift+a{/u}{/i})"

    # game/BKscreens.rpy:2003
    old "\nWhen this is turned on, current upkeep balance will 'lock', and upkeep will rise and fall automatically."
    new "\n当此功能激活时,费用将'锁定'在这个百分比,它会自动调整。"

    # game/BKscreens.rpy:2005
    old " gold"
    new "金币"

    # game/BKscreens.rpy:2055
    old "Tell "
    new "让"

    # game/BKscreens.rpy:2055
    old " to get some rest."
    new "去休息一下吧。"

    # game/BKscreens.rpy:2065
    old "Ask "
    new "指派"

    # game/BKscreens.rpy:2065
    old " to work as a "
    new "作为"

    # game/BKscreens.rpy:2065
    old " AAAAA "
    new "继续工作"

    # game/BKscreens.rpy:2076
    old "\n(unavailable)"
    new "\n(不可用)"

    # game/BKscreens.rpy:2083
    old "Send "
    new "送"

    # game/BKscreens.rpy:2083
    old " to the farm."
    new "到奴隶农场接受调教。"

    # game/BKscreens.rpy:2094
    old "Remove "
    new "今晚 "

    # game/BKscreens.rpy:2094
    old " from your bedroom."
    new "不用来你的私人卧室了。"

    # game/BKscreens.rpy:2097
    old "Add "
    new "今晚让 "

    # game/BKscreens.rpy:2097
    old " to your bedroom."
    new " 到你的私人卧室接受指导。"

    # game/BKscreens.rpy:2219
    old "Defense"
    new "防御"

    # game/BKscreens.rpy:2292
    old "Click to buy "
    new "点击购买 "

    # game/BKscreens.rpy:2327
    old "She is away on a class or assignment for %s more day."
    new "她正在外出完成任务或是正在参加培训，还需要 %s 天返回。"

    # game/BKscreens.rpy:2332
    old "This girl is hurt and will need to rest for 1 more day until she is ready to do anything."
    new "她受伤了,需要休息1天才能恢复工作。"

    # game/BKscreens.rpy:2334
    old "This girl is hurt and will need to rest for "
    new "她受伤了,需要休息"

    # game/BKscreens.rpy:2334
    old " more days until she is ready to do anything."
    new " 天才能恢复工作。"

    # game/BKscreens.rpy:2338
    old "This girl needs to be fully rested until she can work again."
    new "她需要彻底休息恢复精力后才能再次工作。"

    # game/BKscreens.rpy:2342
    old "This girl has been set to rest today according with her schedule."
    new "排班表上显示她今天轮休。"

    # game/BKscreens.rpy:2346
    old "No job assigned. This girl has been set to rest until further instructions."
    new "她没有被分配工作。在得到指示决定工作前她将一直在宿舍里休息。"

    # game/BKscreens.rpy:2350
    old "Working and whoring. Change this girl's job or let her rest."
    new "正在一边工作一边勾引客人。换一个岗位或者让她去休息。"

    # game/BKscreens.rpy:2354
    old "Change this girl's job or let her rest."
    new "换一个岗位或者让她去休息。"

    # game/BKscreens.rpy:2356
    old " ({i}shortcut: {u}j{/u}{/i})"
    new " \n({i}快捷键: {u}J{/u}{/i})"

    # game/BKscreens.rpy:2413
    old "Change this girl's equipment."
    new "更换她的装备 \n({i}快捷键: {u}T{/u}{/i})"

    # game/BKscreens.rpy:2430
    old "Sell this girl for %s gold (original cost: %s gold). ({i}shortcut: {u}Delete{/u}{/i})"
    new "以 %s 金币的价格出售她 (原价: %s 金币). \n({i}快捷键: {u}Delete{/u}{/i})"
    
    old "You have "
    new "你有 "

    # game/BKscreens.rpy:2439
    old " perk points to spend.\nRight-click to access perks."
    new " 天赋点可用。\n右击进入天赋界面。"

    # game/BKscreens.rpy:2579
    old "RANK"
    new "阶级"

    # game/BKscreens.rpy:2589
    old "LEVEL"
    new "等级"

    # game/BKscreens.rpy:2593
    old "REPUTATION"
    new "名气"

    # game/BKscreens.rpy:2597
    old "EXPERIENCE"
    new "经验"

    # game/BKscreens.rpy:2604
    old "SKILLS"
    new "技能"

    # game/BKscreens.rpy:2888
    old " is ready to level up."
    new " 可以升级。"

    # game/BKscreens.rpy:2959
    old "%s's perks"
    new "%s的天赋树"

    # game/BKscreens.rpy:3051
    old "Perk points: "
    new "天赋点: "

    # game/BKscreens.rpy:3086
    old "%s's traits"
    new "%s 的特质"

    # game/BKscreens.rpy:3131
    old "%s's active perks"
    new "%s 的主动技能"

    # game/BKscreens.rpy:3266
    old "{b}Profit: {color=[c_white]}"
    new "{b}利润: {color=[c_white]}"

    # game/BKscreens.rpy:3267
    old "\nJobs: {color=[c_green]}"
    new "\n工作: {color=[c_green]}"

    # game/BKscreens.rpy:3268
    old "     Quests: {color=[c_green]}"
    new "     任务: {color=[c_green]}"

    # game/BKscreens.rpy:3269
    old "\nUpkeep: {color=[c_red]}-"
    new "\n保养费用: {color=[c_red]}-"

    # game/BKscreens.rpy:3302
    old "Waitress: "
    new "服务: "

    # game/BKscreens.rpy:3302
    old "              Dancer: "
    new "              舞伎: "

    # game/BKscreens.rpy:3302
    old "\nMasseuse: "
    new "\n按摩: "

    # game/BKscreens.rpy:3302
    old "            Geisha: "
    new "            艺妓: "

    # game/BKscreens.rpy:3302
    old "\nWhore: "
    new "\n妓女: "

    # game/BKscreens.rpy:3302
    old "                 Work/whore : "
    new "                 勾引客人 : "

    # game/BKscreens.rpy:3578
    old "\nrequired"
    new "\n需要"

    # game/BKscreens.rpy:3746
    old "{/b}. Press "
    new "{/b}。按 "

    # game/BKscreens.rpy:3746
    old " to visit this location."
    new " 来访问该地点。"

    # game/BKscreens.rpy:3788
    old ". You cannot collect as you are out of AP."
    new "。你的行动点用完了,无法收集。"

    # game/BKscreens.rpy:3792
    old ". Costs 1 {image=img_AP}."
    new "。需要消耗1 {image=img_AP}。"

    # game/BKscreens.rpy:3857
    old "Talk to "
    new "与 "

    # game/BKscreens.rpy:3920
    old " (free)."
    new " 交谈 (免费)。"

    # game/BKscreens.rpy:3968
    old "Carpenter's {u}W{/u}agon"
    new "青楼改造"

    # game/BKscreens.rpy:3993
    old "Customer {u}o{/u}ptions"
    new "经营策略"

    # game/BKscreens.rpy:3993
    old "Fine-tune your brothel for various customer populations and preferences."
    new "调整经营策略以满足不同顾客群体和喜好。\n({i}快捷键: {u}O{/u}{/i})"

    # game/BKscreens.rpy:4015
    old "You must pay {b}"
    new "你必须支付 {b}"

    # game/BKscreens.rpy:4015
    old "{/b} gold for your brothel services. You must also pay {b}"
    new "{/b} 金币作为青楼支出。你还需要支付 {b}"

    # game/BKscreens.rpy:4015
    old "{/b} gold for your girls upkeep"
    new "{/b} 金币作为员工的薪水"

    # game/BKscreens.rpy:4019
    old " (not accounting for special effects)."
    new " (不包括特殊效果)。"

    # game/BKscreens.rpy:4021
    old "Daily cost: "
    new "每日支出: "

    # game/BKscreens.rpy:4023
    old "Click to change your brothel's name."
    new "变更你的青楼名称。"

    # game/BKscreens.rpy:4110
    old "{/color} from girls/effects)"
    new "{/color}来自女孩/效果)"

    # game/BKscreens.rpy:4115
    old "[brothel.advertising]  babes"
    new "[brothel.advertising]  宣发人员"

    # game/BKscreens.rpy:4136
    old "[brothel.security]  goons"
    new "[brothel.security]  打手保镖"

    # game/BKscreens.rpy:4159
    old "[brothel.maintenance]  cleaners"
    new "[brothel.maintenance]  家政清洁"

    # game/BKscreens.rpy:4205
    old "Upgrade all your bedrooms for "
    new "花费"

    # game/BKscreens.rpy:4205
    old " gold. Upgraded bedrooms are more comfortable for girls and customers alike."
    new " 金币一次性升级所有房间。升级后房间会更加舒适,女孩的心情也会更好。"

    # game/BKscreens.rpy:4382
    old "Oh, hi. Got a new job for me?"
    new "需要我做些什么?还是说你想和我到马车里疼爱我一番？"

    # game/BKscreens.rpy:4384
    old "I'm still working on that "
    new "我还在处理那个"

    # game/BKscreens.rpy:4384
    old ". You should come back later."
    new ". 你过会儿再来看看吧。"

    # game/BKscreens.rpy:4406
    old "'s Decoration and Furniture"
    new "已拥有的的设施和升级"

    # game/BKscreens.rpy:4436
    old " day to complete)."
    new " 天后可以完成)。"

    # game/BKscreens.rpy:4438
    old " days to complete)."
    new " 天后可以完成)。"

    # game/BKscreens.rpy:4645
    old "Waiting customers ({image=img_cust} %i)/%i"
    new "排队的顾客 ({image=img_cust} %i)/%i"

    # game/BKscreens.rpy:4715
    old "Bedrooms"
    new "卧室"

    # game/BKscreens.rpy:4754
    old " (done)"
    new " (完成)"

    # game/BKscreens.rpy:4792
    old "Brothel reputation: %s"
    new "青楼风评: %s "

    # game/BKscreens.rpy:4804
    old "Customer"
    new "顾客"

    # game/BKscreens.rpy:4805
    old "Satisfaction"
    new "满意度"

    # game/BKscreens.rpy:4806
    old "Rep."
    new "风评"

    # game/BKscreens.rpy:4807
    old "Comment"
    new "评价"

    # game/BKscreens.rpy:5119
    old "Ok"
    new "确定"

    # game/BKscreens.rpy:5337
    old "Restock inventory"
    new "额外采购"

    # game/BKscreens.rpy:5337
    old "Restock this shop's inventory for %s gold (available once a day)."
    new "用 %s 金币刷新商品(一天限一次)。"

    # game/BKscreens.rpy:5346
    #old "Upgrade this shop's inventory (+%s %s item%s) for %s %s."
    #new "扩容货架(+%s %s 件商品 %s )消耗 %s %s 。"

    # game/BKscreens.rpy:5348
    old "Upgrade shop"
    new "扩容商店"

    # game/BKscreens.rpy:5609
    old "Active Girl mixes: %s"
    new "可用的女孩组合: %s"

    # game/BKscreens.rpy:5626
    old "Click to remove mix: {b}%s{/b} from this game's active mixes."
    new "点击移除组合: {b}%s{/b} 出现在这个游戏的活跃组合中。"

    # game/BKscreens.rpy:5631
    old "Click to add mix: {b}%s{/b} to this game's active mixes."
    new "点击添加组合: {b}%s{/b} 到这个游戏的活跃组合中。"

    # game/BKscreens.rpy:5763
    old "You earn prestige everytime you or your girls have sex, or when one of your girl earns a new level."
    new "每当你或你的女孩进行性行为,或者你的女孩升级时,你都会获得声望点数。"

    # game/BKscreens.rpy:5766
    old "\nYou have reached the maximum level."
    new "\n你已经达到最高等级。"

    # game/BKscreens.rpy:5768
    old "\nYou need "
    new "\n你需要 "

    # game/BKscreens.rpy:5768
    old " prestige to level up."
    new " 点声望点来升级。"

    # game/BKscreens.rpy:5782
    old " skill points"
    new " 技能点"

    old "{b}Current goal{/b}\n{i}{size=-2}"
    new "{b}当前目标{/b}\n{i}{size=-2}"

    # game/BKscreens.rpy:5808
    old "Spellboo{u}k{/u}"
    new "法术书"

    # game/BKscreens.rpy:5808
    old "See all available spells and active talents"
    new "查看可用的法术和天赋技能"

    # game/BKscreens.rpy:6025
    old " days"
    new " 天"

    # game/BKscreens.rpy:6039
    old " girls"
    new " 女孩"

    # game/BKscreens.rpy:6150
    old "This task requires "
    new "这个任务需要 "

    # game/BKscreens.rpy:6151
    old " girls can complete this task."
    new " 名女孩合作才能完成。"

    # game/BKscreens.rpy:6155
    old "This class may improve "
    new "这个课程可能会提高 "

    # game/BKscreens.rpy:6704
    old "%s's mood"
    new " %s 的心情"

    # game/BKscreens.rpy:6795
    old "%s's sexual preferences"
    new " %s 的性偏好"

    # # game/BKscreens.rpy:6801
    # old "Act"
    # new "性行为"

    # # game/BKscreens.rpy:6802
    # old "Preference"
    # new "偏好"

    # # game/BKscreens.rpy:6803
    # old "Will train"
    # new "训练意向"

    # # game/BKscreens.rpy:6804
    # old "Will work"
    # new "工作意向"

    # game/BKscreens.rpy:6900
    old " challenge{/b}: This challenges your {b}"
    new " 挑战{/b}: 这挑战了你的 {b}"

    # game/BKscreens.rpy:6900
    old ". Estimated difficulty: {b}"
    new ". 预计难度: {b}"

    # game/BKscreens.rpy:6939
    old "Player challenge: "
    new "玩家挑战: "

    # game/BKscreens.rpy:6948
    old "Player "
    new "玩家 "

    # game/BKscreens.rpy:6949
    old "Active bonus: "
    new "激活奖励: "

    # game/BKscreens.rpy:6957
    old "Final Result: "
    new "最终结果: "

    # game/BKscreens.rpy:6970
    old "Opponent "
    new "对手 "

    # game/BKscreens.rpy:6979
    old "Roll"
    new "判定"

    # game/BKscreens.rpy:6981
    old "Difficulty: "
    new "难度: "

    # game/BKscreens.rpy:7031
    old " You have "
    new " 你有"

    # game/BKscreens.rpy:7031
    old " in store."
    new " 的储备."

    # game/BKscreens.rpy:7106
    old "There is a shortage of "
    new "本周"

    # game/BKscreens.rpy:7106
    old " this week. Value is going up."
    new " 供应短缺。物价上升。"

    # game/BKscreens.rpy:7114
    old " is plentiful this week. Value is going down."
    new " 本周供应充足。物价下跌。"

    # game/BKscreens.rpy:7562
    # old "You know that [girl.name] has "
    # new "你知道 [girl.name] 有 "

    # # game/BKscreens.rpy:7562
    # old " acts."
    # new "行为。"

    # game/BKscreens.rpy:7564
    # old "You do not know [girl.name]'s reaction to "
    # new "你不知道 [girl.name] 对 "

    # game/BKscreens.rpy:7573
    old "Lecture [girl.name] about the virtues of "
    new "的态度。让 [girl.name] 了解 "

    # game/BKscreens.rpy:7573
    old " acts (soft).\nCosts "
    new "行为(温和)的优点。\n成本 "

    # game/BKscreens.rpy:7584
    old "Train [girl.name] for "
    new "训练 [girl.name] 的 "

    # game/BKscreens.rpy:7584
    old " acts.\nCosts "
    new "行为。\n成本 "

    # game/BKscreens.rpy:7686
    old "Content settings"
    new "内容设置"

    # game/BKscreens.rpy:7727
    old "Picture settings"
    new "图片设置"

    # game/BKscreens.rpy:7778
    old "UI settings"
    new "UI设置"

    # game/BKscreens.rpy:7853
    # old "Misc"
    # new "其他"

    # game/BKscreens.rpy:7983
    old "Girl pack mix"
    new "女孩包组合"

    # game/BKscreens.rpy:7983
    old "Update packstates"
    new "更新包状态"

    # game/BKscreens.rpy:7986
    old "Would you like to see girl ratings (this may take some time if you have many girl packs)?"
    new "你想查看女孩评级吗(如果你有很多女孩包,可能需要加载一些时间)?"

    # game/BKscreens.rpy:8910
    old "Welcome to the packstate feature (courtesy of {color=[c_magenta]}{b}Chris12{/b}{/color})"
    new "欢迎来到包状态功能 (由{color=[c_magenta]}{b}Chris12{/b}{/color}提供)"

    # game/BKscreens.rpy:8910
    old "Introduction to Packstates"
    new "包状态介绍"

    # game/BKscreens.rpy:8910
    old "Unrecognized Images: [preferences.packstate_unrecognized]"
    new "无法识别的图像: [preferences.packstate_unrecognized]"

    # game/BKscreens.rpy:8910
    old "Simulation"
    new "模拟"

    # game/BKscreens.rpy:8910
    old "Apply packstate"
    new "应用包状态"

    # game/BKscreens.rpy:8925
    old "Hide: Rename and don't show unrecognized images.\nRename: Rename unrecognized images, but show them.\nIgnore: Don't rename unrecognized images. Will also show them.\n Removes any _UNRECOGNIZED tags again.\n(Renaming means adding _UNRECOGNIZED as tag to the filename)"
    new "隐藏: 重命名并不显示无法识别的图像。\n重命名: 重命名无法识别的图像,但显示它们。\n忽略: 不重命名无法识别的图像。也会显示它们。\n 再次删除任何 _UNRECOGNIZED 标签。\n(重命名意味着向文件名添加 _UNRECOGNIZED 作为标记)"

    # game/BKscreens.rpy:8925
    old "Hide"
    new "隐藏"

    # game/BKscreens.rpy:8925
    old "Rename"
    new "重命名"

    # game/BKscreens.rpy:8925
    old "Ignore"
    new "忽略"

    # game/BKscreens.rpy:8925
    old "Back (don't change setting)"
    new "返回(不保存更改)"

    # game/BKscreens.rpy:8945
    old "It is recommended that you backup your girls folder and run a simulation beforehand. There is no Undo operation!{fast}{nw}"
    new "建议您先备份女孩文件夹并运行模拟。没有撤消操作!{fast}{nw}"

    # game/BKscreens.rpy:245
    old "No guild fee is due."
    new "无需支付会费。"

    # game/BKscreens.rpy:274
    old "Guild Fee"
    new "会费"

    # game/BKscreens.rpy:386
    old "Year: [calendar.year]"
    new "年: [calendar.year]"

    # game/BKscreens.rpy:387
    old "Month: [calendar.month]"
    new "月: [calendar.month]"

    # game/BKscreens.rpy:639
    old "{i}  No girl available  {/i}"
    new "{i}  没有可用的女孩  {/i}"

    # game/BKscreens.rpy:652
    old "Choose a badge for [girl.fullname]"
    new "为[girl.fullname]选择一个标记"

    # game/BKscreens.rpy:796
    old "Rk"
    new "阶级"

    # game/BKscreens.rpy:799
    old "Lv"
    new "等级"

    # game/BKscreens.rpy:1098
    old "Rank"
    new "阶级"

    # game/BKscreens.rpy:1101
    old "Level"
    new "等级"

    # game/BKscreens.rpy:1234
    old "Training mode:"
    new "训练模式:"

    # game/BKscreens.rpy:1244
    old "Training facility:"
    new "训练设施:"

    # game/BKscreens.rpy:1265
    old "Use Weakness:"
    new "针对弱点:"

    # game/BKscreens.rpy:1270
    old "Holding mode:"
    new "当前状态:"

    # game/BKscreens.rpy:1278
    old "Change program"
    new "更改训练内容"

    # game/BKscreens.rpy:1315
    old "Girl rating ("
    new "女孩评级 ("

    # game/BKscreens.rpy:1541
    old "Sanity: "
    new "理智: "

    # game/BKscreens.rpy:1654
    old "Main skills"
    new "主要属性"

    # game/BKscreens.rpy:1770
    old "Sex skills"
    new "性技能力"

    # game/BKscreens.rpy:1892
    old "Traits"
    new "特质"

    # game/BKscreens.rpy:1946
    old "Upkeep"
    new "保养费用"

    # game/BKscreens.rpy:1949
    old "Her upkeep is currently withdrawn."
    new "她目前的保养费用用已被提取。"

    # game/BKscreens.rpy:2292
    old "Bu{u}y{/u}"
    new "购买"

    # game/BKscreens.rpy:2370
    old "Sche{u}d{/u}ule"
    new "排班"

    # game/BKscreens.rpy:2388
    old "{u}I{/u}nteract"
    new "互动"

    # game/BKscreens.rpy:2406
    old "I{u}t{/u}ems"
    new "物品"

    # game/BKscreens.rpy:2419
    old "Dismiss"
    new "解放"

    # game/BKscreens.rpy:2436
    old "Level {u}u{/u}p"
    new "升级"

    # game/BKscreens.rpy:2447
    old "Per{u}k{/u}s"
    new "天赋"

    old "Check her current perks"
    new "点击查看她的天赋\n({i}快捷键: {u}K{/u}{/i})"

    # game/BKscreens.rpy:2459
    old "{u}R{/u}ank up"
    new "升阶"

    # game/BKscreens.rpy:2464
    old "St{u}a{/u}ts"
    new "状态"

    old "Test girl pack with the game's picture generation."
    new "测试女孩包与游戏的图片生成."

    # game/BKscreens.rpy:2470
    old "Pics"
    new "图片"

    # game/BKscreens.rpy:2498
    old "Current relationship: "
    new "亲密度: "

    # game/BKscreens.rpy:2535
    old "Le{u}a{/u}ve farm"
    new "离开奴隶农场"

    # game/BKscreens.rpy:2671
    old "Girl Schedule"
    new "排班表"

    # game/BKscreens.rpy:2783
    old "Autorest options"
    new "自动恢复选项"

    # game/BKscreens.rpy:2795
    old "Save %s's schedule"
    new "保存%s的排班表"

    # game/BKscreens.rpy:2820
    old "Load a schedule for %s"
    new "为%s套用一个排班表"

    # game/BKscreens.rpy:2851
    old "This makes your girls rest automatically if their energy falls too low.\nLeft-click to increase threshold\nRight-click to lower it"
    new "你的员工体力过低时会自动休息。\n左键提高阈值\n右键降低阈值"

    # game/BKscreens.rpy:2860
    old "OK"
    new "确定"

    # game/BKscreens.rpy:2895
    old "Available points:"
    new "可用点数:"

    # game/BKscreens.rpy:3043
    old "Unlock\n{size=-6}(costs 2 perk points){/size}"
    new "解锁这个天赋树\n{size=-6}(需要2个天赋点){/size}"

    # game/BKscreens.rpy:3054
    old "Confirm"
    new "确认"

    # game/BKscreens.rpy:3200
    old "Yesterday"
    new "昨天"

    # game/BKscreens.rpy:3202
    old "Last 7 days"
    new "最近一周"

    # game/BKscreens.rpy:3204
    old "Last 28 days"
    new "最近一个月"

    # game/BKscreens.rpy:3206
    old "All time"
    new "总共"

    # game/BKscreens.rpy:3232
    old "General"
    new "综合"

    # game/BKscreens.rpy:3241
    old "{b}Days{/b}"
    new "{b}天{/b}"

    # game/BKscreens.rpy:3252
    old "{b}Gold{/b}"
    new "{b}金币{/b}"

    # game/BKscreens.rpy:3273
    old "{b}XP{/b}"
    new "{b}经验{/b}"

    # game/BKscreens.rpy:3277
    old "{b}JP{/b}"
    new "{b}技能点{/b}"

    # game/BKscreens.rpy:3281
    old "{b}Reputation{/b}"
    new "{b}风评{/b}"

    # game/BKscreens.rpy:3286
    old "Activity"
    new "活动"

    # game/BKscreens.rpy:3300
    old "{b}Worked{/b}"
    new "{b}工作{/b}"

    # game/BKscreens.rpy:3306
    old "{b}Rested{/b}"
    new "{b}休息{/b}"

    # game/BKscreens.rpy:3310
    old "{b}Away{/b}"
    new "{b}离开{/b}"

    # game/BKscreens.rpy:3322
    old "{b}On strike{/b}"
    new "{b}罢工{/b}"

    # game/BKscreens.rpy:3326
    old "{b}Hurt/Sick{/b}"
    new "{b}受伤/生病{/b}"

    # game/BKscreens.rpy:3338
    old "Jobs"
    new "工作"

    # game/BKscreens.rpy:3348
    old "{b}Customers{/b}"
    new "{b}顾客{/b}"

    # game/BKscreens.rpy:3358
    old "{b}Av. score{/b}"
    new "{b}平均评价{/b}"

    # game/BKscreens.rpy:3393
    old "{b}Whore{/b}"
    new "{b}妓女{/b}"

    # game/BKscreens.rpy:3426
    old "Sex Acts"
    new "性侍奉"

    # game/BKscreens.rpy:3440
    old "{b}Xp{/b}"
    new "{b}经验{/b}"

    # game/BKscreens.rpy:3479
    old "Hints collected:"
    new "收集到的情报:"

    # game/BKscreens.rpy:3544
    old "The Chase"
    new "追杀"

    # game/BKscreens.rpy:3553
    old "No license\nrequired"
    new "无需许可\n"

    # game/BKscreens.rpy:3909
    old "Take a look around (1 "
    new "环顾四周 (1 "

    # game/BKscreens.rpy:3933
    old "Hunt ninjas"
    new "猎杀忍者"

    # game/BKscreens.rpy:4030
    old "{b}Trainer{/b}"
    new "{b}调教师{/b}"

    # game/BKscreens.rpy:4049
    old "A Trainers help your girls learning new skills. Discover new trainers by meeting the people of Zan!"
    new "调教师可以帮助你的员工学会新技能，到处转转，结识一些新朋友吧！"

    # game/BKscreens.rpy:4054
    old "No other trainer available"
    new "无其他可用调教师"

    # game/BKscreens.rpy:4056
    old "Trainers help your girls learning new skills. Discover new trainers by meeting the people of Zan!"
    new "调教师可以帮助你的员工学会新技能，到处转转，结识一些新朋友吧！"

    # game/BKscreens.rpy:4065
    old "Change trainer."
    new "上一个调教师"

    # game/BKscreens.rpy:4070
    old "next trainer."
    new "下一个调教师"

    # game/BKscreens.rpy:4079
    old "{i}Recruit a trainer to help your girls.{/i}"
    new "{i}招募调教师来帮助你的女孩。{/i}"

    # game/BKscreens.rpy:4084
    old "{b}Helpers{/b}"
    new "{b}外 包{/b}"

    # game/BKscreens.rpy:4096
    old "Your brothel's current reputation is {b}"
    new "你的青楼当前风评为 {b}"

    # game/BKscreens.rpy:4098
    old "Advertising"
    new "广告宣传"

    # game/BKscreens.rpy:4106
    old "Pay hot chicks with revealing clothing to hang around your brothel, and tell would-be patrons about your establishment."
    new "让一群穿着性感的舞女在你的青楼附近翩翩起舞，时不时地漏点福利，让潜在的顾客看看你的青楼有多好。"

    # game/BKscreens.rpy:4119
    old "Security"
    new "安全保障"

    # game/BKscreens.rpy:4127
    old "Pay some sellswords to keep unruly patrons and competitors at bay."
    new "这些彪形大汉和他们手里的家伙事能让不安分的客人老实点。如果你加钱，女打手自然更养眼。"

    # game/BKscreens.rpy:4142
    old "Maintenance"
    new "家政清洁"

    # game/BKscreens.rpy:4150
    old "Pay a maintenance team to clean up your brothel. And boy, does it get messy in there..."
    new "花钱请专业家政团队来清理你的青楼。瞧这女仆装，这黑丝袜...可别在小妹打扫卫生时骚扰她，除非你加钱。"

    # game/BKscreens.rpy:4164
    old "Estimated customers"
    new "预计客流量"

    # game/BKscreens.rpy:4168
    old "Threat level"
    new "威胁等级"

    # game/BKscreens.rpy:4173
    old "Dirt level"
    new "卫生情况"

    # game/BKscreens.rpy:4174
    old "Clean up"
    new "打扫青楼"

    # game/BKscreens.rpy:4203
    old "{b}Bedrooms{/b}"
    new "{b}员工宿舍{/b}"

    # game/BKscreens.rpy:4264
    old "{b}Master Bedroom{/b}"
    new "{b}私人卧室{/b}"

    # game/BKscreens.rpy:4316
    old "{b}Common Rooms{/b}"
    new "{b}营业场所{/b}"

    # game/BKscreens.rpy:4376
    #old "Carpenter's Wagon"
    #new "工匠改造"

    # game/BKscreens.rpy:4389
    old "Available resources"
    new "可用资源"

    # game/BKscreens.rpy:4394
    old "Building Queue"
    new "建筑队列"

    # game/BKscreens.rpy:4404
    old "No building in progress."
    new "没有正在进行的改造项目。"

    # game/BKscreens.rpy:4415
    old "Build templates"
    new "改造列表"

    # game/BKscreens.rpy:4474
    old "{b}Customer populations{/b}"
    new "{b}顾客群体{/b}"

    # game/BKscreens.rpy:4478
    old "Choose customer populations to attract to your brothel (build decoration to attract more)"
    new "选择青楼面向的顾客群体(升级青楼来吸引更多种类和数量的客人)"

    # game/BKscreens.rpy:4498
    old "Average budget: %s gold"
    new "平均预算: %s 金币"

    # game/BKscreens.rpy:4500
    old "{b}Customer preferences{/b}"
    new "{b}顾客喜好{/b}"

    # game/BKscreens.rpy:4504
    old "Influence customer preferences for entertainment and sexual acts (build furnishing to get bigger boosts)"
    new "影响顾客对娱乐和性行为的偏好(通过升级青楼获得更多提升)"

    # game/BKscreens.rpy:4531
    old "{b}Matching preferences{/b}"
    new "{b}匹配喜好{/b}"

    # game/BKscreens.rpy:4536
    old "Choose how incoming customers will be matched with your girls."
    new "选择如何将到访顾客与您的员工进行匹配。"

    # game/BKscreens.rpy:4543
    old "By %s"
    new "根据 %s "

    # game/BKscreens.rpy:4551
    old "{b}Special options{/b}"
    new "{b}特殊选项{/b}"

    # game/BKscreens.rpy:4556
    old "Activate or deactivate special brothel furniture."
    new "激活或停用特殊的青楼设施能力。"

    # game/BKscreens.rpy:4571
    old "{b}Advanced settings{/b}"
    new "{b}高级设置{/b}"

    # game/BKscreens.rpy:4576
    old "In addition to improving your brothel reputation, advertising girls will increase customer attraction and customer budget based on advertising power (create new outfits to increase advertising power)."
    new "除了提高青楼的风评外,宣发人员还会根据宣传效果提高顾客吸引力和顾客预算(拥有更多新服装可以增加宣传效果)。"

    # game/BKscreens.rpy:4584
    old "Budget"
    new "预算"

    # game/BKscreens.rpy:4589
    old "{b}Forecast{/b}"
    new "{b}预测{/b}"

    # game/BKscreens.rpy:4655
    old "All customers have been assigned."
    new "所有顾客都得到了服务。"

    # game/BKscreens.rpy:4666
    old "No customers."
    new "没有顾客。"

    # game/BKscreens.rpy:4838
    old "{color=[c_white]}I{/color}"
    new "{color=[c_white]}我{/color}"

    # game/BKscreens.rpy:4902
    old "Click to keep the brothel report showing at all times."
    new "点击后 青楼报告将保持显示状态"

    # game/BKscreens.rpy:4905
    old "Click to hide the brothel report."
    new "点击后 取消青楼报告保持显示"

    # game/BKscreens.rpy:4915
    old "Show brothel report"
    new "青楼报告"

    # game/BKscreens.rpy:4940
    old "Master!!! "
    new "主人!!!"

    # game/BKscreens.rpy:4940
    old " is very dirty... Please do something!"
    new "这里到处都是垃圾，臭不可闻！得赶快清理了！"


    # game/BKscreens.rpy:4943
    old "Welcome to your new brothel, Master! I'm sure you will be a great manager!"
    new "欢迎回来,主人!我相信我们的青楼会蒸蒸日上的!"

    # game/BKscreens.rpy:4946
    old "{color=[c_lightblue]}Did you know? {/color}"
    new "{color=[c_lightblue]}你知道吗?{/color}"

    # game/BKscreens.rpy:4949
    old "is losing money... What's going on?"
    new "我们一直在亏损，再这样下去要破产了！"

    # game/BKscreens.rpy:4966
    old "Show last night's log"
    new "显示昨晚的报告"

    # game/BKscreens.rpy:4969
    old "Show satisfaction report"
    new "显示满意度报告"

    # game/BKscreens.rpy:4974
    old "Nothing to report"
    new "无可报告项目"

    # game/BKscreens.rpy:4987
    #old "Today"
    #new "今天"


    # game/BKscreens.rpy:5472
    old "{i}No girls are available for this task{/i}"
    new "{i}没有女孩符合这个任务的需求{/i}"

    # game/BKscreens.rpy:5489
    #old "Character"
    #new "角色"

    # game/BKscreens.rpy:5491
    #old "Difficulty"
    #new "难度"

    # game/BKscreens.rpy:5510
    old "Name: "
    new "名称: "

    # game/BKscreens.rpy:5519
    #old "Class"
    #new "职业"

    # game/BKscreens.rpy:5537
    #old "Religion"
    #new "信仰"

    old "Change your Main Character's picture."
    new "更改主角的图片."

    # game/BKscreens.rpy:5611
    old "Click on a girl mix to add or remove it from this game."
    new "点击一个女孩组合可将其添加或从此游戏中删除。"

    # game/BKscreens.rpy:5633
    old "Edit girl mixes"
    new "编辑女孩组合"

    # game/BKscreens.rpy:5648
    old "Note: Choosing any of these options will disable achievements for this game."
    new "注意:变更任何选项都会禁用此游戏的成就。"

    # game/BKscreens.rpy:5651
    old "Starting gold: "
    new "起始资金: "

    # game/BKscreens.rpy:5655
    old "Starting chapter: Chapter %s"
    new "起始章节: 第 %s 章"

    # game/BKscreens.rpy:5668
    old "Chapter 1 ending: %s"
    new "第1章结局: %s"

    # game/BKscreens.rpy:5672
    old "Farm: %s"
    new "奴隶农场: %s"

    # game/BKscreens.rpy:5674
    old "Carpenter: %s"
    new "工匠: %s"

    # game/BKscreens.rpy:5676
    old "Locations: %s"
    new "位置: %s"

    # game/BKscreens.rpy:5678
    old "Shops: %s"
    new "商店: %s"

    # game/BKscreens.rpy:5684
    old "Resources: %s"
    new "资源: %s"

    # game/BKscreens.rpy:5690
    old "Trainers: %s"
    new "训练师: %s"

    # game/BKscreens.rpy:5694
    old "Reset extras"
    new "重置额外设置"

    # game/BKscreens.rpy:5701
    old "Achievements will be enabled for this game."
    new "此游戏将启用成就系统。"

    # game/BKscreens.rpy:5703
    old "Achievements will be disabled for this game."
    new "此游戏将禁用成就系统。"

    # game/BKscreens.rpy:5705
    old "CONFIRM"
    new "确认"

    # game/BKscreens.rpy:5832
    old "Active:"
    new "当前:"

    # game/BKscreens.rpy:5847
    old "%s's Spellbook"
    new "%s 的法术书"

    # game/BKscreens.rpy:5859
    old "Right-click on a spell to set-up auto-cast"
    new "右击法术设置为自动施放"

    # game/BKscreens.rpy:5932
    old "/night"
    new "/夜晚"

    # game/BKscreens.rpy:5946
    old "{i}You do not know any spells yet. You must increase your level.{/i}"
    new "{i}你还没有学会任何法术。请提升等级。{/i}"

    # game/BKscreens.rpy:6023
    #old "Duration"
    #new "持续时间"

    # game/BKscreens.rpy:6031
    #old "Cost"
    #new "消耗"

    # game/BKscreens.rpy:6037
    #old "Enrolled"
    #new "已报名"

    # game/BKscreens.rpy:6043
    #old "Gains"
    #new "收益"

    # game/BKscreens.rpy:6058
    old "\nMax skill: "
    new "\n最高等级: "

    # game/BKscreens.rpy:6064
    #old "Reward"
    #new "奖励"

    # game/BKscreens.rpy:6070
    #old "Requirements"
    #new "要求"

    # game/BKscreens.rpy:6078
    #old "Desirable"
    #new "推荐"

    # game/BKscreens.rpy:6084
    #old "Undesirable"
    #new "不推荐"

    # game/BKscreens.rpy:6089
    old "No task is currently available."
    new "没有可以接取任务。"

    # game/BKscreens.rpy:6128
    old "Quests"
    new "任务"

    # game/BKscreens.rpy:6129
    old "Classes"
    new "培训班"

    # game/BKscreens.rpy:6188
    old "Active contract"
    new "签订的合同"

    # game/BKscreens.rpy:6224
    old "Pers. "
    new "个人信息"

    # game/BKscreens.rpy:6225
    old "Tastes"
    new "喜恶"

    # game/BKscreens.rpy:6226
    old "Sex. "
    new "性倾向"

    # game/BKscreens.rpy:6227
    old "Events"
    new "事件"

    # game/BKscreens.rpy:6288
    old "Farm Training Menu"
    new "奴隶农场调教清单"

    # game/BKscreens.rpy:6350
    old "Farm activities"
    new "奴隶训练项目"

    # game/BKscreens.rpy:6367
    old "Sexual Training"
    new "性技培训"

    # game/BKscreens.rpy:6388
    old "Everything (%s)"
    new "所有内容 ( %s )"

    # game/BKscreens.rpy:6394
    old "Facility"
    new "设施"

    # game/BKscreens.rpy:6406
    old "Farm pen"
    new "奴隶农场牢房"

    # game/BKscreens.rpy:6420
    old "Auto."
    new "自动"

    # game/BKscreens.rpy:6470
    old "Hold her (rest)"
    new "让她休息"

    # game/BKscreens.rpy:6475
    old "Train her (%s)"
    new "训练她 (%s)"

    # game/BKscreens.rpy:6510
    old "Gizel's Farm"
    new "吉泽的奴隶农场"

    # game/BKscreens.rpy:6562
    old "{b}Girl pens{/b}"
    new "{b}女孩牢房{/b}"

    # game/BKscreens.rpy:6595
    old "{b}Farm status{/b}"
    new "{b}农场状态{/b}"

    # game/BKscreens.rpy:6600
    old "Hurt minions: "
    new "受伤的仆从: "

    # game/BKscreens.rpy:6610
    old "Nothing to report."
    new "没有任何报告"

    # game/BKscreens.rpy:6615
    old "{b}Facilities & Minions{/b}"
    new "{b}设施和仆从{/b}"

    # game/BKscreens.rpy:6616
    old "{u}U{/u}se item"
    new "使用物品"

    # game/BKscreens.rpy:6724
    old "Current sanity: "
    new "当前理智值: "

    # game/BKscreens.rpy:6817
    old "Unknown"
    new "未知"

    # game/BKscreens.rpy:7101
    old "Weekly trade information"
    new "每周交易信息"

    # game/BKscreens.rpy:7128
    old "Your resources"
    new "您的资源"

    # game/BKscreens.rpy:7156
    old "In storage: "
    new "存储中: "

    # game/BKscreens.rpy:7166
    old "Market resources"
    new "市场资源"

    # game/BKscreens.rpy:7351
    old "Mod Setup"
    new "MOD设置"

    # game/BKscreens.rpy:7397
    old "Active Mod"
    new "启用MOD"

    # game/BKscreens.rpy:7399
    old "Inactive Mod"
    new "禁用MOD"

    # game/BKscreens.rpy:7408
    old "Deactivate Mod"
    new "停用MOD"

    # game/BKscreens.rpy:7410
    old "Activate Mod"
    new "激活MOD"

    # game/BKscreens.rpy:7441
    old "City girl interactions"
    new "城市女孩互动"

    # game/BKscreens.rpy:7532
    old "Girl interactions"
    new "女孩互动"

    # game/BKscreens.rpy:7569
    #old "Talk"
    #new "对话"

    # game/BKscreens.rpy:7547
    #old "Train"
    #new "训练"

    # game/BKscreens.rpy:7547
    #old "Magic"
    #new "催眠"

    # game/BKscreens.rpy:7547
    #old "React"
    #new "奖惩"

    # game/BKscreens.rpy:7547
    #old "Misc"
    #new "其他"

    # game/BKscreens.rpy:7599
    #old "Advanced"
    #new "进阶"

    # game/BKscreens.rpy:7636
    #old "OTHER"
    #new "其他"

    # game/BKscreens.rpy:7690
    old "\nActivate or deactivate objectionable content (won't affect story scenes)"
    new "\n启用或禁用令人反感的内容（不会影响剧情）"

    # game/BKscreens.rpy:7731
    old "\nChoose the behavior of stock (default) pictures and girl pack pictures"
    new "\n选择默认图片和女孩包图片的行为"

    old "When a picture is missing:\nUse stock pictures"
    new "找不到可用图片时：使用默认图片库"

    old "When a picture is missing:\nUse another picture from the girl pack"
    new "找不到可用图片时：随机一位女孩图片包"

    old "When the picture count is low:\nMix stock and girl pack pictures"
    new "当图片数量太少时：混合使用图库和女孩图片包"

    old "When the picture count is low:\nOnly use girl pack pictures"
    new "当图片数量太少时：只使用该女孩图片包"

    # game/BKscreens.rpy:7748
    old "\nChoose the behavior of advanced training pictures"
    new "\n选择高级培养图片的行为"

    # game/BKscreens.rpy:7750
    old "Advanced training pictures: Better variety"
    new "高级培养图片：更好的多样性"

    # game/BKscreens.rpy:7752
    old "Advanced training pictures: Better accuracy"
    new "高级培养图片：更高的准确性"

    # game/BKscreens.rpy:7756
    old "\nChoose the behavior of group and bisexual pictures"
    new "\n选择群交和双性图片的行为"

    old "Group sex: Mix group pictures with regular pictures"
    new "群交：将群交图片与普通图片混合"

    old "Group sex: Only use group sex pictures"
    new "群交：只使用群交图片"

    old "Bisexual sex: Mix bisexual pictures with regular pictures"
    new "双飞：将双飞图片与普通图片混合"

    old "Bisexual sex: Only use bisexual pictures"
    new "双飞：只使用双飞图片"

    # game/BKscreens.rpy:7763
    old "\nChoose the behavior of slavemarket girls' pictures"
    new "\n选择奴隶市场女孩图片的行为"

    old "Allow naked pictures in the slavemarket"
    new "允许奴隶市场上的裸体图片"

    old "No naked pictures in the market"
    new "不允许奴隶市场上的裸体图片"

    # game/BKscreens.rpy:7766
    old "\nChoose the behavior of free girls' pictures"
    new "\n选择自由女孩图片的行为"

    old "Allow naked pictures in the city (naturist trait)"
    new "允许在城市中出现裸体图片(裸体主义特质)"

    old "No naked pictures in the city"
    new "不允许在城市中出现裸体图片"

    # game/BKscreens.rpy:7770
    old "\nAllow 'close enough' tags to be used (e.g. 'swimsuit' for masseuse events)"
    new "\n允许使用'近似'标签（例如 '泳衣'用于按摩事件）"

    # game/BKscreens.rpy:7782
    old "\nDisplay pack rating on girl profiles"
    new "\n在女孩简介上显示包评级"

    # game/BKscreens.rpy:7789
    old "Show Girl Pack Rating: [text1]"
    new "显示女孩包评级: [text1]"

    # game/BKscreens.rpy:7793
    old "\nDisplay notifications next to buttons on the home screen"
    new "\n在主页上的按钮旁显示通知"

    old "Flashing notifications (default)"
    new "闪烁通知（默认）"

    old "Static notifications"
    new "静态通知"

    old "No notifications"
    new "没有通知"

    # game/BKscreens.rpy:7805
    old "\nDisplay girl status icons"
    new "\n显示女孩状态图标"

    # game/BKscreens.rpy:7827
    old "\nYour preferences for setting girl badges"
    new "\n您对设置女孩徽章的偏好"

    old "Badges can be modified directly on girls' portraits"
    new "徽章可以直接在女孩的肖像上修改"

    old "Badges can only be modified on a girl's profile"
    new "徽章只能在女孩的个人资料上修改"

    # game/BKscreens.rpy:7832
    old "\nShow/Skip night events"
    new "\n显示/跳过夜间事件"

    # game/BKscreens.rpy:7848
    old "Block skipping on night reports: "
    new "停止跳过夜间报告: "

    # game/BKscreens.rpy:7849
    old "Block skipping on final recap: "
    new "阻止跳过最终总结: "

    # game/BKscreens.rpy:7857
    old "\nChoose naming options for non-original girls"
    new "\n为非原创女孩选择命名选项"

    # game/BKscreens.rpy:7861
    old "Keep First Name: %s"
    new "保留名字: %s "

    # game/BKscreens.rpy:7862
    old "Keep Last Name: %s"
    new "保留姓氏: %s "

    # game/BKscreens.rpy:7863
    old "Prioritize _BK.ini clone name settings (when available): %s"
    new "优先使用_BK.ini克隆名称设置(如有): %s "

    # game/BKscreens.rpy:7885
    old "SOFT"
    new "温柔"

    # game/BKscreens.rpy:7886
    old "HARD"
    new "强硬"

    # game/BKscreens.rpy:7887
    old "FARM"
    new "农场"

    # game/BKscreens.rpy:7888
    old "FIX"
    new "修复"

    # game/BKscreens.rpy:7899
    # old "Portrait"
    # new "头像"

    # # game/BKscreens.rpy:7900
    # old "Portrait Naked"
    # new "裸体肖像"

    # # game/BKscreens.rpy:7901
    # old "Profile"
    # new "侧身照"

    # # game/BKscreens.rpy:7902
    # old "Profile Naked"
    # new "裸体侧身照"

    # # game/BKscreens.rpy:7905
    # old "Rest Naked"
    # new "裸睡休息"

    # # game/BKscreens.rpy:7907
    # old "Waitress Naked"
    # new "毫无遮掩的服务员"

    # # game/BKscreens.rpy:7909
    # old "Dancer Naked"
    # new "赤身裸体的表演艺伎"

    # # game/BKscreens.rpy:7911
    # old "Masseuse Naked"
    # new "不着寸缕的按摩技师"

    # # game/BKscreens.rpy:7913
    # old "Geisha Naked"
    # new "一丝不挂的脱衣舞娘"

    # game/BKscreens.rpy:7920
    # old "Naked"
    # new "裸体"

    # game/BKscreens.rpy:7921
    #old "Service"
    #new "侍奉"

    # game/BKscreens.rpy:7922
    #old "Sex"
    #new "性交"

    # game/BKscreens.rpy:7923
    #old "Anal"
    #new "肛交"

    # game/BKscreens.rpy:7924
    #old "Fetish"
    #new "调教"

    # game/BKscreens.rpy:7925
    # old "Bisexual Service"
    # new "双性侍奉"

    # # game/BKscreens.rpy:7926
    # old "Bisexual Sex"
    # new "双性性交"

    # # game/BKscreens.rpy:7927
    # old "Bisexual Anal"
    # new "双性肛交"

    # # game/BKscreens.rpy:7928
    # old "Bisexual Fetish"
    # new "被双性调教"

    # # game/BKscreens.rpy:7929
    # old "Group Service"
    # new "群体侍奉"

    # # game/BKscreens.rpy:7930
    # old "Group Sex"
    # new "多人性交"

    # # game/BKscreens.rpy:7931
    # old "Group Anal"
    # new "多人肛交"

    # # game/BKscreens.rpy:7932
    # old "Group Fetish"
    # new "多人调教"

    # # game/BKscreens.rpy:7936
    # old "Stallion Naked"
    # new "与种马裸体相处"

    # # game/BKscreens.rpy:7937
    # old "Stallion Service"
    # new "侍奉种马"

    # # game/BKscreens.rpy:7938
    # old "Stallion Sex"
    # new "种马性交"

    # # game/BKscreens.rpy:7939
    # old "Stallion Anal"
    # new "种马肛交"

    # # game/BKscreens.rpy:7940
    # old "Stallion Fetish"
    # new "种马调教"

    # # game/BKscreens.rpy:7941
    # old "Stallion Bisexual"
    # new "种马双飞"

    # # game/BKscreens.rpy:7942
    # old "Stallion Group"
    # new "种马群交"

    # # game/BKscreens.rpy:7944
    # old "Beast Naked"
    # new "与野兽裸体相处"

    # # game/BKscreens.rpy:7945
    # old "Beast Service"
    # new "侍奉野兽"

    # # game/BKscreens.rpy:7946
    # old "Beast Sex"
    # new "与野兽性交"

    # # game/BKscreens.rpy:7947
    # old "Beast Anal"
    # new "与野兽肛交"

    # # game/BKscreens.rpy:7948
    # old "Beast Fetish"
    # new "被野兽调教"

    # # game/BKscreens.rpy:7949
    # old "Beast Bisexual"
    # new "与野兽双飞"

    # # game/BKscreens.rpy:7950
    # old "Beast Group"
    # new "与野兽群交"

    # # game/BKscreens.rpy:7952
    # old "Monster Naked"
    # new "怪物裸体"

    # # game/BKscreens.rpy:7953
    # old "Monster Service"
    # new "侍奉怪物"

    # # game/BKscreens.rpy:7954
    # old "Monster Sex"
    # new "与怪物性交"

    # # game/BKscreens.rpy:7955
    # old "Monster Anal"
    # new "与怪物肛交"

    # # game/BKscreens.rpy:7956
    # old "Monster Fetish"
    # new "被怪物调教"

    # # game/BKscreens.rpy:7957
    # old "Monster Bisexual"
    # new "与怪物双飞"

    # # game/BKscreens.rpy:7958
    # old "Monster Group"
    # new "与怪物群交"

    # # game/BKscreens.rpy:7960
    # old "Machine Naked"
    # new "机械裸体"

    # # game/BKscreens.rpy:7961
    # old "Machine Service"
    # new "机械侍奉"

    # # game/BKscreens.rpy:7962
    # old "Machine Sex"
    # new "机械性交"

    # # game/BKscreens.rpy:7963
    # old "Machine Anal"
    # new "机械肛交"

    # # game/BKscreens.rpy:7964
    # old "Machine Fetish"
    # new "被机械调教"

    # # game/BKscreens.rpy:7965
    # old "Machine Bisexual"
    # new "机械双飞"

    # # game/BKscreens.rpy:7966
    # old "Machine Group"
    # new "机械群交"

    # game/BKscreens.rpy:8024
    old "Girl Mix"
    new "女孩组合"

    # game/BKscreens.rpy:8032
    old "Click on a girl's profile to add or remove this girl from the mix.\nYou can create a new mix by clicking '+'"
    new "点击女孩的头像可以添加或删除该女孩。\n你可以点击'+'创建一个新的组合。"

    # game/BKscreens.rpy:8077
    old "by "
    new "由 "

    # game/BKscreens.rpy:8079
    old "{size=14}Rating: {/size}"
    new "{size=14}评分: {/size}"

    # game/BKscreens.rpy:8085
    old "Delete mix"
    new "删除组合"

    # game/BKscreens.rpy:8088
    old "Add all"
    new "全部添加"

    # game/BKscreens.rpy:8089
    old "Remove all"
    new "全部移除"

    # game/BKscreens.rpy:8117
    old "Not found"
    new "未找到"

    # game/BKscreens.rpy:8150
    old "Next unlock:"
    new "下一个解锁:"

    # game/BKscreens.rpy:8156
    old "Reset achievements"
    new "重置成就"

    # game/BKscreens.rpy:8159
    old "Reset achievements (%s)"
    new "重置成就 (%s)"

    # game/BKscreens.rpy:8198
    old "Choose a contract"
    new "选择一份合同"

    # game/BKscreens.rpy:8207
    old "Fee: %s gold."
    new "费用: %s 金币."

    # game/BKscreens.rpy:8232
    old "The "
    new "这 "

    # game/BKscreens.rpy:8243
    old "Tasks"
    new "任务内容"

    # game/BKscreens.rpy:8252
    old "Bonus requirement"
    new "额外要求"

    # game/BKscreens.rpy:8256
    old "Reward: "
    new "报酬: "

    # game/BKscreens.rpy:8257
    old "%s gold"
    new "%s 金币"

    # game/BKscreens.rpy:8268
    old "Send"
    new "发送"

    # game/BKscreens.rpy:8304
    old "{color=[c_red]}{i}Failed{/i}{/color}"
    new "{color=[c_red]}{i}失败{/i}{/color}"

    # game/BKscreens.rpy:8314
    old "{i}Missing{/i}"
    new "{i}缺失{/i}"

    # game/BKscreens.rpy:8317
    old "Score"
    new "得分"

    # game/BKscreens.rpy:8412
    old "{image=tb goal} Chapter %i - Your Goals"
    new "{image=tb goal} 第%i章 - 你的目标"

    # game/BKscreens.rpy:8458
    old "Mojo cost:"
    new "魔力消耗:"

    # game/BKscreens.rpy:8472
    old "Sanity cost:"
    new "理智消耗:"

    # game/BKscreens.rpy:8477
    old "Target:"
    new "目标:"

    # game/BKscreens.rpy:8482
    old "Duration:"
    new "持续时间:"

    # game/BKscreens.rpy:8483
    old "[pow.duration] days"
    new "[pow.duration] 天"

    # game/BKscreens.rpy:8495
    old "Draw a card"
    new "抽一张牌"

    # game/BKscreens.rpy:8522
    old "You have used all of your powers this week."
    new "你本周已用完所有能力。"

    # game/BKscreens.rpy:8683
    old "Conduit: "
    new "导体: "

    # game/BKscreens.rpy:8691
    old "Target: "
    new "目标: "

    # game/BKscreens.rpy:8696
    old "Herself"
    new "她自己"

    # game/BKscreens.rpy:8721
    old "Commit"
    new "提交"

    # game/BKscreens.rpy:8761
    old "No girls are available."
    new "没有女孩可用。"

    # game/BKscreens.rpy:8794
    old "Supercharge: "
    new "超充电: "

    # game/BKscreens.rpy:8795
    old "Conduit: : "
    new "导管: : "

    # game/BKscreens.rpy:8799
    old "Payment: "
    new "付款: "

    # game/BKscreens.rpy:8832
    old "Cast"
    new "施放"

    # game/BKscreens.rpy:8858
    old "Current rate:\n{b}%i {image=mojo purple} for %i {image=mojo green}{image=mojo blue}{image=mojo red}{image=mojo yellow}{/b}"
    new "当前费率:\n{b}%i {image=mojo purple} 对于 %i {image=mojo green}{image=mojo blue}{image=mojo red}{image=mojo yellow}{/b}"

    # game/BKscreens.rpy:8902
    old "{b}Clear{/b}"
    new "{b}清除{/b}"

    # game/BKscreens.rpy:8903
    old "{b}Commit{/b}"
    new "{b}提交{/b}"


###########################################################
#  ________  ________  ________        ___  ________      #
# |\   __  \|\   __  \|\   __  \      |\  \|\_____  \     #
# \ \  \|\  \ \  \|\  \ \  \|\  \     \ \  \\|___/  /|    #
#  \ \   ____\ \   _  _\ \  \\\  \  __ \ \  \   /  / /    #
#   \ \  \___|\ \  \\  \\ \  \\\  \|\  \\_\  \ /  /_/__   #
#    \ \__\    \ \__\\ _\\ \_______\ \________\\________\ #
#     \|__|     \|__|\|__|\|_______|\|________|\|_______| #
#                                                         #
#  This rpy file is generated by the project:             #
#  https://github.com/abse4411/projz_renpy_translation    #
###########################################################
# TODO: Translation updated at 2024-07-31 02:24

translate schinese strings:

    # game/BKscreens.rpy:388
    old "[calendar.day]日 ("
    new "[calendar.day]日 ("

    # game/BKscreens.rpy:413
    old "行动力: 你今天还能行动的次数."
    new "行动力: 你今天还能行动的次数."

    # game/BKscreens.rpy:423
    old "魔力: 你剩余的魔力量."
    new "魔力: 你剩余的魔力量."

    # game/BKscreens.rpy:700
    old " 是一个 {0} 等级的 {1}."
    new " 是一个 {0} 等级的 {1}."

    # game/BKscreens.rpy:703
    old "{/b}正在休息."
    new "{/b}正在休息."

    # game/BKscreens.rpy:729
    old ". 点击查看详细信息."
    new ". 点击查看详细信息."

    # game/BKscreens.rpy:776
    old "她还有"
    new "她还有"

    # game/BKscreens.rpy:776
    old "点精力, 其精力上限是"
    new "点精力, 其精力上限是"

    # game/BKscreens.rpy:1134
    old "打开 %s 的时间表"
    new "打开 %s 的时间表"

    # game/BKscreens.rpy:1141
    old "在您的魔法笔记本中打开 %s 的条目"
    new "在您的魔法笔记本中打开 %s 的条目"

    # game/BKscreens.rpy:1462
    old "点击更改她的名字"
    new "点击更改她的名字"

    # game/BKscreens.rpy:1497
    old "{b}手无寸铁"
    new "{b}手无寸铁"

    # game/BKscreens.rpy:1499
    old "{b}娇花易折"
    new "{b}娇花易折"

    # game/BKscreens.rpy:1501
    old "{b}红颜侠客"
    new "{b}红颜侠客"

    # game/BKscreens.rpy:1503
    old "{b}绝色杀手"
    new "{b}绝色杀手"

    # game/BKscreens.rpy:1505
    old "{b}致命诱惑"
    new "{b}致命诱惑"

    # game/BKscreens.rpy:1507
    old "{/b}\n数值越高女孩的自卫能力越强，让女孩装备武器可以提高她的自卫等级"
    new "{/b}\n数值越高女孩的自卫能力越强，让女孩装备武器可以提高她的自卫等级"

    # game/BKscreens.rpy:1705
    old "她还有{b}"
    new "她还有{b}"

    # game/BKscreens.rpy:1705
    old "{/b}点精力。她的最大精力为{b}"
    new "{/b}点精力。她的最大精力为{b}"

    # game/BKscreens.rpy:1705
    old "{/b} (增加体格可以提高精力上限)。"
    new "{/b} (增加体格可以提高精力上限)。"

    # game/BKscreens.rpy:1798
    old " 事先的训练可能会使女孩更适合性行为."
    new " 事先的训练可能会使女孩更适合性行为."

    # game/BKscreens.rpy:1845
    old "这将为这个女孩激活{b}"
    new "这将为这个女孩激活{b}"

    # game/BKscreens.rpy:1845
    old "{/b}."
    new "{/b}."

    # game/BKscreens.rpy:2292
    old " 这将花费 "
    new " 这将花费 "

    # game/BKscreens.rpy:2332
    old "这个女孩受伤了，需要再休息一天，直到她准备好做任何事情."
    new "这个女孩受伤了，需要再休息一天，直到她准备好做任何事情."

    # game/BKscreens.rpy:2338
    old "她需要得到完全休息后才能开始工作."
    new "她需要得到完全休息后才能开始工作."

    # game/BKscreens.rpy:2342
    old "排班表显示她今天休息."
    new "排班表显示她今天休息."

    # game/BKscreens.rpy:2346
    old "没有安排工作. 在得到新的指示前她会一直休息."
    new "没有安排工作. 在得到新的指示前她会一直休息."

    # game/BKscreens.rpy:2350
    old "一边工作一边勾引客人. 点击更换工作或让她去休息."
    new "一边工作一边勾引客人. 点击更换工作或让她去休息."

    # game/BKscreens.rpy:2354
    old "点击更换工作或让她去休息."
    new "点击更换工作或让她去休息."

    # game/BKscreens.rpy:2608
    old "熟练度"
    new "熟练度"

    # game/BKscreens.rpy:3991
    old "d){/size}"
    new "d){/size}"

    # game/BKscreens.rpy:4452
    old "d"
    new "d"

    # game/BKscreens.rpy:5351
    old "扩容商店的货架 (+%s 个  %s 商品) 。需要 %s %s."
    new "扩容商店的货架 (+%s 个  %s 商品) 。需要 %s %s."

    # game/BKscreens.rpy:5377
    old "显示 %s 物品."
    new "显示 %s 物品."

    # game/BKscreens.rpy:5451
    old "\n无工作"
    new "\n无工作"

    # game/BKscreens.rpy:5704
    old "{b}%s, %s{/b} ({b}%s{/b}) - {b}难度：%s{/b}"
    new "{b}%s, %s{/b} ({b}%s{/b}) - {b}难度：%s{/b}"

    # game/BKscreens.rpy:5768
    old "每次你和女孩发生性关系，或者当你的女孩升级时，你都会获得声望点数。"
    new "每次你和女孩发生性关系，或者当你的女孩升级时，你都会获得声望点数。"

    # game/BKscreens.rpy:5771
    old "\n你已经达到了最大等级。"
    new "\n你已经达到了最大等级。"

    # game/BKscreens.rpy:5773
    old "\n你需要 "
    new "\n你需要 "

    # game/BKscreens.rpy:5773
    old " 声望升到下一级。"
    new " 声望升到下一级。"

    # game/BKscreens.rpy:5787
    old " 天赋点"
    new " 天赋点"

    # game/BKscreens.rpy:6030
    old " 天"
    new " 天"

    # game/BKscreens.rpy:6155
    old "这个任务需要 "
    new "这个任务需要 "

    # game/BKscreens.rpy:6156
    old " 女孩可以完成这项任务。"
    new " 女孩可以完成这项任务。"

    # game/BKscreens.rpy:6160
    old "专门课程可能会提高 "
    new "专门课程可能会提高 "

    # game/BKscreens.rpy:6810
    old "性行为"
    new "性行为"

    # game/BKscreens.rpy:6811
    old "偏好"
    new "偏好"

    # game/BKscreens.rpy:6812
    old "训练意向"
    new "训练意向"

    # game/BKscreens.rpy:6813
    old "工作意向"
    new "工作意向"

    # game/BKscreens.rpy:7571
    old "你知道 [girl.name] 对"
    new "你知道 [girl.name] 对"

    # game/BKscreens.rpy:7573
    old "你不知道 [girl.name] 对"
    new "你不知道 [girl.name] 对"

    # game/BKscreens.rpy:7573
    old "是什么感觉。"
    new "是什么感觉。"

    # game/BKscreens.rpy:7862
    old "杂项"
    new "杂项"

    # game/BKscreens.rpy:7992
    old "女孩包"
    new "女孩包"

    # game/BKscreens.rpy:7992
    old "更新组合包"
    new "更新组合包"

    # game/BKscreens.rpy:7995
    old "不必了"
    new "不必了"

    # game/BKscreens.rpy:8919
    old "组合包介绍"
    new "组合包介绍"

    # game/BKscreens.rpy:8919
    old "无法识别的图片: [preferences.packstate_unrecognized]"
    new "无法识别的图片: [preferences.packstate_unrecognized]"

    # game/BKscreens.rpy:8919
    old "模拟"
    new "模拟"

    # game/BKscreens.rpy:8919
    old "应用组合包"
    new "应用组合包"

    # game/BKscreens.rpy:8934
    old "屏蔽"
    new "屏蔽"

    # game/BKscreens.rpy:8934
    old "重命名"
    new "重命名"

    # game/BKscreens.rpy:8934
    old "忽略"
    new "忽略"

    # game/BKscreens.rpy:8934
    old "返回 (不修改设置)"
    new "返回 (不修改设置)"

