    ...
    python:
        ...
        spellbook = {
                    ...
                    "Trader" : [
                            ...
                            Spell('街头霸王', 'strength.jpg', level=6, effects=[Effect("gain", "strength", 1)], description="博尔戈卑鄙的街道让你了解了所有关于街头斗殴的知识：总是从背后捅刀子。"),
                    ...
                    "Wizard" : [
                            ...
                            Spell('创意', 'spirit.webp', level=10, effects=[Effect("gain", "spirit", 1)], description="Now, who would have thought of using a magic wand for that..."),
...
label init_moons():
    python:
        moons = {
                 1 : Moon("金", pic="gold", effects=[Effect("boost", "income", value = 0.5, scope = "brothel")], description="A new year is just starting in Xeros! This is the most important holiday of the year. People are encouraged to spend a lot to show off their good fortune.", sound = s_gold),
                 2 : Moon("狼", pic="wolf", effects=[Effect("boost", "city rewards", 1, scope="brothel"), Effect("change", "city rewards", 1, scope="brothel"), Effect("boost", "resource extraction", 1, scope="brothel")], description="This month is the coldest of the year. It is said that adventurers that brave this tough season can reap the best rewards.", sound = s_wolf),
                 3 : Moon("蓝", pic="blue", effects=[Effect("change", "customers", 2, scope="brothel", scales_with="district"), Effect("change", "mood modifier", -1)], description="On a blue moon, people feel depressed and lonely. Many look for company in the town's brothels.", sound = s_crowd_cheer),
                 4 : Moon("血", pic="blood", effects=[Effect("boost", "crazy", 1, scope="brothel"), Effect("change", "threat", 2, scales_with = "district")], description="Something about this moon drives otherwise normal people crazy. Violent crimes always peak during this month.", sound = s_fire),
                 5 : Moon("密", pic="honey", effects=[Effect("boost", "love gains", 1, scope="world"), Effect("boost", "fear gains", -0.5, scope="world")], description="A favorite month for lovers to elope together, their passion bright and short-lived as a spring flower.", sound = s_potion),
                 ...
                 7 : Moon("狩", pic="hunter", effects=[Effect("boost", "quest rewards", 1, scope="brothel"), Effect("boost", "class results", 1, scope="brothel"), Effect("boost", "contract rewards", 0.5, scope="brothel"), Effect("boost", "income", -0.25, scope="brothel")], description="Hunting season is at its peak. This month was especially holy to the elder races.", sound = s_sheath),
                 8 : Moon("农", pic="harvest", effects=[Effect("boost", "farm preference increase", 1, scope="farm"), Effect("boost", "all sex acts preference increase", -0.5, scope="brothel")], description="It's time to harvest your crops, tend to your cattle, find a partner, and bang her brains out.", sound = s_moo),
                 9 : Moon("银", pic="silver", effects=[Effect("change", "mana", 1, scope="brothel", scales_with="spirit")], description="The silver moon is at its peak magical energy. Experienced spellcasters are careful to cast their most powerful spells and invocations on this month.", sound = s_spell),
                 ...
                 11 : Moon("皎", pic="hallow", effects=[Effect("boost", "threat", 0.3, scope="brothel"), Effect("boost", "crazy", 2, scope="brothel"), Effect("change", "customer defense", 2, scope="brothel")], description="This pagan holly month is the time of the year when monsters and spirits thrive, and the kids in Zan play 'Tricks or Tits'.", sound = s_maniacal_laugh),
                 12 : Moon("暗", pic="dark", effects=[Effect("boost", "love gains", -0.5, scope="world"), Effect("boost", "fear gains", 1, scope="world")], description="Under a dark moon, everything seems more frightening. This is the darkest of winter, only broken by the season festival Zan people call H-mas.", sound = s_mystery),
                }
    return
