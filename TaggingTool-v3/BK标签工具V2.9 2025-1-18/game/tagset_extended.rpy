...
init -1 python:
    ...
    tag_main_help.update({"market" : "{b}市场{/b} 图片表示女孩在等待有人购买她，例如在她身上贴着价格标签、在奴隶市场上、在牢房或笼子里等等。"})
    ...
    tag_main_help.update({"advertise" : "女孩穿着挑逗性的衣服或赤身裸体作宣传，打广告。"})
    ...
    tag_main_help.update({"ceremony" : "{b}仪式{/b} 表示女孩正在举行某种仪式、从事宗教工作、祈祷、冥想等。"})
    ...
    tag_main_help.update({"date" : "表示该女孩正在{b}与其他人约会{/b}。如果女孩正在“看电脑屏幕外”，则可以暗示另一个人。如果是另一个女人，请添加百合标签。"})
    ...
    tag_main_help.update({"party" : "女孩身处人群之中，通常与来自同一职业的女孩在一起。"})
    ...
    tag_special_help.update({"refuse" : "她拒绝接受对她的命令，例如表现出叛逆或不情愿，遮掩自己的身体等。"})
    offset += 1

    optional_tags.append(Tag(name="hurt", order=order1, order2=offset, tagsets=[tagset_extended]))
    tag_button2_dict.update({"hurt" : "受伤"})
    tag_special_help.update({"hurt" : "她（正在）受伤、生病或筋疲力尽。 可用于立绘图片和性图片。"})
    offset += 1
    ...
    tag_special_help.update({"hooker" : "她穿着的衣服清楚地表明她是个妓女。"})
    offset += 1
    ...
    tag_special_help.update({"teacher" : "她扮演着老师的角色，可能拿着戒尺或教鞭"})
    offset += 1
    ...
    tag_special_help.update({"nurse" : "她扮演着护士的角色，可能拿着一些医疗器械"})
    offset += 1
    ...
    tag_special_help.update({"catgirl" : "她扮演着兽耳娘的角色。 对于真正的兽娘角色，您不必添加此标签。"})
    offset += 1
    ...
    tag_special_help.update({"beach" : "图像表示女孩在海滩或站在海中。"})
    offset += 1
    ...
    tag_special_help.update({"blindfold" : "她的眼睛被眼罩遮挡住，看不到东西。"})
    offset += 1
    ...
    tag_special_help.update({"collar" : "她的脖子上戴着项圈。"})
    offset += 1
    ...
    tag_special_help.update({"futanari" : "她是一个扶她/有阴茎的女孩"})
    offset += 1
    ...
    tag_special_help.update({"cunnilingus" : "她的阴户正在被另一个人舔舐。"})
    offset += 1
    ...
    tag_special_help.update({"tempt" : "她在诱惑某人与她发生性关系。 例如揉捏自己的乳房，挑逗地张开她的腿、她的小穴或她的臀部。"})
    offset += 1
    ...
    tag_special_help.update({"panties" : "她的内裤是可见的。 如果她故意展示（例如掀起裙子）请增加性欲标签。 如果是意外露出内裤可以添加尴尬标签。 与裸体标签一起，此标签可以表示半裸。"})
    offset += 1
    ...
    tag_special_help.update({"sprite" : "图像是透明的，没有任何背景图层。"})
    offset += 1
    ...
    tag_special_help.update({"freq_low" : "图像将较少地显示。"})
    offset += 1
